...
fun functions : list (string * (list string * list string * cmd)) =>
    ...
    WeakestPrecondition.call functions ( "Vesta_add") t m0
    ...
    (exists (woutx wouty woutz : list Interface.word.rep) Rout,
        (
                (Vesta_add_Gallina_spec (toZ wX1) (toZ wY1) (toZ wZ1) (toZ wX2) (toZ wY2)
...
Lemma montadd_to_Mp x y z (Hx : valid' m bw n x) (Hy : valid' m bw n y) (Hz : valid' m bw n z) :
  montadd z x y -> (enc_mont VestaCurve_G1_bedrock.m bw n z Hz)
    = mont_add (enc_mont VestaCurve_G1_bedrock.m bw n x Hx) (enc_mont VestaCurve_G1_bedrock.m bw n y Hy).
Proof.
...
montsub z x y -> (enc_mont VestaCurve_G1_bedrock.m bw n z Hz)
  = mont_sub (enc_mont VestaCurve_G1_bedrock.m bw n x Hx) (enc_mont VestaCurve_G1_bedrock.m bw n y Hy).
Proof.
...
montmul z x y -> (enc_mont VestaCurve_G1_bedrock.m bw n z Hz)
  = mont_mul (enc_mont VestaCurve_G1_bedrock.m bw n x Hx) (enc_mont VestaCurve_G1_bedrock.m bw n y Hy).
Proof.
...
assert (H : MontgomeryRingTheory.valid' VestaCurve_G1_bedrock.m bw n (toZ x)) by (apply H'; assumption).
...
Theorem Vesta_add_func_ok : program_logic_goal_for_function! Vesta_add.
Proof.
  ...
  remember (MontgomeryCurveSpecs.three_b_mont VestaCurve_G1_bedrock.m bw n r' m'
