(** * Closes [Secp256k1_FpInv.by_convergence_dfg_secp256k1] axiom via the existing bridge.

    Mirrors [BLS12_FpInv_closed.v] for secp256k1.  Composes:

    - The generic [divsteps_bridge.oconnor_bridge] (parametric in the
      prime [M] and iteration count [N]),
    - The secp256k1-specific cert axiom
      [divsteps_secp256k1.secp256k1_certificate] (741 iterations),
    - A re-proof of the convergence-monotone lemma for secp256k1
    ...
    - A trivial [iter_divstep_dfg_eq] (the two definitions in
      [divsteps_bridge.v] and [Secp256k1_FpInv.v] are syntactically
      identical → defining-equation reflexivity).

    Result: [by_convergence_dfg_secp256k1_closed] has the exact
    statement of [Secp256k1_FpInv.by_convergence_dfg_secp256k1],
    discharging the axiom.

    Net axiom in the chain: just [secp256k1_certificate] (axiomatized
    in [divsteps_secp256k1.v] in parallel to [bls12_certificate];
    both can be independently verified by OCaml extraction via
...
Require Import divsteps_secp256k1.
...
(** The two [iter_divstep_dfg] definitions (one in [divsteps_bridge.v],
    one in [Secp256k1_FpInv.v]) have identical bodies; they coincide. *)
Lemma iter_divstep_dfg_eq : forall n d f g,
  divsteps_bridge.iter_divstep_dfg n d f g =
  Secp256k1_FpInv.iter_divstep_dfg n d f g.
Proof.
...
(** *** secp256k1-specific O'Connor convergence. *)
Theorem oconnor_implies_convergence_secp256k1 : forall x,
  0 < x < Secp256k1_FpInv.secp256k1_p ->
  Z.gcd x Secp256k1_FpInv.secp256k1_p = 1 ->
  let '(_, f_N, g_N) :=
    divsteps_bridge.iter_divstep_dfg (N.to_nat 741) 1 Secp256k1_FpInv.secp256k1_p x in
  g_N = 0 /\ (f_N = 1 \/ f_N = -1).
Proof.
  intros x Hx Hgcd.
  apply (divsteps_bridge.oconnor_bridge 741 divsteps_secp256k1.secp256k1_M).
  - exists (Secp256k1_FpInv.secp256k1_p / 2). vm_compute. reflexivity.
  - unfold divsteps_secp256k1.secp256k1_M, divsteps_secp256k1.secp256k1_p,
           Secp256k1_FpInv.secp256k1_p. lia.
  - lia.
  - apply Zgcd_1_rel_prime. rewrite Z.gcd_comm. exact Hgcd.
  - exact divsteps_secp256k1.secp256k1_certificate.
Qed.

(** *** Monotonicity for secp256k1. *)
Theorem convergence_monotone_secp256k1 : forall N x,
  (N.to_nat 741 <= N)%nat ->
  0 < x < Secp256k1_FpInv.secp256k1_p ->
  Z.gcd x Secp256k1_FpInv.secp256k1_p = 1 ->
  let '(_, f_N, g_N) :=
    divsteps_bridge.iter_divstep_dfg N 1 Secp256k1_FpInv.secp256k1_p x in
  g_N = 0 /\ (f_N = 1 \/ f_N = -1).
Proof.
  intros N x HN Hx Hgcd.
  pose proof (oconnor_implies_convergence_secp256k1 x Hx Hgcd) as H741.
  destruct (divsteps_bridge.iter_divstep_dfg (N.to_nat 741) 1
              Secp256k1_FpInv.secp256k1_p x)
...
(** Discharges [Secp256k1_FpInv.by_convergence_dfg_secp256k1]. *)
Theorem by_convergence_dfg_secp256k1_closed : forall x,
  0 < x < Secp256k1_FpInv.secp256k1_p ->
  Z.gcd x Secp256k1_FpInv.secp256k1_p = 1 ->
  let '(_, f_N, g_N) :=
    Secp256k1_FpInv.iter_divstep_dfg
      (Z.to_nat Secp256k1_FpInv.secp256k1_divstep_iters) 1
      Secp256k1_FpInv.secp256k1_p x in
  g_N = 0 /\ (f_N = 1 \/ f_N = -1).
Proof.
  intros x Hx Hgcd.
  rewrite <- iter_divstep_dfg_eq.
  apply convergence_monotone_secp256k1; auto.
Qed.
...
Local Notation p := Secp256k1_FpInv.secp256k1_p.
...
(* ================================================================ *)
(* The headline: fp_inv_correct without the by_convergence_dfg_secp  *)
(* axiom — goes through by_convergence_dfg_secp256k1_closed instead. *)
(* ================================================================ *)

Theorem fp_inv_correct_closed : forall x,
  0 < x < p ->
  Z.gcd x p = 1 ->
  (Secp256k1_FpInv.fp_inv_spec x * x) mod p = 1.
Proof.
  intros x Hx Hgcd.
  unfold Secp256k1_FpInv.fp_inv_spec.
  set (N := Z.to_nat Secp256k1_FpInv.secp256k1_divstep_iters).
  destruct (Secp256k1_FpInv.iter_divstep_spec p N 1 p x 0 1)
    as [[[[d_N f_N] g_N] v_N] r_N] eqn:Espec.
  pose proof (Secp256k1_FpInv.iter_invariant p N 1 p x 0 1 x
                Secp256k1_FpInv.p_pos Secp256k1_FpInv.p_odd) as Hinv.
  ...
  pose proof (Secp256k1_FpInv.iter_dfg_agree N p 1 p x 0 1) as Hagree.
  rewrite Espec in Hagree.
  destruct (Secp256k1_FpInv.iter_divstep_dfg N 1 p x) as [[d2 f2] g2] eqn:Edfg.
  destruct Hagree as [_ [Hf_eq Hg_eq]]. subst f2 g2.
  pose proof (by_convergence_dfg_secp256k1_closed x Hx Hgcd) as Hconv.
  unfold N in Edfg. rewrite Edfg in Hconv.
  destruct Hconv as [Hg0 Hf_pm1]. subst g_N.
  assert (Hp : p <> 0) by (pose proof Secp256k1_FpInv.p_pos; lia).
  Opaque Secp256k1_FpInv.secp256k1_p.
  ...
  - subst f_N. rewrite Z.mul_1_r in Hinv_v.
    ...
    transitivity ((2 ^ Z.of_nat N * Secp256k1_FpInv.precomp_val) mod p).
    { apply Zmul_mod_compat. exact Hvx. }
    rewrite Z.mul_comm.
    unfold N.
    rewrite Z2Nat.id by (unfold Secp256k1_FpInv.secp256k1_divstep_iters;
                         vm_compute; discriminate).
    exact Secp256k1_FpInv.precomp_times_pow2.
  - subst f_N.
