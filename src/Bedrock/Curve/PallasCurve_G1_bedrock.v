...
Import Syntax.Coercions.
Local Open Scope Z_scope.
Require Import Bedrock.Field.pallasprime.
...
(*Parameters for the Pallas curve.*)
    Local Notation m := pallasprime.m.
    Local Definition prefix := "pallas_"%string.
    Local Notation a := (0 mod m).
    Local Notation b := (5 mod m).

    Existing Instances Defaults64.default_parameters Defaults64.default_parameters_ok pallas_bedrock2_funcs
    pallas_bedrock2_specs pallas_bedrock2_correctness.
...
Definition Pallas_add : Syntax.func :=
    let outx := "outx" in
    let outy := "outy" in
    let outz := "outz" in
