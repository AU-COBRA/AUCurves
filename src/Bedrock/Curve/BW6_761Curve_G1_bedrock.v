...
Import Syntax.Coercions.
Local Open Scope Z_scope.
Require Import Bedrock.Field.bw6prime.
...
(*Parameters for the BW6-761 curve.*)
    Local Notation m := bw6prime.m.
    Local Definition prefix := "bw6_761_"%string.
    Local Notation a := (0 mod m).
    Local Notation b := ((m - 1) mod m).

    Existing Instances Defaults64.default_parameters Defaults64.default_parameters_ok bw6_bedrock2_funcs
    bw6_bedrock2_specs bw6_bedrock2_correctness.
...
Definition BW6_761_add_Gallina_spec :=
    BLS12_add_Gallina_spec m bw n m' a three_b.

Definition BW6_761_add_Gallina_spec_a_0 X1 Y1 Z1 X2 Y2 Z2 outx outy outz :=
  let X1 := evfrom X1 in
  let Y1 := evfrom Y1 in
  let Z1 := evfrom Z1 in
  let X2 := evfrom X2 in
...
Lemma add_specs_equiv : forall X1 X2 Y1 Y2 Z1 Z2 outx outy outz,
  BW6_761_add_Gallina_spec X1 X2 Y1 Y2 Z1 Z2 outx outy outz
  <-> BW6_761_add_Gallina_spec_a_0 X1 X2 Y1 Y2 Z1 Z2 outx outy outz.
Proof.
  intros X1 X2 Y1 Y2 Z1 Z2 outx outy outz.
  unfold BW6_761_add_Gallina_spec, MontgomeryCurveSpecs.BLS12_add_Gallina_spec.
  unfold BW6_761_add_Gallina_spec_a_0.
...
Definition BW6_761_add : Syntax.func :=
    let outx := "outx" in
    let outy := "outy" in
    let outz := "outz" in
              ...
              stackalloc num_bytes as t3 {
                stackalloc num_bytes as t4 {
                  stackalloc num_bytes as t5 {
                      ...
                      store(three_b + coq:(48), coq:(nth 6 three_b_mont 0));
                      store(three_b + coq:(56), coq:(nth 7 three_b_mont 0));
                      store(three_b + coq:(64), coq:(nth 8 three_b_mont 0));
                      store(three_b + coq:(72), coq:(nth 9 three_b_mont 0));
                      store(three_b + coq:(80), coq:(nth 10 three_b_mont 0));
                      store(three_b + coq:(88), coq:(nth 11 three_b_mont 0));
                      mul (t0, X1, X2);
                      mul (t1, Y1, Y2);
                      mul (t2, Z1, Z2);
                      add (t3, X1, Y1);
...
Instance spec_of_BW6_761_add: spec_of BW6_761_add :=
fun functions : list (string * (list string * list string * cmd)) =>
    ...
    WeakestPrecondition.call functions ( "BW6_761_add") t m0
    ...
    (exists (woutx wouty woutz : list Interface.word.rep) Rout,
        (
                (BW6_761_add_Gallina_spec (toZ wX1) (toZ wY1) (toZ wZ1) (toZ wX2) (toZ wY2)
...
Add Ring Mp : (MontgomeryRingTheory.mont_enc_ring BW6_761Curve_G1_bedrock.m bw n r' m' r'_correct m'_correct bw_big n_nz m_small m_big).
...
Lemma montadd_to_Mp x y z (Hx : valid' m bw n x) (Hy : valid' m bw n y) (Hz : valid' m bw n z) :
  montadd z x y -> (enc_mont BW6_761Curve_G1_bedrock.m bw n z Hz)
    = mont_add (enc_mont BW6_761Curve_G1_bedrock.m bw n x Hx) (enc_mont BW6_761Curve_G1_bedrock.m bw n y Hy).
Proof.
...
montsub z x y -> (enc_mont BW6_761Curve_G1_bedrock.m bw n z Hz)
  = mont_sub (enc_mont BW6_761Curve_G1_bedrock.m bw n x Hx) (enc_mont BW6_761Curve_G1_bedrock.m bw n y Hy).
Proof.
...
montmul z x y -> (enc_mont BW6_761Curve_G1_bedrock.m bw n z Hz)
  = mont_mul (enc_mont BW6_761Curve_G1_bedrock.m bw n x Hx) (enc_mont BW6_761Curve_G1_bedrock.m bw n y Hy).
Proof.
...
assert (H : MontgomeryRingTheory.valid' BW6_761Curve_G1_bedrock.m bw n (toZ x)) by (apply H'; assumption).
...
Theorem BW6_761_add_func_ok : program_logic_goal_for_function! BW6_761_add.
Proof.
  ...
  unfold BW6_761_add_Gallina_spec.
  ...
  destruct (MontgomeryCurveG1Equiv.BLS12_add_specs_equiv' m bw n r' m' (BW6_761Curve_G1_bedrock.a) three_b a_small three_b_small r'_correct m'_correct bw_big n_nz m_small m_big _ _ _ _ _ _ _ _ _ Hvalid Hvalid0 Hvalid1 Hvalid2 Hvalid3 Hvalid4 Hvalid5 Hvalid6 Hvalid7) as [Heq _].
  apply Heq; clear Heq.
  ...
  remember (MontgomeryCurveSpecs.three_b_mont BW6_761Curve_G1_bedrock.m bw n r' m'
