(** * BN256 Fp inversion via Bernstein-Yang divstep algorithm.

    Mirrors [BN254_FpInv.v] for the BN256 base field.

    BN256 base prime:
      p = 36 u^4 + 36 u^3 + 24 u^2 + 6 u + 1,  u = 0x5A76AE9AEC588301
...
(* ================================================================== *)
(* BN256 prime and its parameters                                      *)
(* ================================================================== *)

(** The BN256 base field prime. *)
Definition bn256_p : Z :=
  Eval vm_compute in bn256_modulus.
...
Definition bn256_mbits : Z := Z.log2 bn256_p + 1.

Lemma bn256_mbits_val : bn256_mbits = 256.
Proof. vm_compute. reflexivity. Qed.

(** BN256 has >= 46 bits, so we use the large-prime formula. *)
Lemma bn256_mbits_ge_46 : 46 <= bn256_mbits.
...
(** The number of divstep iterations for BN256 inversion. *)
Definition bn256_divstep_iters : Z :=
  Eval vm_compute in ((49 * bn256_mbits + 57) / 17).

Lemma bn256_divstep_iters_val : bn256_divstep_iters = 741.
Proof. vm_compute. reflexivity. Qed.

(** This equals the formula from divstep_precomp_correct when mbits >= 46. *)
Lemma bn256_divstep_iters_eq_precomp_exp :
  bn256_divstep_iters =
  (if Decidable.dec (bn256_mbits < 46) then (49 * bn256_mbits + 80) / 17
   else (49 * bn256_mbits + 57) / 17).
Proof.
...
Section FpInv.
  ...
  (** Precomputation: ((p+1)/2)^N mod p with N = 741.
      Computed externally via:
        python3 -c "p=0x8fb501e34aa387f9aa6fecb86184dc21ee5b88d120b5b59e185cac6c5e089667; print(hex(pow((p+1)//2, 741, p)))"
  *)
  Definition precomp_val : Z :=
    0x6e25cfd4ba433acee5bd56889dc46859a02b27e66c531821a479d6900538ca6a.

  Definition fp_inv_spec (x : Z) : Z :=
    let '(d, f, g, v, r) :=
      iter_divstep_spec p (Z.to_nat bn256_divstep_iters) 1 p x 0 1 in
  ...
  (** Bernstein-Yang convergence axiom (CHES 2019, Theorem 11.2),
      specialized to BN256. *)
  Axiom by_convergence_dfg_bn256 : forall x,
    0 < x < p ->
    Z.gcd x p = 1 ->
    let '(_, f_N, g_N) :=
      iter_divstep_dfg (Z.to_nat bn256_divstep_iters) 1 p x in
    g_N = 0 /\ (f_N = 1 \/ f_N = -1).
  ...
  Lemma precomp_times_pow2 :
    (precomp_val * 2 ^ bn256_divstep_iters) mod p = 1.
  Proof.
    subst p. unfold precomp_val, bn256_divstep_iters, bn256_p.
    vm_compute. reflexivity.
  Qed.

  (** Any 0 < x < p is coprime to p since p is prime. *)
  Lemma bn256_coprime : forall x, 0 < x < p -> Z.gcd x p = 1.
  Proof.
    intros x Hx.
    apply Zgcd_1_rel_prime.
    apply rel_prime_sym.
    apply prime_rel_prime.
    - subst p. unfold bn256_p. exact prime_bn256.
    - intros [k Hk]. subst x.
  ...
  Proof.
    intros x Hx Hgcd.
    unfold fp_inv_spec.
    destruct (iter_divstep_spec p (Z.to_nat bn256_divstep_iters) 1 p x 0 1)
      as [[[[d_N f_N] g_N] v_N] r_N] eqn:Hiter.
    pose proof (iter_invariant p (Z.to_nat bn256_divstep_iters) 1 p x 0 1 x
                  p_pos p_odd) as Hinv.
    ...
    pose proof (by_convergence_dfg_bn256 x Hx Hgcd) as Hconv.
    pose proof (iter_dfg_agree (Z.to_nat bn256_divstep_iters) p 1 p x 0 1) as Hagree.
    rewrite Hiter in Hagree.
    destruct (iter_divstep_dfg (Z.to_nat bn256_divstep_iters) 1 p x)
    ...
    - replace (1 <? 0) with false by reflexivity.
      ...
      assert (Hmod : (v_N * x) mod p = (2 ^ Z.of_nat (Z.to_nat bn256_divstep_iters)) mod p). {
        assert (Hp : p <> 0) by (pose proof p_pos; lia).
        apply Z.mod_divide in Hv_inv; [|exact Hp].
        destruct Hv_inv as [k Hk].
        assert (v_N * x = 2 ^ Z.of_nat (Z.to_nat bn256_divstep_iters) + k * p) by lia.
        rewrite H.
        rewrite Zplus_mod, Z_mod_mult, Z.add_0_r, Z.mod_mod by lia.
        reflexivity.
      }
      rewrite <- Zmult_mod_idemp_r, Hmod, Zmult_mod_idemp_r.
      rewrite Z2Nat.id by (unfold bn256_divstep_iters; vm_compute; discriminate).
      exact precomp_times_pow2.
    - replace (-1 <? 0) with true by reflexivity.
      replace (2 ^ Z.of_nat (Z.to_nat bn256_divstep_iters) * -1)
        with (- (2 ^ Z.of_nat (Z.to_nat bn256_divstep_iters))) in Hv_inv by ring.
      replace (v_N * x - - 2 ^ Z.of_nat (Z.to_nat bn256_divstep_iters))
        with (v_N * x + 2 ^ Z.of_nat (Z.to_nat bn256_divstep_iters)) in Hv_inv by ring.
      set (iters := Z.of_nat (Z.to_nat bn256_divstep_iters)) in *.
  ...
  Theorem fp_inv_correct : forall x,
    ...
    (fp_inv_spec x * x) mod p = 1.
  Proof.
    intros x Hx. apply fp_inv_correct_ax; auto. apply bn256_coprime; auto.
  Qed.

End FpInv.

(* ================================================================== *)
(* Concrete parameters for BN256 divstep                               *)
(* ================================================================== *)
...
Lemma bn256_p_pos : 1 < bn256_p.
Proof. vm_compute. reflexivity. Qed.

Lemma bn256_p_fits : bn256_p < (2^machine_wordsize) ^ Z.of_nat n.
Proof. vm_compute. reflexivity. Qed.

Lemma bn256_p_odd : Z.odd bn256_p = true.
Proof. vm_compute. reflexivity. Qed.
