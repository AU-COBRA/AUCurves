...
//! with `3b` recomputed here from `bls377_from_word`.  The two are
...
//! baked-in `cB3` constant.  Curve `b = 1`, so `3b = 3`.

use crate::g1_double_a0_extracted::g1_proj_double_limbs;
use crate::tower::{bls377_add, bls377_from_word, bls377_mul, bls377_sub, Fp};

const LIMBS: usize = 6;

fn three_b() -> Fp {
    let mut o = Fp([0u64; LIMBS]);
    bls377_from_word(&mut o, 3u64);
    o
}
...
fn alg9_reference(x: &Fp, y: &Fp, z: &Fp, b3: &Fp) -> [Fp; 3] {
    ...
    let mut t0 = Fp([0u64; LIMBS]); bls377_mul(&mut t0, y, y);     // 1
    let mut z3 = Fp([0u64; LIMBS]); bls377_add(&mut z3, &t0, &t0); // 2
    bls377_add(&mut u, &z3, &z3); z3 = u;                          // 3
    bls377_add(&mut u, &z3, &z3); z3 = u;                          // 4
    let mut t1 = Fp([0u64; LIMBS]); bls377_mul(&mut t1, y, z);     // 5
    let mut t2 = Fp([0u64; LIMBS]); bls377_mul(&mut t2, z, z);     // 6
    bls377_mul(&mut u, b3, &t2); t2 = u;                           // 7
    let mut x3 = Fp([0u64; LIMBS]); bls377_mul(&mut x3, &t2, &z3); // 8
    let mut y3 = Fp([0u64; LIMBS]); bls377_add(&mut y3, &t0, &t2); // 9
    bls377_mul(&mut u, &t1, &z3); z3 = u;                          // 10
    bls377_add(&mut u, &t2, &t2); t1 = u;                          // 11
    bls377_add(&mut u, &t1, &t2); t2 = u;                          // 12
    bls377_sub(&mut u, &t0, &t2); t0 = u;                          // 13
    bls377_mul(&mut u, &t0, &y3); y3 = u;                          // 14
    bls377_add(&mut u, &x3, &y3); y3 = u;                          // 15
    bls377_mul(&mut u, x, y); t1 = u;                              // 16
    bls377_mul(&mut u, &t0, &t1); x3 = u;                          // 17
    bls377_add(&mut u, &x3, &x3); x3 = u;                          // 18
    [x3, y3, z3]
}
...
impl Rng {
    fn next_fp(&mut self) -> Fp {
        ...
        bls377_from_word(&mut a, self.0 | 1);
        self.0 = self.0.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
        let mut b = Fp([0u64; LIMBS]);
        bls377_from_word(&mut b, self.0 | 1);
        let mut c = Fp([0u64; LIMBS]);
        bls377_mul(&mut c, &a, &b);
        let mut d = Fp([0u64; LIMBS]);
        bls377_add(&mut d, &c, &a);
        let mut e = Fp([0u64; LIMBS]);
        bls377_mul(&mut e, &d, &c);
        e
    }
}
...
fn emitted_curve_constant_is_three_b() {
    ...
    let mut sq = Fp([0u64; LIMBS]); bls377_mul(&mut sq, &b3, &b3);   // 9b^2
    let mut t = Fp([0u64; LIMBS]); bls377_add(&mut t, &sq, &sq);      // 18b^2
    let mut s = Fp([0u64; LIMBS]); bls377_add(&mut s, &t, &sq);       // 27b^2
    let mut neg = Fp([0u64; LIMBS]); bls377_sub(&mut neg, &zero, &s); // -27b^2
