(** * Fast processDivstep v9: Sint63 D + FMapAVL State + log2 narrow.

    Key insight: the bottleneck at large N is State_join, not D arithmetic
    or narrow checks. Using FMapAVL (O(log K) operations) instead of sorted
    lists (O(K) operations) eliminates the quadratic scaling.

    Combines:
    - Sint63 machine integers for D arithmetic (from v4)
    - FMapAVL for State (from original O'Connor, O(log K))
    - Sorted list + merge sort for DDSet (from v4)
    - Log2 narrow fast path (from v5)
    - Hull-before-union (from v4) *)
...
(** *** Narrow — log2 fast path + Z fallback *)
...
(** *** State: FMapAVL keyed by Z — O(log K) operations *)

Module ZMap := FMapAVL.Make OrderedTypeEx.Z_as_OT.

Definition State := ZMap.t (list DD).

Definition State_join (k : Z) (v : list DD) (m : State) : State :=
  ZMap.add k (match ZMap.find k m with
              | None => v
              | Some v0 => sl_union v v0
              end) m.

Definition State_fromList (l : list (Z * list DD)) : State :=
  fold_left (fun s kv => State_join (fst kv) (snd kv) s) l (ZMap.empty _).

Definition State_is_empty (s : State) : bool :=
  ZMap.is_empty s.

(** *** processDivstep — FMapAVL State + Sint63 D + hull-before-union *)
...
Definition state0 : State := ZMap.add 1%Z set0 (ZMap.empty _).
