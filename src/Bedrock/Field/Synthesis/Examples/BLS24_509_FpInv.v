(** * BLS24-509 Fp inversion via Bernstein-Yang divstep algorithm.

    Mirrors [BLS12_FpInv.v] / [BN254_FpInv.v] for the BLS24-509 base field.

    BLS24-509 base prime:
      p(z) = ((z-1)^2 * (z^8 - z^4 + 1)) / 3 + z, z = -0x800000ffff801
      Z.log2 p + 1 = 509
      iterations = (49 * 509 + 57) / 17 = 24998 / 17 = 1470

    Word-by-word Montgomery layout: 8 limbs of 64 bits (saturated: 9 limbs).
...
(* ================================================================== *)
(* BLS24-509 prime and its parameters                                  *)
(* ================================================================== *)

(** The BLS24-509 base field prime. *)
Definition bls24_509_p : Z :=
  Eval vm_compute in bls24_509_modulus.

(** Machine parameters for 64-bit limbs. *)
Definition machine_wordsize : Z := 64.

(** Number of 64-bit limbs for Montgomery representation (8 limbs = 512 bits). *)
Definition n : nat := 8.

(** Number of limbs for the saturated representation (n+1 = 9). *)
Definition sat_limbs : nat := 9.

Definition bls24_509_mbits : Z := Z.log2 bls24_509_p + 1.

Lemma bls24_509_mbits_val : bls24_509_mbits = 509.
Proof. vm_compute. reflexivity. Qed.

Lemma bls24_509_mbits_ge_46 : 46 <= bls24_509_mbits.
...
Definition bls24_509_divstep_iters : Z :=
  Eval vm_compute in ((49 * bls24_509_mbits + 57) / 17).

Lemma bls24_509_divstep_iters_val : bls24_509_divstep_iters = 1470.
Proof. vm_compute. reflexivity. Qed.

Lemma bls24_509_divstep_iters_eq_precomp_exp :
  bls24_509_divstep_iters =
  (if Decidable.dec (bls24_509_mbits < 46) then (49 * bls24_509_mbits + 80) / 17
   else (49 * bls24_509_mbits + 57) / 17).
Proof.
...
Section FpInv.
  ...
  (** Precomputation: ((p+1)/2)^N mod p with N = 1470. *)
  Definition precomp_val : Z :=
    0x6b75af91e4c71ef94c6cc8be52e895b56a7ea916f228a806ccfe43c35553a330cf98fb17e8339794385bfff1affa6b65161cb8b6b65a309358e5bec27e6037d.

  Definition fp_inv_spec (x : Z) : Z :=
    let '(d, f, g, v, r) :=
      iter_divstep_spec p (Z.to_nat bls24_509_divstep_iters) 1 p x 0 1 in
    let v_corrected := if (f <? 0) then (p - v) mod p else v mod p in
    (v_corrected * precomp_val) mod p.

  Axiom by_convergence_dfg_bls24_509 : forall x,
    0 < x < p ->
    Z.gcd x p = 1 ->
    let '(_, f_N, g_N) :=
      iter_divstep_dfg (Z.to_nat bls24_509_divstep_iters) 1 p x in
    g_N = 0 /\ (f_N = 1 \/ f_N = -1).
  ...
  Lemma precomp_times_pow2 :
    (precomp_val * 2 ^ bls24_509_divstep_iters) mod p = 1.
  Proof.
    subst p. unfold precomp_val, bls24_509_divstep_iters, bls24_509_p.
    vm_compute. reflexivity.
  Qed.

  Lemma bls24_509_coprime : forall x, 0 < x < p -> Z.gcd x p = 1.
  Proof.
    intros x Hx.
    apply Zgcd_1_rel_prime.
    apply rel_prime_sym.
    apply prime_rel_prime.
    - subst p. unfold bls24_509_p. exact prime_bls24_509.
    - intros [k Hk]. subst x.
  ...
  Proof.
    intros x Hx Hgcd.
    unfold fp_inv_spec.
    destruct (iter_divstep_spec p (Z.to_nat bls24_509_divstep_iters) 1 p x 0 1)
      as [[[[d_N f_N] g_N] v_N] r_N] eqn:Hiter.
    pose proof (iter_invariant p (Z.to_nat bls24_509_divstep_iters) 1 p x 0 1 x
                  p_pos p_odd) as Hinv.
    ...
    pose proof (by_convergence_dfg_bls24_509 x Hx Hgcd) as Hconv.
    pose proof (iter_dfg_agree (Z.to_nat bls24_509_divstep_iters) p 1 p x 0 1) as Hagree.
    rewrite Hiter in Hagree.
    destruct (iter_divstep_dfg (Z.to_nat bls24_509_divstep_iters) 1 p x)
    ...
    - replace (1 <? 0) with false by reflexivity.
      ...
      assert (Hmod : (v_N * x) mod p = (2 ^ Z.of_nat (Z.to_nat bls24_509_divstep_iters)) mod p). {
        assert (Hp : p <> 0) by (pose proof p_pos; lia).
        apply Z.mod_divide in Hv_inv; [|exact Hp].
        destruct Hv_inv as [k Hk].
        assert (v_N * x = 2 ^ Z.of_nat (Z.to_nat bls24_509_divstep_iters) + k * p) by lia.
        rewrite H.
        rewrite Zplus_mod, Z_mod_mult, Z.add_0_r, Z.mod_mod by lia.
        reflexivity.
      }
      rewrite <- Zmult_mod_idemp_r, Hmod, Zmult_mod_idemp_r.
      rewrite Z2Nat.id by (unfold bls24_509_divstep_iters; vm_compute; discriminate).
      exact precomp_times_pow2.
    - replace (-1 <? 0) with true by reflexivity.
      replace (2 ^ Z.of_nat (Z.to_nat bls24_509_divstep_iters) * -1)
        with (- (2 ^ Z.of_nat (Z.to_nat bls24_509_divstep_iters))) in Hv_inv by ring.
      replace (v_N * x - - 2 ^ Z.of_nat (Z.to_nat bls24_509_divstep_iters))
        with (v_N * x + 2 ^ Z.of_nat (Z.to_nat bls24_509_divstep_iters)) in Hv_inv by ring.
      set (iters := Z.of_nat (Z.to_nat bls24_509_divstep_iters)) in *.
  ...
  Theorem fp_inv_correct : forall x,
    ...
    (fp_inv_spec x * x) mod p = 1.
  Proof.
    intros x Hx. apply fp_inv_correct_ax; auto. apply bls24_509_coprime; auto.
  Qed.

End FpInv.

(* ================================================================== *)
(* Concrete parameters for BLS24-509 divstep                           *)
(* ================================================================== *)

Definition bls24_509_r' : Z :=
  Eval vm_compute in
    (let r := 2^machine_wordsize in
     let p := bls24_509_p in
     (fst (Z.ggcd r p))).

Definition bls24_509_m' : Z :=
  Eval vm_compute in
    (let r := 2^machine_wordsize in
     let p := bls24_509_p in
     ((-1 / p) mod r + r) mod r).

Lemma bls24_509_p_pos : 1 < bls24_509_p.
Proof. vm_compute. reflexivity. Qed.

Lemma bls24_509_p_fits : bls24_509_p < (2^machine_wordsize) ^ Z.of_nat n.
Proof. vm_compute. reflexivity. Qed.

Lemma bls24_509_p_odd : Z.odd bls24_509_p = true.
Proof. vm_compute. reflexivity. Qed.
