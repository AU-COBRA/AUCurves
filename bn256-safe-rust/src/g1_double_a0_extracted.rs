//! Rocq-EMITTED complete projective doubling for BN256 G1
...
//!
//!   Compute bn256_g1_double_a0_rs.       (* §4 of *)
...
//!   * `bn256_g1_double_a0_borrow_ok` holds by `vm_compute`.
//!   * The `cB3` constant below is
//!     `mont_bytes 4 bn256_m ((3*b) mod m)`, certified
//!     `bn256_threeb_bytes_len = 32`.
//!
...
use crate::extracted_leaves::{bn256_fp_add, bn256_fp_mul, bn256_fp_sub, FBYTES, LIMBS};
...
pub fn g1_proj_double_bytes(out: &mut [u8; PTBYTES], p: &[u8; PTBYTES]) {
    let mut inb = *p;
    bn256_g1_double_a0_extracted(out, &mut inb);
}
...
pub fn g1_proj_double_limbs(p: &[[u64; LIMBS]; 3]) -> [[u64; LIMBS]; 3] {
    ...
    bn256_g1_double_a0_extracted(&mut outb, &mut inb);
...
pub fn bn256_g1_double_a0_extracted(out: &mut [u8; 96], arg0: &mut [u8; 96]) {
    ...
    cB3.copy_from_slice(&[144u8, 153u8, 118u8, 31u8, 58u8, 57u8, 53u8, 122u8, 30u8, 166u8, 164u8, 244u8, 237u8, 114u8, 71u8, 26u8, 225u8, 61u8, 178u8, 231u8, 121u8, 52u8, 1u8, 89u8, 101u8, 128u8, 199u8, 85u8, 203u8, 225u8, 175u8, 4u8]);
    unsafe { bn256_fp_mul(t0.as_mut_ptr(), Y1.as_ptr(), Y1.as_ptr()) };
    unsafe { bn256_fp_add(za.as_mut_ptr(), t0.as_ptr(), t0.as_ptr()) };
    unsafe { bn256_fp_add(zb.as_mut_ptr(), za.as_ptr(), za.as_ptr()) };
    unsafe { bn256_fp_add(zc.as_mut_ptr(), zb.as_ptr(), zb.as_ptr()) };
    unsafe { bn256_fp_mul(t1.as_mut_ptr(), Y1.as_ptr(), Z1.as_ptr()) };
    unsafe { bn256_fp_mul(t2.as_mut_ptr(), Z1.as_ptr(), Z1.as_ptr()) };
    unsafe { bn256_fp_mul(t2b.as_mut_ptr(), cB3.as_ptr(), t2.as_ptr()) };
    unsafe { bn256_fp_mul(xa.as_mut_ptr(), t2b.as_ptr(), zc.as_ptr()) };
    unsafe { bn256_fp_add(ya.as_mut_ptr(), t0.as_ptr(), t2b.as_ptr()) };
    unsafe { bn256_fp_mul(Z3.as_mut_ptr(), t1.as_ptr(), zc.as_ptr()) };
    unsafe { bn256_fp_add(t1b.as_mut_ptr(), t2b.as_ptr(), t2b.as_ptr()) };
    unsafe { bn256_fp_add(t2c.as_mut_ptr(), t1b.as_ptr(), t2b.as_ptr()) };
    unsafe { bn256_fp_sub(t0b.as_mut_ptr(), t0.as_ptr(), t2c.as_ptr()) };
    unsafe { bn256_fp_mul(yb.as_mut_ptr(), t0b.as_ptr(), ya.as_ptr()) };
    unsafe { bn256_fp_add(Y3.as_mut_ptr(), xa.as_ptr(), yb.as_ptr()) };
    unsafe { bn256_fp_mul(t1c.as_mut_ptr(), X1.as_ptr(), Y1.as_ptr()) };
    unsafe { bn256_fp_mul(xb.as_mut_ptr(), t0b.as_ptr(), t1c.as_ptr()) };
    unsafe { bn256_fp_add(X3.as_mut_ptr(), xb.as_ptr(), xb.as_ptr()) };
    for i in 0u64..32u64 {
