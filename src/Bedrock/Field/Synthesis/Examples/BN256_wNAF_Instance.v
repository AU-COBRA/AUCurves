(** * BN256 single-scalar wNAF — instance discharge + composition.

    Discharges the algebraic hypotheses (Horner step, digit-point
    correctness, weighted-sum non-negativity) for BN256 and composes
    the full WP chain:

      single_load_and_process_ok  →  wnaf_single_loop_body_ok  →  wnaf_single_ok

    The result is a single theorem:
      Given BN256 curve_add/double specs and the digit/table data,
...
Section BN256_wNAF_Instance.
  ...
  Context (Hhorner_step : forall n (Ox Oy Oz : F),
    ...
    (Ox,Oy,Oz) = scmul_s (Z.to_nat (2 * ws_old)) (Px,Py,Pz) ->
    let d := nth n dk 0 in
  ...
  (** ================================================================
      Full single-scalar wNAF theorem for BN256 (or any non-GLV curve).
  ...
  Theorem wnaf_single_full :
    ...
    WeakestPrecondition.cmd functions
      (wnaf_single_func_body curve_add_name curve_double_name "store_zero"
         felem_copy opp (Z.of_nat num_iters) felem_size_in_bytes
         "digits_k" "table_P")
      tr m l
      (fun t m' l' =>
        exists Rx Ry Rz Ax' Ay' Az',
        (Rx,Ry,Rz) = scmul_s (Z.to_nat k) (Px,Py,Pz)
  ...
  Proof.
    ...
    eapply WeakestPreconditionProperties.Proper_cmd;
      [|eapply wnaf_single_ok; try eassumption; try ecancel_assumption].
