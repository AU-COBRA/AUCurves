(** * Closes [P256_FpInv.by_convergence_dfg_p256] axiom via the existing bridge.

    Mirrors [BLS12_FpInv_closed.v] for NIST P-256 (secp256r1).  Composes:

    - The generic [divsteps_bridge.oconnor_bridge] (parametric in the
      prime [M] and iteration count [N]),
    - The P-256-specific cert axiom [divsteps_p256.p256_certificate]
      (741 iterations),
    - A re-proof of the convergence-monotone lemma for P-256
    ...
    - A trivial [iter_divstep_dfg_eq] (the two definitions in
      [divsteps_bridge.v] and [P256_FpInv.v] are syntactically
      identical → defining-equation reflexivity).

    Result: [by_convergence_dfg_p256_closed] has the exact statement
    of [P256_FpInv.by_convergence_dfg_p256], discharging the axiom.

    Net axiom in the chain: just [p256_certificate] (axiomatized in
    [divsteps_p256.v] in parallel to [bls12_certificate]; both can be
...
(** The two [iter_divstep_dfg] definitions (one in [divsteps_bridge.v],
    one in [P256_FpInv.v]) have identical bodies; they coincide. *)
Lemma iter_divstep_dfg_eq : forall n d f g,
  divsteps_bridge.iter_divstep_dfg n d f g =
  P256_FpInv.iter_divstep_dfg n d f g.
Proof.
...
(** *** P-256-specific O'Connor convergence. *)
Theorem oconnor_implies_convergence_p256 : forall x,
  0 < x < P256_FpInv.p256_p ->
  Z.gcd x P256_FpInv.p256_p = 1 ->
  let '(_, f_N, g_N) :=
    divsteps_bridge.iter_divstep_dfg (N.to_nat 741) 1 P256_FpInv.p256_p x in
  g_N = 0 /\ (f_N = 1 \/ f_N = -1).
Proof.
  intros x Hx Hgcd.
  apply (divsteps_bridge.oconnor_bridge 741 divsteps_p256.p256_M).
  - exists (P256_FpInv.p256_p / 2). vm_compute. reflexivity.
  - unfold divsteps_p256.p256_M, divsteps_p256.p256_p, P256_FpInv.p256_p. lia.
  - lia.
  - apply Zgcd_1_rel_prime. rewrite Z.gcd_comm. exact Hgcd.
  - exact divsteps_p256.p256_certificate.
Qed.

(** *** Monotonicity for P-256. *)
Theorem convergence_monotone_p256 : forall N x,
  (N.to_nat 741 <= N)%nat ->
  0 < x < P256_FpInv.p256_p ->
  Z.gcd x P256_FpInv.p256_p = 1 ->
  let '(_, f_N, g_N) := divsteps_bridge.iter_divstep_dfg N 1 P256_FpInv.p256_p x in
  g_N = 0 /\ (f_N = 1 \/ f_N = -1).
Proof.
  intros N x HN Hx Hgcd.
  pose proof (oconnor_implies_convergence_p256 x Hx Hgcd) as H741.
  destruct (divsteps_bridge.iter_divstep_dfg (N.to_nat 741) 1 P256_FpInv.p256_p x)
...
(** Discharges [P256_FpInv.by_convergence_dfg_p256]. *)
Theorem by_convergence_dfg_p256_closed : forall x,
  0 < x < P256_FpInv.p256_p ->
  Z.gcd x P256_FpInv.p256_p = 1 ->
  let '(_, f_N, g_N) :=
    P256_FpInv.iter_divstep_dfg
      (Z.to_nat P256_FpInv.p256_divstep_iters) 1 P256_FpInv.p256_p x in
  g_N = 0 /\ (f_N = 1 \/ f_N = -1).
Proof.
  intros x Hx Hgcd.
  rewrite <- iter_divstep_dfg_eq.
  apply convergence_monotone_p256; auto.
Qed.
...
Local Notation p := P256_FpInv.p256_p.
...
(* ================================================================ *)
(* The headline: fp_inv_correct without the by_convergence_dfg_p256  *)
(* axiom — goes through by_convergence_dfg_p256_closed instead.      *)
(* ================================================================ *)

Theorem fp_inv_correct_closed : forall x,
  0 < x < p ->
  Z.gcd x p = 1 ->
  (P256_FpInv.fp_inv_spec x * x) mod p = 1.
Proof.
  intros x Hx Hgcd.
  unfold P256_FpInv.fp_inv_spec.
  set (N := Z.to_nat P256_FpInv.p256_divstep_iters).
  destruct (P256_FpInv.iter_divstep_spec p N 1 p x 0 1)
    as [[[[d_N f_N] g_N] v_N] r_N] eqn:Espec.
  pose proof (P256_FpInv.iter_invariant p N 1 p x 0 1 x
                P256_FpInv.p_pos P256_FpInv.p_odd) as Hinv.
  ...
  pose proof (P256_FpInv.iter_dfg_agree N p 1 p x 0 1) as Hagree.
  rewrite Espec in Hagree.
  destruct (P256_FpInv.iter_divstep_dfg N 1 p x) as [[d2 f2] g2] eqn:Edfg.
  destruct Hagree as [_ [Hf_eq Hg_eq]]. subst f2 g2.
  pose proof (by_convergence_dfg_p256_closed x Hx Hgcd) as Hconv.
  unfold N in Edfg. rewrite Edfg in Hconv.
  destruct Hconv as [Hg0 Hf_pm1]. subst g_N.
  assert (Hp : p <> 0) by (pose proof P256_FpInv.p_pos; lia).
  Opaque P256_FpInv.p256_p.
  ...
  - subst f_N. rewrite Z.mul_1_r in Hinv_v.
    ...
    transitivity ((2 ^ Z.of_nat N * P256_FpInv.precomp_val) mod p).
    { apply Zmul_mod_compat. exact Hvx. }
    rewrite Z.mul_comm.
    unfold N.
    rewrite Z2Nat.id by (unfold P256_FpInv.p256_divstep_iters; vm_compute; discriminate).
    exact P256_FpInv.precomp_times_pow2.
  - subst f_N.
