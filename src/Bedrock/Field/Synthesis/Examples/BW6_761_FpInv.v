(** * BW6-761 Fp inversion via Bernstein-Yang divstep algorithm.

    Mirrors [BLS12_FpInv.v] / [BN254_FpInv.v] for the BW6-761 base field.
    ...
    BW6-761 base prime:
      ...
      Z.log2 p + 1 = 761
      iterations = (49 * 761 + 57) / 17 = 37346 / 17 = 2196

    Word-by-word Montgomery layout: 12 limbs of 64 bits (saturated: 13 limbs).

    The algorithm and proof structure are identical to BN254_FpInv.v;
    only the prime, iteration count, and precomp value change.

    NOTE: [precomp_times_pow2] uses [vm_compute] over [2^2196] modulo a
    761-bit prime, which is the largest such computation in this repo.
    If this becomes a build bottleneck, see [BN254_FpInv.v]'s
    parallel comment.
*)
...
(* ================================================================== *)
(* BW6-761 prime and its parameters                                    *)
(* ================================================================== *)

Definition bw6_761_p : Z :=
  Eval vm_compute in bw6_761_modulus.

Definition machine_wordsize : Z := 64.

(** Number of 64-bit limbs (12 limbs = 768 bits). *)
Definition n : nat := 12.

(** Saturated representation. *)
Definition sat_limbs : nat := 13.

Definition bw6_761_mbits : Z := Z.log2 bw6_761_p + 1.

Lemma bw6_761_mbits_val : bw6_761_mbits = 761.
Proof. vm_compute. reflexivity. Qed.

Lemma bw6_761_mbits_ge_46 : 46 <= bw6_761_mbits.
...
Definition bw6_761_divstep_iters : Z :=
  Eval vm_compute in ((49 * bw6_761_mbits + 57) / 17).

Lemma bw6_761_divstep_iters_val : bw6_761_divstep_iters = 2196.
Proof. vm_compute. reflexivity. Qed.

Lemma bw6_761_divstep_iters_eq_precomp_exp :
  bw6_761_divstep_iters =
  (if Decidable.dec (bw6_761_mbits < 46) then (49 * bw6_761_mbits + 80) / 17
   else (49 * bw6_761_mbits + 57) / 17).
Proof.
...
Section FpInv.
  ...
  (** Precomputation: ((p+1)/2)^N mod p with N = 2196. *)
  Definition precomp_val : Z :=
    0xd208d85330f85033b6f5fb838523fd94b83536cb6672273e6901243d7a0c63abd654240ccd2eef0ed96e6f5a5e1fa4b14f63dc63eb070d5f3695e43b3f0c18841a6f8db46cebc937b71a06c6bb1690708730378fd553f62a95ff128791135b.

  Definition fp_inv_spec (x : Z) : Z :=
    let '(d, f, g, v, r) :=
      iter_divstep_spec p (Z.to_nat bw6_761_divstep_iters) 1 p x 0 1 in
    let v_corrected := if (f <? 0) then (p - v) mod p else v mod p in
    (v_corrected * precomp_val) mod p.

  Axiom by_convergence_dfg_bw6_761 : forall x,
    0 < x < p ->
    Z.gcd x p = 1 ->
    let '(_, f_N, g_N) :=
      iter_divstep_dfg (Z.to_nat bw6_761_divstep_iters) 1 p x in
    g_N = 0 /\ (f_N = 1 \/ f_N = -1).
  ...
  (** NOTE: [precomp_times_pow2] for BW6-761 computes [2^2196 mod p] via
      vm_compute on a 761-bit modulus.  See file header for details.
      If this fails to terminate within memory, it can be Admitted with
      an externally-verified sketch (the precomp value was computed in
      Python and is correct). *)
  Lemma precomp_times_pow2 :
    (precomp_val * 2 ^ bw6_761_divstep_iters) mod p = 1.
  Proof.
    subst p. unfold precomp_val, bw6_761_divstep_iters, bw6_761_p.
    vm_compute. reflexivity.
  Qed.

  Lemma bw6_761_coprime : forall x, 0 < x < p -> Z.gcd x p = 1.
  Proof.
    intros x Hx.
    apply Zgcd_1_rel_prime.
    apply rel_prime_sym.
    apply prime_rel_prime.
    - subst p. unfold bw6_761_p. exact prime_bw6_761.
    - intros [k Hk]. subst x.
  ...
  Proof.
    intros x Hx Hgcd.
    unfold fp_inv_spec.
    destruct (iter_divstep_spec p (Z.to_nat bw6_761_divstep_iters) 1 p x 0 1)
      as [[[[d_N f_N] g_N] v_N] r_N] eqn:Hiter.
    pose proof (iter_invariant p (Z.to_nat bw6_761_divstep_iters) 1 p x 0 1 x
                  p_pos p_odd) as Hinv.
    ...
    pose proof (by_convergence_dfg_bw6_761 x Hx Hgcd) as Hconv.
    pose proof (iter_dfg_agree (Z.to_nat bw6_761_divstep_iters) p 1 p x 0 1) as Hagree.
    rewrite Hiter in Hagree.
    destruct (iter_divstep_dfg (Z.to_nat bw6_761_divstep_iters) 1 p x)
    ...
    - replace (1 <? 0) with false by reflexivity.
      ...
      assert (Hmod : (v_N * x) mod p = (2 ^ Z.of_nat (Z.to_nat bw6_761_divstep_iters)) mod p). {
        assert (Hp : p <> 0) by (pose proof p_pos; lia).
        apply Z.mod_divide in Hv_inv; [|exact Hp].
        destruct Hv_inv as [k Hk].
        assert (v_N * x = 2 ^ Z.of_nat (Z.to_nat bw6_761_divstep_iters) + k * p) by lia.
        rewrite H.
        rewrite Zplus_mod, Z_mod_mult, Z.add_0_r, Z.mod_mod by lia.
        reflexivity.
      }
      rewrite <- Zmult_mod_idemp_r, Hmod, Zmult_mod_idemp_r.
      rewrite Z2Nat.id by (unfold bw6_761_divstep_iters; vm_compute; discriminate).
      exact precomp_times_pow2.
    - replace (-1 <? 0) with true by reflexivity.
      replace (2 ^ Z.of_nat (Z.to_nat bw6_761_divstep_iters) * -1)
        with (- (2 ^ Z.of_nat (Z.to_nat bw6_761_divstep_iters))) in Hv_inv by ring.
      replace (v_N * x - - 2 ^ Z.of_nat (Z.to_nat bw6_761_divstep_iters))
        with (v_N * x + 2 ^ Z.of_nat (Z.to_nat bw6_761_divstep_iters)) in Hv_inv by ring.
      set (iters := Z.of_nat (Z.to_nat bw6_761_divstep_iters)) in *.
  ...
  Theorem fp_inv_correct : forall x,
    ...
    (fp_inv_spec x * x) mod p = 1.
  Proof.
    intros x Hx. apply fp_inv_correct_ax; auto. apply bw6_761_coprime; auto.
  Qed.

End FpInv.

(* ================================================================== *)
(* Concrete parameters for BW6-761 divstep                             *)
(* ================================================================== *)
...
Lemma bw6_761_p_pos : 1 < bw6_761_p.
Proof. vm_compute. reflexivity. Qed.

Lemma bw6_761_p_fits : bw6_761_p < (2^machine_wordsize) ^ Z.of_nat n.
Proof. vm_compute. reflexivity. Qed.

Lemma bw6_761_p_odd : Z.odd bw6_761_p = true.
Proof. vm_compute. reflexivity. Qed.
