(** * Pallas (pasta_fp) Fp inversion via Bernstein-Yang divstep algorithm.

    Mirrors [BN254_FpInv.v] for the Pallas base field.
...
(* ================================================================== *)
(* Pallas prime and its parameters                                     *)
(* ================================================================== *)

(** The Pallas base field prime. *)
Definition pallas_p : Z :=
  Eval vm_compute in pallas_modulus.
...
Definition pallas_mbits : Z := Z.log2 pallas_p + 1.

Lemma pallas_mbits_val : pallas_mbits = 255.
Proof. vm_compute. reflexivity. Qed.

(** Pallas has >= 46 bits, so we use the large-prime formula. *)
Lemma pallas_mbits_ge_46 : 46 <= pallas_mbits.
...
(** The number of divstep iterations for Pallas inversion. *)
Definition pallas_divstep_iters : Z :=
  Eval vm_compute in ((49 * pallas_mbits + 57) / 17).

Lemma pallas_divstep_iters_val : pallas_divstep_iters = 738.
Proof. vm_compute. reflexivity. Qed.

(** This equals the formula from divstep_precomp_correct when mbits >= 46. *)
Lemma pallas_divstep_iters_eq_precomp_exp :
  pallas_divstep_iters =
  (if Decidable.dec (pallas_mbits < 46) then (49 * pallas_mbits + 80) / 17
   else (49 * pallas_mbits + 57) / 17).
Proof.
...
Section FpInv.
  ...
  (** Precomputation: ((p+1)/2)^N mod p with N = 738.
      Computed externally via:
        python3 -c "p=0x40000000000000000000000000000000224698fc094cf91b992d30ed00000001; print(hex(pow((p+1)//2, 738, p)))"
  *)
  Definition precomp_val : Z :=
    0x0e6c2b0d45d5910062f969b72be7e8e324c842089bbddcdbd26f157f4a2cfc20.

  Definition fp_inv_spec (x : Z) : Z :=
    let '(d, f, g, v, r) :=
      iter_divstep_spec p (Z.to_nat pallas_divstep_iters) 1 p x 0 1 in
  ...
  (** Bernstein-Yang convergence axiom (CHES 2019, Theorem 11.2),
      specialized to Pallas. *)
  Axiom by_convergence_dfg_pallas : forall x,
    0 < x < p ->
    Z.gcd x p = 1 ->
    let '(_, f_N, g_N) :=
      iter_divstep_dfg (Z.to_nat pallas_divstep_iters) 1 p x in
    g_N = 0 /\ (f_N = 1 \/ f_N = -1).
  ...
  Lemma precomp_times_pow2 :
    (precomp_val * 2 ^ pallas_divstep_iters) mod p = 1.
  Proof.
    subst p. unfold precomp_val, pallas_divstep_iters, pallas_p.
    vm_compute. reflexivity.
  Qed.

  (** Any 0 < x < p is coprime to p since p is prime. *)
  Lemma pallas_coprime : forall x, 0 < x < p -> Z.gcd x p = 1.
  Proof.
    intros x Hx.
    apply Zgcd_1_rel_prime.
    apply rel_prime_sym.
    apply prime_rel_prime.
    - subst p. unfold pallas_p. exact prime_pallas.
    - intros [k Hk]. subst x.
  ...
  Proof.
    intros x Hx Hgcd.
    unfold fp_inv_spec.
    destruct (iter_divstep_spec p (Z.to_nat pallas_divstep_iters) 1 p x 0 1)
      as [[[[d_N f_N] g_N] v_N] r_N] eqn:Hiter.
    pose proof (iter_invariant p (Z.to_nat pallas_divstep_iters) 1 p x 0 1 x
                  p_pos p_odd) as Hinv.
    ...
    pose proof (by_convergence_dfg_pallas x Hx Hgcd) as Hconv.
    pose proof (iter_dfg_agree (Z.to_nat pallas_divstep_iters) p 1 p x 0 1) as Hagree.
    rewrite Hiter in Hagree.
    destruct (iter_divstep_dfg (Z.to_nat pallas_divstep_iters) 1 p x)
    ...
    - replace (1 <? 0) with false by reflexivity.
      ...
      assert (Hmod : (v_N * x) mod p = (2 ^ Z.of_nat (Z.to_nat pallas_divstep_iters)) mod p). {
        assert (Hp : p <> 0) by (pose proof p_pos; lia).
        apply Z.mod_divide in Hv_inv; [|exact Hp].
        destruct Hv_inv as [k Hk].
        assert (v_N * x = 2 ^ Z.of_nat (Z.to_nat pallas_divstep_iters) + k * p) by lia.
        rewrite H.
        rewrite Zplus_mod, Z_mod_mult, Z.add_0_r, Z.mod_mod by lia.
        reflexivity.
      }
      rewrite <- Zmult_mod_idemp_r, Hmod, Zmult_mod_idemp_r.
      rewrite Z2Nat.id by (unfold pallas_divstep_iters; vm_compute; discriminate).
      exact precomp_times_pow2.
    - replace (-1 <? 0) with true by reflexivity.
      replace (2 ^ Z.of_nat (Z.to_nat pallas_divstep_iters) * -1)
        with (- (2 ^ Z.of_nat (Z.to_nat pallas_divstep_iters))) in Hv_inv by ring.
      replace (v_N * x - - 2 ^ Z.of_nat (Z.to_nat pallas_divstep_iters))
        with (v_N * x + 2 ^ Z.of_nat (Z.to_nat pallas_divstep_iters)) in Hv_inv by ring.
      set (iters := Z.of_nat (Z.to_nat pallas_divstep_iters)) in *.
  ...
  Theorem fp_inv_correct : forall x,
    ...
    (fp_inv_spec x * x) mod p = 1.
  Proof.
    intros x Hx. apply fp_inv_correct_ax; auto. apply pallas_coprime; auto.
  Qed.

End FpInv.

(* ================================================================== *)
(* Concrete parameters for Pallas divstep                              *)
(* ================================================================== *)

(** Montgomery reduction parameter r' such that 2^64 * r' = 1 mod p. *)
Definition pallas_r' : Z :=
  Eval vm_compute in
...
Definition pallas_m' : Z :=
  Eval vm_compute in
    (let r := 2^machine_wordsize in
     let p := pallas_p in
     ((-1 / p) mod r + r) mod r).

Lemma pallas_p_pos : 1 < pallas_p.
Proof. vm_compute. reflexivity. Qed.

Lemma pallas_p_fits : pallas_p < (2^machine_wordsize) ^ Z.of_nat n.
Proof. vm_compute. reflexivity. Qed.

Lemma pallas_p_odd : Z.odd pallas_p = true.
Proof. vm_compute. reflexivity. Qed.
