(** * Closes [BLS12_377_FpInv.by_convergence_dfg_bls12_377] axiom via the existing bridge.

    Mirrors [BN254_FpInv_closed.v] for BLS12-377. *)

From Stdlib Require Import ZArith Lia Znumtheory.
Require Import divsteps_def.
Require Import divsteps_bridge.
Require Import divsteps_bls12_377.
...
Lemma iter_divstep_dfg_eq : forall n d f g,
  ...
  BLS12_377_FpInv.iter_divstep_dfg n d f g.
Proof.
...
Theorem oconnor_implies_convergence_bls12_377 : forall x,
  0 < x < BLS12_377_FpInv.bls12_377_p ->
  Z.gcd x BLS12_377_FpInv.bls12_377_p = 1 ->
  let '(_, f_N, g_N) :=
    divsteps_bridge.iter_divstep_dfg (N.to_nat 1090) 1 BLS12_377_FpInv.bls12_377_p x in
  g_N = 0 /\ (f_N = 1 \/ f_N = -1).
Proof.
  intros x Hx Hgcd.
  apply (divsteps_bridge.oconnor_bridge 1090 divsteps_bls12_377.bls12_377_M).
  - exists (BLS12_377_FpInv.bls12_377_p / 2). vm_compute. reflexivity.
  - unfold divsteps_bls12_377.bls12_377_M, divsteps_bls12_377.bls12_377_p, BLS12_377_FpInv.bls12_377_p. lia.
  - lia.
  - apply Zgcd_1_rel_prime. rewrite Z.gcd_comm. exact Hgcd.
  - exact divsteps_bls12_377.bls12_377_certificate.
Qed.

Theorem convergence_monotone_bls12_377 : forall N x,
  (N.to_nat 1090 <= N)%nat ->
  0 < x < BLS12_377_FpInv.bls12_377_p ->
  Z.gcd x BLS12_377_FpInv.bls12_377_p = 1 ->
  let '(_, f_N, g_N) := divsteps_bridge.iter_divstep_dfg N 1 BLS12_377_FpInv.bls12_377_p x in
  g_N = 0 /\ (f_N = 1 \/ f_N = -1).
Proof.
  intros N x HN Hx Hgcd.
  pose proof (oconnor_implies_convergence_bls12_377 x Hx Hgcd) as H0.
  destruct (divsteps_bridge.iter_divstep_dfg (N.to_nat 1090) 1 BLS12_377_FpInv.bls12_377_p x)
    as [[d0 f0] g0] eqn:E0.
  destruct H0 as [Hg0 Hf_pm1].
  assert (exists extra, N = (N.to_nat 1090 + extra)%nat) as [extra Hextra].
  { exists (N - N.to_nat 1090)%nat. lia. }
...
Theorem by_convergence_dfg_bls12_377_closed : forall x,
  0 < x < BLS12_377_FpInv.bls12_377_p ->
  Z.gcd x BLS12_377_FpInv.bls12_377_p = 1 ->
  let '(_, f_N, g_N) :=
    BLS12_377_FpInv.iter_divstep_dfg
      (Z.to_nat BLS12_377_FpInv.bls12_377_divstep_iters) 1 BLS12_377_FpInv.bls12_377_p x in
  g_N = 0 /\ (f_N = 1 \/ f_N = -1).
Proof.
  intros x Hx Hgcd.
  rewrite <- iter_divstep_dfg_eq.
  apply convergence_monotone_bls12_377; auto.
Qed.

Local Notation p := BLS12_377_FpInv.bls12_377_p.
...
Theorem fp_inv_correct_closed : forall x,
  0 < x < p ->
  Z.gcd x p = 1 ->
  (BLS12_377_FpInv.fp_inv_spec x * x) mod p = 1.
Proof.
  intros x Hx Hgcd.
  unfold BLS12_377_FpInv.fp_inv_spec.
  set (N := Z.to_nat BLS12_377_FpInv.bls12_377_divstep_iters).
  destruct (BLS12_377_FpInv.iter_divstep_spec p N 1 p x 0 1)
    as [[[[d_N f_N] g_N] v_N] r_N] eqn:Espec.
  pose proof (BLS12_377_FpInv.iter_invariant p N 1 p x 0 1 x
                BLS12_377_FpInv.p_pos BLS12_377_FpInv.p_odd) as Hinv.
  ...
  pose proof (BLS12_377_FpInv.iter_dfg_agree N p 1 p x 0 1) as Hagree.
  rewrite Espec in Hagree.
  destruct (BLS12_377_FpInv.iter_divstep_dfg N 1 p x) as [[d2 f2] g2] eqn:Edfg.
  destruct Hagree as [_ [Hf_eq Hg_eq]]. subst f2 g2.
  pose proof (by_convergence_dfg_bls12_377_closed x Hx Hgcd) as Hconv.
  unfold N in Edfg. rewrite Edfg in Hconv.
  destruct Hconv as [Hg0 Hf_pm1]. subst g_N.
  assert (Hp : p <> 0) by (pose proof BLS12_377_FpInv.p_pos; lia).
  Opaque BLS12_377_FpInv.bls12_377_p.
  ...
  - subst f_N. rewrite Z.mul_1_r in Hinv_v.
    ...
    transitivity ((2 ^ Z.of_nat N * BLS12_377_FpInv.precomp_val) mod p).
    { apply Zmul_mod_compat. exact Hvx. }
    rewrite Z.mul_comm.
    unfold N.
    rewrite Z2Nat.id by (unfold BLS12_377_FpInv.bls12_377_divstep_iters; vm_compute; discriminate).
    exact BLS12_377_FpInv.precomp_times_pow2.
  - subst f_N.
