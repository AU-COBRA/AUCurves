...
//! `unsafe { p256_fp_mul(dest.as_mut_ptr(), a.as_ptr(), b.as_ptr()) }`
...
//! fiat-rust field functions.  Little-endian host assumed (as for the
...
unsafe fn load_fp(p: *const u8) -> Fp {
    ...
    core::ptr::copy_nonoverlapping(p, limbs.as_mut_ptr() as *mut u8, 32);
    Fp(limbs)
}

#[inline]
unsafe fn store_fp(p: *mut u8, v: &Fp) {
    core::ptr::copy_nonoverlapping(v.0.as_ptr() as *const u8, p, 32);
}

/// # Safety
/// `out` must point to 32 writable bytes; `a`, `b` to 32 readable bytes.
pub unsafe fn p256_fp_mul(out: *mut u8, a: *const u8, b: *const u8) {
...
/// # Safety
/// `out` must point to 32 writable bytes; `a`, `b` to 32 readable bytes.
pub unsafe fn p256_fp_add(out: *mut u8, a: *const u8, b: *const u8) {
    let (a, b) = (load_fp(a), load_fp(b));
    let mut o = Fp([0u64; 4]);
    fp_add(&mut o, &a, &b);
    store_fp(out, &o);
...
/// # Safety
/// `out` must point to 32 writable bytes; `a`, `b` to 32 readable bytes.
pub unsafe fn p256_fp_sub(out: *mut u8, a: *const u8, b: *const u8) {
    let (a, b) = (load_fp(a), load_fp(b));
    let mut o = Fp([0u64; 4]);
    fp_sub(&mut o, &a, &b);
    store_fp(out, &o);
...
pub unsafe fn p256_fp_opp(out: *mut u8, a: *const u8) {
    let a = load_fp(a);
    let mut o = Fp([0u64; 4]);
    fp_opp(&mut o, &a);
    store_fp(out, &o);
