(* Vesta curve integration into AUCurves Montgomery framework.
   Instantiates MontgomeryCurveSpecs for y^2 = x^3 + 5 over the
   Vesta prime field (m = 0x40000000000000000000000000000000224698fc0994a8dd8c46eb2100000001). *)
...
(* Vesta prime *)
Local Definition m := Eval vm_compute in (0x40000000000000000000000000000000224698fc0994a8dd8c46eb2100000001).
...
(* Instantiate Montgomery curve specs for Vesta *)
Definition vesta_three_b_list := MontgomeryCurveSpecs.three_b_list bw n three_b.
Definition vesta_three_b_mont := Eval vm_compute in
  (MontgomeryCurveSpecs.three_b_mont_list m bw n m' three_b).
Definition vesta_a_list := MontgomeryCurveSpecs.a_list bw n a.
Definition vesta_a_mont_list := Eval vm_compute in
  (MontgomeryCurveSpecs.a_mont_list m bw n m' a).

(* Validity lemmas *)
Lemma vesta_three_b_list_valid : WordByWordMontgomery.valid bw n m vesta_three_b_list.
Proof.
...
(* Gallina specification of Vesta point addition (projective coordinates).
   This is the generic spec from MontgomeryCurveSpecs, instantiated for Vesta. *)
Definition Vesta_add_Gallina_spec :=
...
Definition Vesta_add_Gallina_spec_a_0 X1 Y1 Z1 X2 Y2 Z2 outx outy outz :=
  let X1 := evfrom X1 in
  let Y1 := evfrom Y1 in
  let Z1 := evfrom Z1 in
  let X2 := evfrom X2 in
  ...
  let X3 := t1+'t2 in
  let t5 := t5-'X3 in
  let Z3 := eval vesta_three_b_list *'t2 in
  let X3 := t1-'Z3 in
  let Z3 := t1+'Z3 in
  let Y3 := X3*'Z3 in
  let t1 := t0+'t0 in
  let t1 := t1+'t0 in
  let t4 := eval vesta_three_b_list *'t4 in
  let t0 := t1*'t4 in
  let Y3 := Y3+'t0 in
  let t0 := t5*'t4 in
  let X3 := t3*'X3 in
...
(* The generic and a=0 specialized specs are equivalent for Vesta. *)
Lemma add_specs_equiv : forall X1 X2 Y1 Y2 Z1 Z2 outx outy outz,
  Vesta_add_Gallina_spec X1 X2 Y1 Y2 Z1 Z2 outx outy outz
  <-> Vesta_add_Gallina_spec_a_0 X1 X2 Y1 Y2 Z1 Z2 outx outy outz.
Proof.
  intros X1 X2 Y1 Y2 Z1 Z2 outx outy outz.
  unfold Vesta_add_Gallina_spec, MontgomeryCurveSpecs.BLS12_add_Gallina_spec.
  unfold Vesta_add_Gallina_spec_a_0.
