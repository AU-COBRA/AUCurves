(** * P-224 (secp224r1) Fp inversion via Bernstein-Yang divstep algorithm.

    Mirrors [P256_FpInv.v] for the NIST P-224 base field.

    P-224 base prime:
      p = 2^224 - 2^96 + 1
      Z.log2 p + 1 = 224
      iterations = (49 * 224 + 57) / 17 = 11033 / 17 = 649
...
(* ================================================================== *)
(* P-224 prime and its parameters                                      *)
(* ================================================================== *)

(** The P-224 base field prime. *)
Definition p224_p : Z :=
  Eval vm_compute in p224_modulus.

(** Machine parameters for 64-bit limbs. *)
Definition machine_wordsize : Z := 64.

(** Number of 64-bit limbs for Montgomery representation
    (4 limbs = 256 bits; P-224 fits in 224 < 256 bits). *)
Definition n : nat := 4.
...
Definition p224_mbits : Z := Z.log2 p224_p + 1.

Lemma p224_mbits_val : p224_mbits = 224.
Proof. vm_compute. reflexivity. Qed.

Lemma p224_mbits_ge_46 : 46 <= p224_mbits.
...
Definition p224_divstep_iters : Z :=
  Eval vm_compute in ((49 * p224_mbits + 57) / 17).

Lemma p224_divstep_iters_val : p224_divstep_iters = 649.
Proof. vm_compute. reflexivity. Qed.

Lemma p224_divstep_iters_eq_precomp_exp :
  p224_divstep_iters =
  (if Decidable.dec (p224_mbits < 46) then (49 * p224_mbits + 80) / 17
   else (49 * p224_mbits + 57) / 17).
Proof.
...
Section FpInv.
  ...
  (** Precomputation: ((p+1)/2)^N mod p with N = 649.
      Computed externally via:
        python3 -c "p=2**224-2**96+1; print(hex(pow((p+1)//2, 649, p)))"
  *)
  Definition precomp_val : Z :=
    0xfd000000027fffffff7ffffffe7fffff04fffffffd00000000000001.

  Definition fp_inv_spec (x : Z) : Z :=
    let '(d, f, g, v, r) :=
      iter_divstep_spec p (Z.to_nat p224_divstep_iters) 1 p x 0 1 in
    let v_corrected := if (f <? 0) then (p - v) mod p else v mod p in
    (v_corrected * precomp_val) mod p.

  Axiom by_convergence_dfg_p224 : forall x,
    0 < x < p ->
    Z.gcd x p = 1 ->
    let '(_, f_N, g_N) :=
      iter_divstep_dfg (Z.to_nat p224_divstep_iters) 1 p x in
    g_N = 0 /\ (f_N = 1 \/ f_N = -1).
  ...
  Lemma precomp_times_pow2 :
    (precomp_val * 2 ^ p224_divstep_iters) mod p = 1.
  Proof.
    subst p. unfold precomp_val, p224_divstep_iters, p224_p.
    vm_compute. reflexivity.
  Qed.

  Lemma p224_coprime : forall x, 0 < x < p -> Z.gcd x p = 1.
  Proof.
    intros x Hx.
    apply Zgcd_1_rel_prime.
    apply rel_prime_sym.
    apply prime_rel_prime.
    - subst p. unfold p224_p. exact prime_p224.
    - intros [k Hk]. subst x.
  ...
  Proof.
    intros x Hx Hgcd.
    unfold fp_inv_spec.
    destruct (iter_divstep_spec p (Z.to_nat p224_divstep_iters) 1 p x 0 1)
      as [[[[d_N f_N] g_N] v_N] r_N] eqn:Hiter.
    pose proof (iter_invariant p (Z.to_nat p224_divstep_iters) 1 p x 0 1 x
                  p_pos p_odd) as Hinv.
    ...
    pose proof (by_convergence_dfg_p224 x Hx Hgcd) as Hconv.
    pose proof (iter_dfg_agree (Z.to_nat p224_divstep_iters) p 1 p x 0 1) as Hagree.
    rewrite Hiter in Hagree.
    destruct (iter_divstep_dfg (Z.to_nat p224_divstep_iters) 1 p x)
    ...
    - replace (1 <? 0) with false by reflexivity.
      ...
      assert (Hmod : (v_N * x) mod p = (2 ^ Z.of_nat (Z.to_nat p224_divstep_iters)) mod p). {
        assert (Hp : p <> 0) by (pose proof p_pos; lia).
        apply Z.mod_divide in Hv_inv; [|exact Hp].
        destruct Hv_inv as [k Hk].
        assert (v_N * x = 2 ^ Z.of_nat (Z.to_nat p224_divstep_iters) + k * p) by lia.
        rewrite H.
        rewrite Zplus_mod, Z_mod_mult, Z.add_0_r, Z.mod_mod by lia.
        reflexivity.
      }
      rewrite <- Zmult_mod_idemp_r, Hmod, Zmult_mod_idemp_r.
      rewrite Z2Nat.id by (unfold p224_divstep_iters; vm_compute; discriminate).
      exact precomp_times_pow2.
    - replace (-1 <? 0) with true by reflexivity.
      replace (2 ^ Z.of_nat (Z.to_nat p224_divstep_iters) * -1)
        with (- (2 ^ Z.of_nat (Z.to_nat p224_divstep_iters))) in Hv_inv by ring.
      replace (v_N * x - - 2 ^ Z.of_nat (Z.to_nat p224_divstep_iters))
        with (v_N * x + 2 ^ Z.of_nat (Z.to_nat p224_divstep_iters)) in Hv_inv by ring.
      set (iters := Z.of_nat (Z.to_nat p224_divstep_iters)) in *.
  ...
  Theorem fp_inv_correct : forall x,
    ...
    (fp_inv_spec x * x) mod p = 1.
  Proof.
    intros x Hx. apply fp_inv_correct_ax; auto. apply p224_coprime; auto.
  Qed.

End FpInv.

(* ================================================================== *)
(* Concrete parameters for P-224 divstep                               *)
(* ================================================================== *)
...
Lemma p224_p_pos : 1 < p224_p.
Proof. vm_compute. reflexivity. Qed.

Lemma p224_p_fits : p224_p < (2^machine_wordsize) ^ Z.of_nat n.
Proof. vm_compute. reflexivity. Qed.

Lemma p224_p_odd : Z.odd p224_p = true.
Proof. vm_compute. reflexivity. Qed.
