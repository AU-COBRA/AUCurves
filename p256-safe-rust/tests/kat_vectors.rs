...
fn base_mul_matches_hardcoded_multiples() {
    for (k, x, y) in kat_table() {
        let pt = g1_scalar_mul_base(&scalar_be(&small_scalar(k)));
        assert_eq!(
            crate_affine(&pt),
            Some((x, y)),
...
fn base_mul_large_scalar_kat() {
    ...
    let want = Some((hex_to_big(KBIG_X), hex_to_big(KBIG_Y)));
    assert_eq!(
        crate_affine(&g1_scalar_mul_base(&scalar_be(&k))),
        want,
        "fixed-base large-scalar result disagrees with the KAT"
    );
    // n * G = O on the fixed-base path as well
    let n = hex_to_big(N_HEX);
    assert!(
        g1_is_identity(&g1_scalar_mul_base(&scalar_be(&n))),
        "fixed-base n * G != O"
    );
}
...
fn base_mul_agrees_with_reference_on_random_scalars() {
    ...
    for _ in 0..4 {
        ...
        let got = crate_affine(&g1_scalar_mul_base(&scalar_be(&k)));
        assert_eq!(
            got,
            want,
