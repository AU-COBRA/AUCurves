(** * Closes [P384_FpInv.by_convergence_dfg_p384] axiom via the existing bridge.

    Mirrors [P256_FpInv_closed.v] for NIST P-384 (secp384r1).  Composes:

    - The generic [divsteps_bridge.oconnor_bridge] (parametric in the
      prime [M] and iteration count [N]),
    - The P-384-specific cert axiom [divsteps_p384.p384_certificate]
      (1110 iterations),
    - A re-proof of the convergence-monotone lemma for P-384, with
    - A trivial [iter_divstep_dfg_eq].

    Result: [by_convergence_dfg_p384_closed] has the exact statement
    of [P384_FpInv.by_convergence_dfg_p384], discharging the axiom.

    Net axiom in the chain: just [p384_certificate].
*)

From Stdlib Require Import ZArith Lia Znumtheory.
Require Import divsteps_def.
Require Import divsteps_bridge.
Require Import divsteps_p384.
...
Lemma iter_divstep_dfg_eq : forall n d f g,
  ...
  P384_FpInv.iter_divstep_dfg n d f g.
Proof.
...
Theorem oconnor_implies_convergence_p384 : forall x,
  0 < x < P384_FpInv.p384_p ->
  Z.gcd x P384_FpInv.p384_p = 1 ->
  let '(_, f_N, g_N) :=
    divsteps_bridge.iter_divstep_dfg (N.to_nat 1110) 1 P384_FpInv.p384_p x in
  g_N = 0 /\ (f_N = 1 \/ f_N = -1).
Proof.
  intros x Hx Hgcd.
  apply (divsteps_bridge.oconnor_bridge 1110 divsteps_p384.p384_M).
  - exists (P384_FpInv.p384_p / 2). vm_compute. reflexivity.
  - unfold divsteps_p384.p384_M, divsteps_p384.p384_p, P384_FpInv.p384_p. lia.
  - lia.
  - apply Zgcd_1_rel_prime. rewrite Z.gcd_comm. exact Hgcd.
  - exact divsteps_p384.p384_certificate.
Qed.

Theorem convergence_monotone_p384 : forall N x,
  (N.to_nat 1110 <= N)%nat ->
  0 < x < P384_FpInv.p384_p ->
  Z.gcd x P384_FpInv.p384_p = 1 ->
  let '(_, f_N, g_N) := divsteps_bridge.iter_divstep_dfg N 1 P384_FpInv.p384_p x in
  g_N = 0 /\ (f_N = 1 \/ f_N = -1).
Proof.
  intros N x HN Hx Hgcd.
  pose proof (oconnor_implies_convergence_p384 x Hx Hgcd) as H1110.
  destruct (divsteps_bridge.iter_divstep_dfg (N.to_nat 1110) 1 P384_FpInv.p384_p x)
    as [[d1110 f1110] g1110] eqn:E1110.
  destruct H1110 as [Hg0 Hf_pm1].
  assert (exists extra, N = (N.to_nat 1110 + extra)%nat) as [extra Hextra].
  { exists (N - N.to_nat 1110)%nat. lia. }
  subst N. rewrite divsteps_bridge.iter_divstep_dfg_decompose, E1110, Hg0.
  assert (Hf_odd : Z.Odd f1110).
  { destruct Hf_pm1 as [-> | ->]; [exists 0; lia | exists (-1); lia]. }
  pose proof (divsteps_bridge.iter_dfg_stable_at_zero extra d1110 f1110 Hf_odd) as Hstab.
  destruct (divsteps_bridge.iter_divstep_dfg extra d1110 f1110 0) as [[d_N f_N] g_N].
  destruct Hstab as [Hg Hf]. rewrite Hf. auto.
Qed.

Theorem by_convergence_dfg_p384_closed : forall x,
  0 < x < P384_FpInv.p384_p ->
  Z.gcd x P384_FpInv.p384_p = 1 ->
  let '(_, f_N, g_N) :=
    P384_FpInv.iter_divstep_dfg
      (Z.to_nat P384_FpInv.p384_divstep_iters) 1 P384_FpInv.p384_p x in
  g_N = 0 /\ (f_N = 1 \/ f_N = -1).
Proof.
  intros x Hx Hgcd.
  rewrite <- iter_divstep_dfg_eq.
  apply convergence_monotone_p384; auto.
Qed.
...
Local Notation p := P384_FpInv.p384_p.
...
Theorem fp_inv_correct_closed : forall x,
  0 < x < p ->
  Z.gcd x p = 1 ->
  (P384_FpInv.fp_inv_spec x * x) mod p = 1.
Proof.
  intros x Hx Hgcd.
  unfold P384_FpInv.fp_inv_spec.
  set (N := Z.to_nat P384_FpInv.p384_divstep_iters).
  destruct (P384_FpInv.iter_divstep_spec p N 1 p x 0 1)
    as [[[[d_N f_N] g_N] v_N] r_N] eqn:Espec.
  pose proof (P384_FpInv.iter_invariant p N 1 p x 0 1 x
                P384_FpInv.p_pos P384_FpInv.p_odd) as Hinv.
  ...
  pose proof (P384_FpInv.iter_dfg_agree N p 1 p x 0 1) as Hagree.
  rewrite Espec in Hagree.
  destruct (P384_FpInv.iter_divstep_dfg N 1 p x) as [[d2 f2] g2] eqn:Edfg.
  destruct Hagree as [_ [Hf_eq Hg_eq]]. subst f2 g2.
  pose proof (by_convergence_dfg_p384_closed x Hx Hgcd) as Hconv.
  unfold N in Edfg. rewrite Edfg in Hconv.
  destruct Hconv as [Hg0 Hf_pm1]. subst g_N.
  assert (Hp : p <> 0) by (pose proof P384_FpInv.p_pos; lia).
  Opaque P384_FpInv.p384_p.
  ...
  - subst f_N. rewrite Z.mul_1_r in Hinv_v.
    ...
    transitivity ((2 ^ Z.of_nat N * P384_FpInv.precomp_val) mod p).
    { apply Zmul_mod_compat. exact Hvx. }
    rewrite Z.mul_comm.
    unfold N.
    rewrite Z2Nat.id by (unfold P384_FpInv.p384_divstep_iters; vm_compute; discriminate).
    exact P384_FpInv.precomp_times_pow2.
  - subst f_N.
