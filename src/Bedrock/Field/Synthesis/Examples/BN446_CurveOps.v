(** * BN446 Curve Operations: point_double, store_zero, point_negate,
    curve_add_inplace.

    Instantiates the generic curve operations from [CurveAdd/] for BN446
    (y^2 = x^3 + 257, so 3b = 771 -- see [bn446_three_b.v]).

    Provides:
    - [bn446_point_double] : dedicated doubling (RCB 2015 Algorithm 9,
    ...
    - [bn446_store_zero]   : store the identity point (0:1:0)
    - [bn446_point_negate_func] : negate Y coordinate
    - [bn446_curve_add_inplace] : ladderstep with in-place output

    These are used by the wNAF scalar multiplication chain
    ([BN446_wNAF_Instance.v]), which expects [HCurveDouble] and
    [HCurveAddInplace] function specs. *)

Require Import Bedrock.Field.Synthesis.Examples.bn446_prime.
Require Import Bedrock.Field.Synthesis.Examples.bn446_three_b.
Require Import Bedrock.Field.Synthesis.Examples.BN446_G1.
...
(* ================================================================== *)
(** * Section 1: Concrete BN446 function definitions                   *)
(* ================================================================== *)

Section BN446_CurveOps.
  ...
  (** ** Point doubling: RCB 2015 Algorithm 9, homogeneous, a = 0.
      ...
      the callee list [ladderstep_body] uses, so BN446 needs no leaf it
      does not already have; the loader is "bn446_three_b", the same
      constant [bn446_G1_add] passes to [ladderstep_body].
  ...
  Definition bn446_point_double : function_t :=
    ("curve_double_a0", rcb_double_a0_body "bn446_three_b").
  ...
  (** ** Store zero (identity point): (0 : 1 : 0) in Jacobian coords.
      ...
      We supply "bn446_from_word" for both, since the from_word function
      ...
      This matches the pattern in [BN446_wNAF_Extract.v] and
      [StorePointAtInfinity.v] where function bodies are defined inline. *)
  Definition bn446_store_zero : function_t :=
    ("store_zero",
     (["outx"; "outy"; "outz"],
      []:list String.string,
      bedrock_func_body:(
  ...
  Definition bn446_curve_add_inplace : function_t :=
    @curve_add_inplace_wrapper _ _ _ _ bn446_field_parameters bn446_frep.

  (** ** Point negation: negate Y coordinate via [opp]. *)
  Definition bn446_point_negate_func : function_t :=
    @point_negate_func bn446_field_parameters.
...
Section BN446_CurveOps_Specs.
  ...
  (** The point_double spec: calling "curve_double_a0" with 6 DISJOINT
      ...
      This is NOT yet [BN446_wNAF_Instance.HCurveDouble].  That
  ...
  (** The ladderstep (curve_add) spec, parameterized by three_b witness.
      BN446 uses three_b = 9; the bounded witness is in [bn446_three_b.v]. *)
  Definition bn446_curve_add_spec
    (three_b : Crypto.Bedrock.Specs.Field.felem)
    (functions : Semantics.env) : Prop :=
  ...
  Definition bn446_point_negate_spec
    (functions : Semantics.env) : Prop :=
  ...
  (** ** Correctness lemmas.
      ...
      BN446 functions. The proofs require showing that the Derived
      ...
      provided by the bn446 field synthesis.

      [bn446_point_double_correct] is Qed from
      [PointDoubleA0.rcb_double_a0_correct];
      [bn446_store_zero_correct] is Qed from
      [NistWnafWrappers.store_zero_from_word_ok]. *)

  Lemma bn446_point_double_correct :
    forall (three_b : Crypto.Bedrock.Specs.Field.felem) functions,
      (* The point_double function is in the environment *)
      map.get functions "curve_double_a0" = Some (snd bn446_point_double) ->
      ...
      PointDoubleA0.spec_of_three_b_loader_a0
        (field_parameters:=bn446_field_parameters)
        (field_representation:=bn446_frep)
        three_b "bn446_three_b" functions ->
      bn446_point_double_spec three_b functions.
  Proof.
    intros three_b functions Henv Hmul Hadd Hsub Hloader.
    (* loose_bounds = tight_bounds for BN446 word-by-word Montgomery *)
    assert (Hbe : loose_bounds = tight_bounds) by reflexivity.
    exact (@rcb_double_a0_correct
             _ _ _ _ _ _ _ _ _ _
             bn446_field_parameters bn446_frep bn446_frep_ok
             Hbe three_b "bn446_three_b" I
  ...
  (** ** The doubling really doubles.
      ...
      on-curve input.  BN446 is y^2 = x^3 + 257, so b = 257 and
      3b = 771 ([bn446_three_b.v]).

      The [feval three_b = 771] hypothesis is the a = 0 curve-constant
      obligation [BN254_wNAF_Laws] lists (that file is shared by all
      three a = 0 BN curves); the bounded witness lives in
      [bn446_three_b.v]. *)
  Lemma bn446_double_is_curve_add
    (three_b : Crypto.Bedrock.Specs.Field.felem)
    (Hthree_b : feval (proj1_sig three_b)
                = ModularArithmetic.F.of_Z M_pos 771) :
    forall P,
      PointDoubleA0.oncurve_a0 (ModularArithmetic.F.of_Z M_pos 257) P ->
  ...
  Proof.
    ...
    apply (PointDoubleA0.rcb_double_a0_eq_ladderstep
             (ModularArithmetic.F.of_Z M_pos 257)); [ | exact HP ].
    rewrite Hthree_b.
  ...
  (** ** What is still missing before [HCurveDouble] follows.
      ...
      [BN446_wNAF_Instance.HCurveDouble] is
  ...
  Lemma bn446_store_zero_correct :
    forall functions,
      spec_of_from_word (field_representation:=bn446_frep) functions ->
      map.get functions "store_zero" = Some (snd bn446_store_zero) ->
      bn446_store_zero_spec functions.
  Proof.
    intros functions Hfw Hsz.
    (* [bn446_store_zero] is [NistWnafWrappers.store_zero_from_word_func]
       at these instances (same three [from_word] calls with literals
       0, 1, 0, same parameter names), so [Hsz] meets the table premise
       of [store_zero_from_word_ok] by conversion.  The limb count (7 x
       64 here) plays no role: the lemma quantifies over
       [FieldRepresentation] and reasons through [felem_to_bytes] and
       [spec_of_from_word], never over the encoding's length.
...
End BN446_CurveOps_Specs.

(* ================================================================== *)
(** * Section 3: All BN446 curve operation functions                   *)
(* ================================================================== *)

Section BN446_AllCurveOps.
  ...
  (** Complete list of curve operation functions for extraction.
      ...
      multiplication on BN446 G1 (beyond the leaf Fp operations). *)
  Definition bn446_curve_op_funcs : list function_t :=
    [ bn446_G1_add;            (* ladderstep: "curve_add" *)
      bn446_point_double;      (* RCB Alg. 9 doubling: "curve_double_a0" *)
      bn446_store_zero;        (* identity point: "store_zero" *)
      bn446_curve_add_inplace; (* inplace wrapper: "curve_add_inplace" *)
      bn446_point_negate_func  (* Y negation: "point_negate" *)
    ].

End BN446_AllCurveOps.
