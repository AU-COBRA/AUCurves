...
//! leaves they replace.
//!
//! `p224::fp_mul` / `p224::fp_square` are the CryptOpt assembly when
//! `build.rs` linked it and the fiat functions otherwise;
//! `p224::fp_mul_fiat` / `p224::fp_square_fiat` are always the fiat
...
//!
//! Note that fiat-crypto represents P-224 word-by-word in four 64-bit limbs
//! rather than in a Solinas form, so the CryptOpt `fiat_p224_*` leaves and
//! this crate's field type agree on the representation.

use p224::{fp_mul, fp_mul_fiat, fp_square, fp_square_fiat, fp_to_montgomery, Fp, FpRaw};

/// p = 2^224 - 2^96 + 1, little-endian 64-bit limbs.  The top 32 bits of
/// the high limb are always zero.
const P: [u64; 4] = [
...
fn edges() -> Vec<Fp> {
    ...
    vec![
        ...
        Fp([!0, !0, 0, 0]),
        // Only the high limb set, at its maximum.
        Fp([0, 0, 0, 0x0000_0000_ffff_ffff]),
        // Straddles the structured limb 1 of p.
        Fp([0, 0xffff_ffff_0000_0000, 0, 0]),
        Fp([!0, 0xffff_fffe_ffff_ffff, !0, 0x0000_0000_ffff_fffe]),
...
impl Rng {
    ...
    fn fe(&mut self) -> Fp {
        loop {
            ...
            // The modulus is only 224 bits, so an unmasked high limb almost
            // never lands in range.
            w[3] &= 0x0000_0000_ffff_ffff;
            if w[0] & 1 == 0 {
