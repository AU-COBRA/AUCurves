(** * ExtractFe25519AddReal: drive [fe25519_add] through
 *    [rust_cmd_ed_to_real_jasmin] to a real Jasmin [expr.cmd] AST.
 *
 *  Companion to [ExtractFe25519MulReal.v].  5-limb radix-2^51 add chain.
 *)
...
Definition fe25519_add_concrete : SafeRustEd25519Sim.rust_cmd_ed :=
  fe25519_add_body
    {| loc_var := "out"; loc_type := TBytes 40 |}
    [ {| loc_var := "x"; loc_type := TBytes 40 |};
      {| loc_var := "y"; loc_type := TBytes 40 |} ].

Definition fe25519_add_real :
  list (@instr x86_extended_op asm_opI) :=
  rust_cmd_ed_to_real_jasmin fe25519_add_concrete.

Definition fe25519_add_real_normalised :
  list (@instr x86_extended_op asm_opI) :=
  Eval vm_compute in fe25519_add_real.

Redirect "fe25519_add_real_jasmin_ast"
  Print fe25519_add_real_normalised.
