...
//! *same function* as `fiat_p256_mul` / `fiat_p256_square` — CryptOpt's
...
//! `P256_NO_CRYPTOPT=1` is set, the fiat leaves are used unchanged.
...
#[cfg(p256_cryptopt_asm)]
unsafe extern "C" {
    /// `out = x * y * R^-1 mod p`.  See `generated/p256_mul_cryptopt.asm`.
    fn p256_cryptopt_mul(out: *mut u64, x: *const u64, y: *const u64);
    /// `out = x * x * R^-1 mod p`.  See `generated/p256_square_cryptopt.asm`.
    fn p256_cryptopt_square(out: *mut u64, x: *const u64);
}

/// Whether this build calls the CryptOpt assembly for [`fp_mul`] and
/// [`fp_square`].  `false` means the fiat-rust leaves are in use.
pub const CRYPTOPT_ASM: bool = cfg!(p256_cryptopt_asm);
...
pub fn fp_mul(out: &mut Fp, x: &Fp, y: &Fp) {
    ...
    #[cfg(p256_cryptopt_asm)]
    unsafe {
        p256_cryptopt_mul(out.0.as_mut_ptr(), x.0.as_ptr(), y.0.as_ptr())
    }
    #[cfg(not(p256_cryptopt_asm))]
...
pub fn fp_square(out: &mut Fp, x: &Fp) {
    ...
    #[cfg(p256_cryptopt_asm)]
    unsafe {
        p256_cryptopt_square(out.0.as_mut_ptr(), x.0.as_ptr())
    }
    #[cfg(not(p256_cryptopt_asm))]
...
#[inline] pub fn fp_mul_fiat(out: &mut Fp, x: &Fp, y: &Fp) { fiat_p256_mul(out, x, y) }

/// The fiat-rust squaring, whatever [`fp_square`] is bound to.
#[inline] pub fn fp_square_fiat(out: &mut Fp, x: &Fp) { fiat_p256_square(out, x) }
