(** * P-256 felem_copy: 4 x 64-bit limbs (32 bytes).

    Verbatim clone of [bn254_felem_copy.v] at the P-256 field
    representation [p256_frep] (same limb count and byte size, so the
    ...
    Honesty ledger: 0 Admitted (expected; uncompiled draft). *)
...
Section FelemCopy.
    ...
    Existing Instance p256_field_parameters.
    Existing Instance p256_frep.
    Existing Instance p256_frep_ok.

    (* P-256 uses 4 x 64-bit words (256 bits).  The definition is named
       after the FieldParameters string [felem_copy] = "p256_coord_felem_copy"
       (prefix "p256_coord_"), because [program_logic_goal_for_function!]
       looks up [spec_of "<definition name>"]; bn254_felem_copy.v gets
       this for free since its prefix is "bn254_". *)
    Definition p256_coord_felem_copy : Syntax.func := (["out"; "in"], (nil : list string), bedrock_func_body:(
    ...
    Instance p256_felem_copy_spec : spec_of felem_copy := spec_of_felem_copy.

    Lemma p256_felem_copy_ok : program_logic_goal_for_function! p256_coord_felem_copy.
    Proof.
      ...
      cbv match beta delta [WeakestPrecondition.func p256_coord_felem_copy].
