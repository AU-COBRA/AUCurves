...
let even_map_half = function
| (k, v) -> ((Z.sub k Big_int_Z.unit_big_int), (dDSet_map even_trans v))

(** val odd_map_half :
    (Big_int_Z.big_int * DDSet.t) -> Big_int_Z.big_int * DDSet.t **)

let odd_map_half = function
| (k, v) ->
  if Z.ltb k Big_int_Z.zero_big_int
  then ((Z.sub (Z.opp k) (Big_int_Z.mult_int_big_int 2
          Big_int_Z.unit_big_int)),
         (dDSet_map odd_pos_trans v))
  else ((Z.sub k Big_int_Z.unit_big_int), (dDSet_map odd_nonpos_trans v))

(** val processDivstep_half : Big_int_Z.big_int -> state -> state **)

let processDivstep_half m s =
  let f = fun kv -> (even_map_half kv) :: ((odd_map_half kv) :: []) in
...
(** val state0_half : state **)

let state0_half =
  ZMap.add (Big_int_Z.minus_big_int Big_int_Z.unit_big_int) set0 empty0
