...
Section Spec.
    ...
    Require Import Crypto.Bedrock.Field.Synthesis.Examples.p224_64.
    Local Notation m := p224_64.m.
    Local Definition prefix := p224_64.prefix. (* placed before function names *)

    Require Import Bedrock.Examples.felem_copy_p224_64.
    Local Definition affix := aff.

      Existing Instances Defaults64.default_parameters p224_bedrock2_funcs
      p224_bedrock2_specs p224_bedrock2_correctness.
  ...
  Definition Fp2_add_alt2 : Syntax.func :=
    ...
    let yisep := "yisep" in
    let add := (append prefix "add") in  
    let felem_copy := ("felem_copy" ++ aff) in  
    ("Fp2_add_alt2", (
      [outr; outi; xr; xi; yr; yi], [],
      bedrock_func_body:(
