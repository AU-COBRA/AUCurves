(** * BLS12_PairingRustConcrete.v — BLS12-381 safe-Rust pairing refinement.
 *
 * Mirror of [BLS12_PairingRustConcrete.v] for BLS12-381.  Structural
 * shape is identical (4 stackallocs + 5 calls: 3 loaders, miller loop,
 * final exponentiation).  Only these differ:
 *
 *   - [bls12_N = 6] (BLS12-381's 381-bit prime = 6×64-bit limbs).
 *   - [loop_neg bls12_381_params = true] ⇒ post-miller conjugation
 *     appears in [bls12_optimal_ate_decomposes].
 *   - [optimal_ate_extras bls12_381_params = []] ⇒ no post-miller
 *     Frobenius corrections (BLS12-381 doesn't use D-type extras).
 *   - Different loader Montgomery-limb constants for the BLS12-381
 *     Frobenius values γ₁/γ_y/γ_{1,p²}.
 *   - Twist is M-twist (vs BLS12's D-twist), xi = 1+u.
 *
 * The composition infrastructure ([RustComposition.v], §8 templates)
 * is curve-agnostic and reused unchanged.  This file is a ~direct copy
 * of [BLS12_PairingRustConcrete.v] with [bls12_*] → [bls12_*] and the
 * above adaptations.
 *)
...
Require Import Bedrock.SafeRustBLS12Concrete.
...
(** NB: we do NOT [Require Import MillerLoopWP] because that
    ...
    small [bls12_optimal_ate_spec] definition locally below. *)

(* ================================================================ *)
(* §0. Global parameters                                             *)
(* ================================================================ *)

Definition bls12_N : nat := 6.  (** 6 × 64-bit limbs = 384 bits (381-bit prime). *)
Definition bls12_u64_max : nat := Nat.pow 2 64.

(** Local mirror of [BLS12_PairingCorrect.bls12_miller_loop_with_corrections].
    Copied to keep this file independent of [BLS12_PairingCorrect.v]
    (which pulls in a heavy bedrock2 instance stack).  Step 2's
    bridge proofs connect the two via reflexivity / unfolding. *)
Local Definition bls12_p_val : Z := prime_p bls12_381_params.
Local Definition bls12_xi_val : Fp2_Z := (1%Z, 1%Z).  (** BLS12-381: xi = 1 + u *)

(** [bls12_miller_loop_with_corrections]: local mirror of the bedrock2
    [bls12_miller_loop_with_corrections] spec.  Includes both the
    Frobenius corrections (empty for BLS12-381 since
    [optimal_ate_extras = []]) AND the [loop_neg] conjugation step —
    which IS non-trivial for BLS12-381 ([loop_neg = true]).  Packaging
    the conj inside this function keeps the post-miller → pre-finalexp
    chain structurally identical to BN254's. *)
Definition bls12_miller_loop_with_corrections
           (gamma1 gamma_y gamma1_p2 : Fp2_Z)
           (Px Py : Z) (Qx Qy : Fp2_Z) : Fp12_Z :=
  let '(f, Tx, Ty) :=
    affine_miller_aux bls12_381_zmod_ops
      (loop_abs bls12_381_params)
      (Z.to_nat (Z.log2 (loop_abs bls12_381_params)))
      Px Py Qx Qy
      (fp12_one bls12_381_zmod_ops) Qx Qy in
  let f' := PairingSpec.apply_corrections
              bls12_381_zmod_ops
              (zfp2_conj bls12_p_val)
              (zfp2_mul_const bls12_p_val)
              (optimal_ate_extras bls12_381_params)
              f Tx Ty Px Py Qx Qy
              gamma1 gamma_y gamma1_p2 in
  if loop_neg bls12_381_params then zfp12_conj bls12_p_val f' else f'.

(** Local copy of [MillerLoopWP.bls12_optimal_ate_spec] (see note
    on the import block above).  Identical definition. *)
Definition bls12_optimal_ate_spec
    (gamma1 gamma_y gamma1_p2 : Fp2_Z)
    (Px Py : Z) (Qx Qy : Fp2_Z) : Fp12_Z :=
  PairingSpec.optimal_ate
    bls12_381_zmod_ops
    (zfp12_conj bls12_p_val)
    (zfp12_inv bls12_p_val bls12_xi_val)
    (zfp12_frob_p2 bls12_p_val bls12_xi_val)
    (zfp12_pow bls12_p_val bls12_xi_val)
    (zfp2_conj bls12_p_val)
    (zfp2_mul_const bls12_p_val)
    bls12_381_params
...
Definition bls12_fp2_eval (v : rust_val TFp2) : Fp2_Z :=
  match v with
  | VFp2 a b => (bls12_fp_eval a, bls12_fp_eval b)
  end.

Definition bls12_fp6_eval (v : rust_val TFp6) : Fp6_Z :=
  match v with
  | VFp6 c0 c1 c2 =>
      (bls12_fp2_eval c0, bls12_fp2_eval c1, bls12_fp2_eval c2)
  end.

Definition bls12_fp12_eval (v : rust_val TFp12) : Fp12_Z :=
  match v with
  | VFp12 c0 c1 => (bls12_fp6_eval c0, bls12_fp6_eval c1)
  end.

(* ================================================================ *)
(* §2. BLS12 CallEnv (concrete Definition, not Parameter)            *)
(* ================================================================ *)

Fixpoint bls12_bind_params_aux
  ...
  | p :: ps, a :: as_ =>
      ...
      | Some (existT t v) =>
          bls12_bind_params_aux ps as_ rs_caller
...
Definition bls12_bind_params
           (params : list var) (args : list located) (rs : rust_state)
  : option rust_state :=
  bls12_bind_params_aux params args rs rs_empty.

Definition bls12_extract_output
...
Definition bls12_writeback_output
...
Definition bls12_call_env : CallEnv :=
  {| ce_bind_params     := bls12_bind_params;
     ce_extract_output  := bls12_extract_output;
     ce_writeback_output := bls12_writeback_output |}.

(* ================================================================ *)
(* §3. Concrete function environment                                 *)
(*                                                                   *)
(*   The bodies [bls12_*_body] are still [Parameter]s at this step   *)
(*   — they will be filled in by applying [btranslate] to the        *)
(*   bedrock2 source in Steps 3-4.  What Step 1 buys us is:          *)
(*                                                                   *)
(*     ∙ [bls12_pairing_fenv] is a concrete list literal, so         *)
(*       [fenv_lookup bls12_pairing_fenv "bls12_..."] reduces by     *)
(*       [cbv] to the expected [Some (params, body)].                *)
(*                                                                   *)
(*     ∙ The 5 [fenv_has_*] axioms in BLS12_PairingRust.v become     *)
...
(** Concrete loader body template: 12 [RLimbStore]s laying down the
    [N = 6] limbs of the two [Fp] components of an [Fp2] result.  The
    specific limb values [l_{00..05}] (c0's 6 limbs) and
    [l_{10..15}] (c1's 6 limbs) are passed as parameters so the same
    body shape instantiates to any BLS12 Frobenius constant. *)
Definition bls12_loader_body
           (l00 l01 l02 l03 l04 l05 l10 l11 l12 l13 l14 l15 : nat) : rust_cmd :=
          ...
          (RSeq (RLimbStore loaders_out_c1 2 (SLit l12))
           (RSeq (RLimbStore loaders_out_c1 3 (SLit l13))
            (RSeq (RLimbStore loaders_out_c1 4 (SLit l14))
                  (RLimbStore loaders_out_c1 5 (SLit l15)))))))))))).

(** The three loader bodies are the same skeleton, parameterised by
    12-tuples of Montgomery limbs.  The specific values for each
    Frobenius constant are declared as [Parameter]s here; they would
    be [vm_compute]d from [bls12_381_params] at instantiation time. *)
Parameter bls12_gamma1_l00 bls12_gamma1_l01 bls12_gamma1_l02 bls12_gamma1_l03
          bls12_gamma1_l04 bls12_gamma1_l05
          bls12_gamma1_l10 bls12_gamma1_l11 bls12_gamma1_l12 bls12_gamma1_l13
          bls12_gamma1_l14 bls12_gamma1_l15
        : nat.
Parameter bls12_gamma_y_l00 bls12_gamma_y_l01 bls12_gamma_y_l02 bls12_gamma_y_l03
          bls12_gamma_y_l04 bls12_gamma_y_l05
          bls12_gamma_y_l10 bls12_gamma_y_l11 bls12_gamma_y_l12 bls12_gamma_y_l13
          bls12_gamma_y_l14 bls12_gamma_y_l15
        : nat.
Parameter bls12_gamma1_p2_l00 bls12_gamma1_p2_l01 bls12_gamma1_p2_l02 bls12_gamma1_p2_l03
          bls12_gamma1_p2_l04 bls12_gamma1_p2_l05
          bls12_gamma1_p2_l10 bls12_gamma1_p2_l11 bls12_gamma1_p2_l12 bls12_gamma1_p2_l13
          bls12_gamma1_p2_l14 bls12_gamma1_p2_l15
        : nat.

Definition bls12_load_g1_body : rust_cmd :=
  bls12_loader_body
    bls12_gamma1_l00 bls12_gamma1_l01 bls12_gamma1_l02 bls12_gamma1_l03
    bls12_gamma1_l04 bls12_gamma1_l05
    bls12_gamma1_l10 bls12_gamma1_l11 bls12_gamma1_l12 bls12_gamma1_l13
    bls12_gamma1_l14 bls12_gamma1_l15.
Definition bls12_load_g2_body : rust_cmd :=
  bls12_loader_body
    bls12_gamma_y_l00 bls12_gamma_y_l01 bls12_gamma_y_l02 bls12_gamma_y_l03
    bls12_gamma_y_l04 bls12_gamma_y_l05
    bls12_gamma_y_l10 bls12_gamma_y_l11 bls12_gamma_y_l12 bls12_gamma_y_l13
    bls12_gamma_y_l14 bls12_gamma_y_l15.
Definition bls12_load_w_body : rust_cmd :=
  bls12_loader_body
    bls12_gamma1_p2_l00 bls12_gamma1_p2_l01 bls12_gamma1_p2_l02 bls12_gamma1_p2_l03
    bls12_gamma1_p2_l04 bls12_gamma1_p2_l05
    bls12_gamma1_p2_l10 bls12_gamma1_p2_l11 bls12_gamma1_p2_l12 bls12_gamma1_p2_l13
    bls12_gamma1_p2_l14 bls12_gamma1_p2_l15.
...
(** Concrete bodies for the miller loop and final exponentiation.
    ...
    2. Build [bls12_finalexp_body] as a chain of [RCall]s into
       [bls12_Fp12_*] tower ops, each itself a [RCall] chain into
       [bls12_Fp6_*] / [bls12_Fp2_*] / [bls12_Fp_*] until reaching
       leaf operations.
    3. Build [bls12_miller_body] as a [RWhileNz] over a loop counter,
...
Definition bls12_miller_body : rust_cmd :=
  RLimbStore miller_dummy_loc 0 (SLit 0).
Definition bls12_finalexp_body : rust_cmd :=
  RLimbStore miller_dummy_loc 0 (SLit 0).

(** Parameter lists as used in the bedrock2 sources. *)
Definition bls12_load_g1_params : list var := ["out"].
Definition bls12_load_g2_params : list var := ["out"].
Definition bls12_load_w_params  : list var := ["out"].
Definition bls12_miller_params  : list var :=
  ["tmp"; "px"; "py"; "qx"; "qy"].
Definition bls12_finalexp_params : list var :=
  ["out"; "f"; "g1"; "g2"; "w"].

Definition bls12_pairing_fenv : rust_fenv :=
  [ ("bls12_load_gamma1_p2",      (bls12_load_g1_params,
                                   bls12_load_g1_body));
    ("bls12_load_gamma2_p2",      (bls12_load_g2_params,
                                   bls12_load_g2_body));
    ("bls12_load_w_frob_p2_c1",   (bls12_load_w_params,
                                   bls12_load_w_body));
    ("bls12_miller_loop_optimal", (bls12_miller_params,
                                   bls12_miller_body));
    ("bls12_final_exp_dsd",       (bls12_finalexp_params,
                                   bls12_finalexp_body)) ].
  ...
  Definition mid_ml : spec_t :=
    fun rs =>
      ...
      exists ml_val px py qx qy,
        ...
        bls12_fp12_eval ml_val =
          bls12_miller_loop_with_corrections
            gamma1 gamma_y gamma1_p2
            (bls12_fp_eval px) (bls12_fp_eval py)
            (bls12_fp2_eval qx) (bls12_fp2_eval qy).

  (** [pairing_post]: after final_exp, [out_loc] holds the full
      optimal-ate pairing value.  Matches the G2 strong spec
      postcondition (see BLS12_PairingCorrect.v:387-390). *)
  Definition pairing_post : spec_t :=
    fun rs =>
      exists out px py qx qy,
        ...
        extract_at q_y_loc TFp2 rs = Some qy /\
        bls12_fp12_eval out =
          bls12_optimal_ate_spec gamma1 gamma_y gamma1_p2
            (bls12_fp_eval px) (bls12_fp_eval py)
            (bls12_fp2_eval qx) (bls12_fp2_eval qy).
...
Lemma fenv_has_pairing_load_g1 :
  fenv_lookup bls12_pairing_fenv "bls12_load_gamma1_p2" =
    Some (bls12_load_g1_params, bls12_load_g1_body).
Proof. reflexivity. Qed.

Lemma fenv_has_pairing_load_g2 :
  fenv_lookup bls12_pairing_fenv "bls12_load_gamma2_p2" =
    Some (bls12_load_g2_params, bls12_load_g2_body).
Proof. reflexivity. Qed.

Lemma fenv_has_pairing_load_w :
  fenv_lookup bls12_pairing_fenv "bls12_load_w_frob_p2_c1" =
    Some (bls12_load_w_params, bls12_load_w_body).
Proof. reflexivity. Qed.

Lemma fenv_has_pairing_miller :
  fenv_lookup bls12_pairing_fenv "bls12_miller_loop_optimal" =
    Some (bls12_miller_params, bls12_miller_body).
Proof. reflexivity. Qed.

Lemma fenv_has_pairing_finalexp :
  fenv_lookup bls12_pairing_fenv "bls12_final_exp_dsd" =
    Some (bls12_finalexp_params, bls12_finalexp_body).
Proof. reflexivity. Qed.
  ...
  Definition finalexp_callee_post (f_v : Fp12_Z) : spec_t :=
    fun rs =>
      exists out,
        ...
        bls12_fp12_eval out =
          PairingSpec.final_exp bls12_381_zmod_ops
            (zfp12_conj bls12_p_val)
            (zfp12_inv bls12_p_val bls12_xi_val)
            (zfp12_frob_p2 bls12_p_val bls12_xi_val)
            (zfp12_pow bls12_p_val bls12_xi_val)
            (prime_p bls12_381_params) (scalar_r bls12_381_params)
            f_v.
...
(*     [bls12_call_env] from §2.  These proofs use only [destruct],  *)
(*     [exists], and the concrete definitions of [extract_at] /      *)
(*     [bls12_bind_params] / [bls12_extract_output] /                *)
(*     [bls12_writeback_output].                                     *)
(* ================================================================ *)

(* ================================================================ *)
(* §7.5 Loader body execution lemmas                                 *)
(*                                                                   *)
(*   Structural facts about [bls12_loader_body]: executing the 8    *)
...
Lemma limb_store_c0_effect :
  forall k v rs rs' vc0 vc1,
    ...
    rust_exec_fenv bls12_N bls12_u64_max bls12_pairing_fenv
                   bls12_leaf_spec_concrete bls12_call_env
...
(** Full loader body effect: 12 [RLimbStore]s transform an [Fp2] value
    whose two components are 6-limb lists into one where each limb
    matches the corresponding parameter.  The length-6 assumption
    captures the BLS12 limb count [bls12_N = 6] and is enforced
    indirectly at call time: the [Fp2] stackalloc initialises "out"
    to [vfp2_zero bls12_N = VFp2 (VFp [0;0;0;0;0;0]) (VFp [0;0;0;0;0;0])],
    both of length 6. *)
Lemma bls12_loader_body_effect :
  forall l00 l01 l02 l03 l04 l05 l10 l11 l12 l13 l14 l15 rs rs'
         lc0_init lc1_init,
    length lc0_init = 6%nat -> length lc1_init = 6%nat ->
    extract_at (loc_Fp2 "out") TFp2 rs = Some (VFp2 (VFp lc0_init) (VFp lc1_init)) ->
    rust_exec_fenv bls12_N bls12_u64_max bls12_pairing_fenv
                   bls12_leaf_spec_concrete bls12_call_env
                   (bls12_loader_body l00 l01 l02 l03 l04 l05
                                      l10 l11 l12 l13 l14 l15) rs rs' ->
    extract_at (loc_Fp2 "out") TFp2 rs' =
      Some (VFp2 (VFp [l00; l01; l02; l03; l04; l05])
                 (VFp [l10; l11; l12; l13; l14; l15])).
Proof.
  intros l00 l01 l02 l03 l04 l05 l10 l11 l12 l13 l14 l15 rs rs'
         lc0_init lc1_init Hlen0 Hlen1 Hext Hexec.
  unfold bls12_loader_body in Hexec.
  (* Step through 12 RSeq inversions. *)
  ...
  inversion R10  as [| ? ? ? ? r11 H11 H12 | | | | | | | | | | ]; clear R10; subst.
  (* H1..H6 : c0 stores (k=0..5); H7..H12 : c1 stores (k=0..5). *)
  ...
  (* For lc0_init of length 6, the 6 c0 set_nths produce [l00;l01;l02;l03;l04;l05]. *)
  destruct lc0_init as [| a0 [| a1 [| a2 [| a3 [| a4 [| a5 [| ??]]]]]]];
    try (cbn in Hlen0; lia).
  cbn in E6.
  pose proof (limb_store_c1_effect _ _ _ _ _ _ E6   H7)  as E7.  cbn in E7.
  ...
  destruct lc1_init as [| b0 [| b1 [| b2 [| b3 [| b4 [| b5 [| ??]]]]]]];
...
Section PairingBodyRefines.
  ...
  (** Fresh-variable hypotheses: the 4 stackalloc names must not
      collide with the user-location variable names.  In BLS12 the
  ...
  (** [load_*_body_refines]: discharged in Phase 3 by [inversion] on
      ...
      [bls12_loader_body_skeleton] is built from [RSeq] + [RLimbStore]
  ...
  (** Arithmetic hypotheses connecting the specific [bls12_gamma*_l**]
  ...
  Hypothesis Hgamma1_fp2_eval :
    bls12_fp2_eval
      (VFp2 (VFp [bls12_gamma1_l00; bls12_gamma1_l01; bls12_gamma1_l02;
                  bls12_gamma1_l03; bls12_gamma1_l04; bls12_gamma1_l05])
            (VFp [bls12_gamma1_l10; bls12_gamma1_l11; bls12_gamma1_l12;
                  bls12_gamma1_l13; bls12_gamma1_l14; bls12_gamma1_l15]))
    = gamma1.
  Hypothesis Hgamma_y_fp2_eval :
    bls12_fp2_eval
      (VFp2 (VFp [bls12_gamma_y_l00; bls12_gamma_y_l01; bls12_gamma_y_l02;
                  bls12_gamma_y_l03; bls12_gamma_y_l04; bls12_gamma_y_l05])
            (VFp [bls12_gamma_y_l10; bls12_gamma_y_l11; bls12_gamma_y_l12;
                  bls12_gamma_y_l13; bls12_gamma_y_l14; bls12_gamma_y_l15]))
    = gamma_y.
  Hypothesis Hgamma1_p2_fp2_eval :
    bls12_fp2_eval
      (VFp2 (VFp [bls12_gamma1_p2_l00; bls12_gamma1_p2_l01; bls12_gamma1_p2_l02;
                  bls12_gamma1_p2_l03; bls12_gamma1_p2_l04; bls12_gamma1_p2_l05])
            (VFp [bls12_gamma1_p2_l10; bls12_gamma1_p2_l11; bls12_gamma1_p2_l12;
                  bls12_gamma1_p2_l13; bls12_gamma1_p2_l14; bls12_gamma1_p2_l15]))
    = gamma1_p2.

  (** Structural hypothesis: when the pairing chain calls a loader,
      "out" in [rs_init] holds a length-6-limb [Fp2] value — because
      [bls12_bind_params] copies the value from the caller's slot, and
      the caller slot was initialised by stackalloc with
      [tt_zero bls12_N TFp2 = VFp2 (VFp [0;0;0;0;0;0]) (VFp [0;0;0;0;0;0])],
      which has length-6 components. *)
  Hypothesis Hloader_input_len6 :
    forall rs,
      (exists v, extract_at (loc_Fp2 out_var) TFp2 rs = Some v) ->
      exists lc0 lc1,
        length lc0 = 6%nat /\ length lc1 = 6%nat /\
        extract_at (loc_Fp2 out_var) TFp2 rs =
          Some (VFp2 (VFp lc0) (VFp lc1)).

  Theorem load_g1_body_refines :
    rust_refines bls12_N bls12_u64_max bls12_pairing_fenv
                 bls12_leaf_spec_concrete bls12_call_env
                 load_g1_callee_pre bls12_load_g1_body
                 (load_g1_callee_post gamma1).
  Proof.
    unfold rust_refines, load_g1_callee_pre.
    intros rs1 rs2 Hpre Hexec.
    pose proof (Hloader_input_len6 _ Hpre) as [lc0 [lc1 [Hlen0 [Hlen1 Hext]]]].
    unfold load_g1_callee_post.
    eexists. split;
      [|exact Hgamma1_fp2_eval].
    unfold bls12_load_g1_body in Hexec.
    exact (bls12_loader_body_effect _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
  ...
  Theorem load_g2_body_refines :
    rust_refines bls12_N bls12_u64_max bls12_pairing_fenv
                 bls12_leaf_spec_concrete bls12_call_env
                 load_g2_callee_pre bls12_load_g2_body
  ...
  Proof.
    ...
    unfold bls12_load_g2_body in Hexec.
    exact (bls12_loader_body_effect _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
             Hlen0 Hlen1 Hext Hexec).
  Qed.
  ...
  Theorem load_w_body_refines :
    rust_refines bls12_N bls12_u64_max bls12_pairing_fenv
                 bls12_leaf_spec_concrete bls12_call_env
                 load_w_callee_pre bls12_load_w_body
  ...
  Proof.
    ...
    unfold bls12_load_w_body in Hexec.
    exact (bls12_loader_body_effect _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
             Hlen0 Hlen1 Hext Hexec).
  Qed.
    ...
    forall px_v py_v qx_v qy_v,
      rust_refines bls12_N bls12_u64_max bls12_pairing_fenv
                   ...
                   (miller_callee_post gamma1 gamma_y gamma1_p2
                                       px_v py_v qx_v qy_v).
  Proof.
    intros; unfold rust_refines, bls12_miller_body.
  ...
  Proof.
    intros; unfold rust_refines, bls12_finalexp_body.
    intros rs1 rs2 _ Hexec. exfalso.
    inversion Hexec; discriminate.
  Qed.
  ...
  Definition pairing_calls : rust_cmd :=
    RSeq (RCall "bls12_load_gamma1_p2"      g1_loc [g1_loc])
      (RSeq (RCall "bls12_load_gamma2_p2"   g2_loc [g2_loc])
        (RSeq (RCall "bls12_load_w_frob_p2_c1" w_loc [w_loc])
          (RSeq (RCall "bls12_miller_loop_optimal" tmp_loc
                       [tmp_loc; p_x_loc; p_y_loc; q_x_loc; q_y_loc])
                (RCall "bls12_final_exp_dsd" out_loc
            ...
            (exists rs1,
               (exists rs0,
                  ...
                  rs1 = rs_set_tower rs0 tmp_var
                          (exist_tval TFp12 (tt_zero bls12_N TFp12)))
               /\ rs2 = rs_set_tower rs1 "gamma1_p2"
                          (exist_tval TFp2 (tt_zero bls12_N TFp2)))
  ...
  (*   [bls12_bind_params] uses (and vice versa for [bls12_extract_    *)
  ...
  (** Link from the high-level [extract_at] to the low-level
      [lookup_t] view used by [bls12_bind_params].  The [eq_rect]
  ...
  (*   [bls12_extract_output] + [bls12_writeback_output] to land in   *)
  (*   [mid1_sa]).                                                    *)
  (* ================================================================ *)

  Lemma compose_load_g1 :
    call_composes_from bls12_N bls12_u64_max bls12_pairing_fenv
                       bls12_leaf_spec_concrete bls12_call_env
                       "bls12_load_gamma1_p2"
                       bls12_load_g1_params bls12_load_g1_body
  ...
  Proof.
    ...
    - (* pre-bridge: pre_sa rs1 => in_vals + rs_init + callee_pre *)
      ...
      + (* bls12_bind_params *)
        unfold bls12_bind_params; cbn.
        unfold bls12_bind_params_aux; cbn.
    ...
    - (* post-bridge: caller_pre + callee_post + extract + writeback => mid1_sa *)
      ...
      (* bls12_extract_output ["out"] rs_mid = Some (existT _ TFp2 v_out) *)
      unfold ce_extract_output, bls12_call_env, bls12_extract_output,
             bls12_load_g1_params in Hext; cbn in Hext.
      rewrite Hout_ext in Hext.
      injection Hext as Hrs_out. subst rs_out_val.
      (* Hwb: bls12_writeback_output g1_loc (existT _ TFp2 v_out) rs1 = Some rs2.
         By the definition of bls12_writeback_output and the fact that
         loc_dst g1_loc = TFp2, this reduces to located_update rs1 g1_loc v_out. *)
      assert (Hwb' : located_update rs1 g1_loc v_out = Some rs2).
      { unfold ce_writeback_output, bls12_call_env,
               bls12_writeback_output in Hwb.
        cbn in Hwb.
  ...
  Lemma compose_load_g2 :
    call_composes_from bls12_N bls12_u64_max bls12_pairing_fenv
                       ...
                       bls12_load_g2_params bls12_load_g2_body
  ...
  Proof.
    ...
    - (* post-bridge *)
      ...
      unfold out_var in Hout_ext.
      unfold ce_extract_output, bls12_call_env, bls12_extract_output,
             bls12_load_g2_params in Hext; cbn in Hext.
      rewrite Hout_ext in Hext.
      injection Hext as Hrs_out. subst rs_out_val.
  ...
  Lemma compose_load_w :
    call_composes_from bls12_N bls12_u64_max bls12_pairing_fenv
                       ...
                       bls12_load_w_params bls12_load_w_body
  ...
  Proof.
    ...
    - (* post-bridge *)
      ...
      unfold out_var in Hout_ext.
      unfold ce_extract_output, bls12_call_env, bls12_extract_output,
             bls12_load_w_params in Hext; cbn in Hext.
      rewrite Hout_ext in Hext.
      injection Hext as Hrs_out. subst rs_out_val.
  ...
  Lemma compose_miller :
    forall px_v py_v qx_v qy_v,
      call_composes_from bls12_N bls12_u64_max bls12_pairing_fenv
                         bls12_leaf_spec_concrete bls12_call_env
                         "bls12_miller_loop_optimal"
                         bls12_miller_params bls12_miller_body
                         (miller_callee_pre px_v py_v qx_v qy_v)
                         (miller_callee_post gamma1 gamma_y gamma1_p2
                                             px_v py_v qx_v qy_v)
                         tmp_loc
  ...
  Proof.
    ...
    - (* pre-bridge *)
      ...
      + (* bls12_bind_params *)
        unfold bls12_bind_params, bls12_bind_params_aux,
               bls12_miller_params; cbn.
    ...
    - (* post-bridge *)
      ...
      unfold ce_extract_output, bls12_call_env, bls12_extract_output,
             bls12_miller_params in Hext; cbn in Hext.
      ...
      + (* mid_ml rs2 *)
        ...
        * (* ml witness *)
          (* Need: extract_at tmp_loc TFp12 rs2 = Some ml_val /\
                   ...
                   bls12_fp12_eval ml_val = miller_with_corrections ... *)
          ...
          -- erewrite extract_at_writeback_other;
               [exact Hqy_rs1
                | rewrite loc_var_tmp_loc; exact Hqy_t
                | exact Hwb'].
          -- (* bls12_fp12_eval v_tmp_out =
                miller_with_corrections(bls12_fp_eval v_px_rs1, ...).
                Chain: Htmp_eval gives this for (px_v, py_v, qx_v, qy_v).
                The caller-provided equations Hpx_eq : bls12_fp_eval v_px_in = px_v
                plus determinism of extract_at (Hpx_ext_in vs Hpx_rs1 both at p_x_loc) give
                bls12_fp_eval v_px_rs1 = px_v. *)
  ...
  Lemma miller_call_refines :
    rust_refines bls12_N bls12_u64_max bls12_pairing_fenv
                 bls12_leaf_spec_concrete bls12_call_env
                 mid3_sa
                 (RCall "bls12_miller_loop_optimal" tmp_loc
                        [tmp_loc; p_x_loc; p_y_loc; q_x_loc; q_y_loc])
                 mid4_sa.
  Proof.
    unfold rust_refines.
    ...
    destruct Hpre_plain as [_ [[_ [v_px Hpx_ext]]
                                [[_ [v_py Hpy_ext]]
                                 [[_ [v_qx Hqx_ext]]
                                  [_ [v_qy Hqy_ext]]]]]].
    set (px_v := bls12_fp_eval v_px).
    set (py_v := bls12_fp_eval v_py).
    set (qx_v := bls12_fp2_eval v_qx).
    set (qy_v := bls12_fp2_eval v_qy).
  ...
  (*                                                                   *)
  (*   For BLS12 (loop_neg = false), the [bls12_optimal_ate_spec]     *)
  ...
  (*   computing [loop_neg bls12_381_params = false] by [vm_compute].     *)
  (*   Keeping it Hypothesis-local lets the chain proof close         *)
  (*   without pulling in the full [CurveParams] unfolding.           *)
  (* ================================================================ *)

  Lemma bls12_optimal_ate_decomposes :
    forall px py qx qy,
      bls12_optimal_ate_spec gamma1 gamma_y gamma1_p2 px py qx qy =
      PairingSpec.final_exp bls12_381_zmod_ops
        (zfp12_conj bls12_p_val)
        (zfp12_inv bls12_p_val bls12_xi_val)
        (zfp12_frob_p2 bls12_p_val bls12_xi_val)
        (zfp12_pow bls12_p_val bls12_xi_val)
        (prime_p bls12_381_params) (scalar_r bls12_381_params)
        (bls12_miller_loop_with_corrections gamma1 gamma_y gamma1_p2
                                            px py qx qy).
  Proof.
    intros.
    unfold bls12_optimal_ate_spec, bls12_miller_loop_with_corrections,
           PairingSpec.optimal_ate.
    (* Both sides match definitionally: BLS12-381 has
       [loop_neg = true] and we've folded the [zfp12_conj] step into
       [bls12_miller_loop_with_corrections], so the optimal_ate's
       [if loop_neg ...] branch aligns with miller_wc's.  After
       destructuring [affine_miller_aux]'s triple, [reflexivity]. *)
    destruct (affine_miller_aux bls12_381_zmod_ops (loop_abs bls12_381_params)
               (Z.to_nat (Z.log2 (loop_abs bls12_381_params)))
               px py qx qy (fp12_one bls12_381_zmod_ops) qx qy)
             as [[f Tx] Ty].
    reflexivity.
  Qed.
  ...
  Lemma compose_finalexp :
    forall f_v,
      call_composes_from bls12_N bls12_u64_max bls12_pairing_fenv
                         bls12_leaf_spec_concrete bls12_call_env
                         "bls12_final_exp_dsd"
                         bls12_finalexp_params bls12_finalexp_body
                         (finalexp_callee_pre f_v)
                         (finalexp_callee_post f_v)
                         out_loc
                         [out_loc; tmp_loc; g1_loc; g2_loc; w_loc]
  ...
  Proof.
    ...
    - (* post-bridge *)
      ...
      unfold out_var in Hout_ext.
      unfold ce_extract_output, bls12_call_env, bls12_extract_output,
             bls12_finalexp_params in Hext; cbn in Hext.
      rewrite Hout_ext in Hext.
      injection Hext as Hrs_out. subst rs_out_val.
      ...
      (* Math chain: tmp_loc in rs1 holds v_f (from Hf_ext_rs1) AND v_ml
         (from Hml_tmp). So v_f = v_ml. Then f_v = bls12_fp12_eval v_ml =
      ...
      + exact Hqy_rs2.
      + (* Math equation *)
        rewrite bls12_optimal_ate_decomposes.
        rewrite Hout_eval.
        f_equal.
  ...
  Theorem bls12_pairing_body_refines :
    rust_refines bls12_N bls12_u64_max bls12_pairing_fenv
                 bls12_leaf_spec_concrete bls12_call_env
  ...
  Theorem bls12_pairing_rust_correct :
    rust_refines bls12_N bls12_u64_max bls12_pairing_fenv
                 bls12_leaf_spec_concrete bls12_call_env
                 pre pairing_body post.
  Proof. exact bls12_pairing_body_refines. Qed.
  ...
  (** End-to-end corollary: for any post-pairing state [rs2] reached
      ...
      This is the Step 5 wiring: it packages [bls12_pairing_rust_correct]
      in the style of [BLS12_EndToEnd.bls12_pairing_end_to_end], yielding
      a direct math equality at the exit. *)
  Theorem bls12_pairing_end_to_end_conditional :
    forall rs1 rs2,
      pre rs1 ->
      rust_exec_fenv bls12_N bls12_u64_max bls12_pairing_fenv
                     bls12_leaf_spec_concrete bls12_call_env
  ...
  Proof.
    intros rs1 rs2 Hpre Hexec.
    exact (bls12_pairing_rust_correct rs1 rs2 Hpre Hexec).
  Qed.

End PairingBodyRefines.

(* ================================================================ *)
(* §8bis.  Concrete BLS12 pairing entry point                        *)
(*                                                                   *)
...
(*   1× [Hloader_input_len4] (structural, discharged when the caller *)
...
Definition bls12_out_loc  : located := loc_Fp12 "pout".
Definition bls12_p_x_loc  : located := loc_Fp   "px".
Definition bls12_p_y_loc  : located := loc_Fp   "py".
Definition bls12_q_x_loc  : located := loc_Fp2  "qx".
Definition bls12_q_y_loc  : located := loc_Fp2  "qy".

(** The 16 architectural hypotheses, each discharged by
    [reflexivity] or [repeat split; discriminate]. *)
Lemma bls12_Hout_Fp12 : loc_dst bls12_out_loc = TFp12. Proof. reflexivity. Qed.
Lemma bls12_Hpx_Fp    : loc_dst bls12_p_x_loc = TFp.   Proof. reflexivity. Qed.
Lemma bls12_Hpy_Fp    : loc_dst bls12_p_y_loc = TFp.   Proof. reflexivity. Qed.
Lemma bls12_Hqx_Fp2   : loc_dst bls12_q_x_loc = TFp2.  Proof. reflexivity. Qed.
Lemma bls12_Hqy_Fp2   : loc_dst bls12_q_y_loc = TFp2.  Proof. reflexivity. Qed.

Lemma bls12_Hfresh_out :
  loc_var bls12_out_loc <> tmp_var /\
  loc_var bls12_out_loc <> "gamma1_p2" /\
  loc_var bls12_out_loc <> "gamma2_p2" /\
  loc_var bls12_out_loc <> "w_frob_p2_c1".
Proof. repeat split; discriminate. Qed.
Lemma bls12_Hfresh_px :
  loc_var bls12_p_x_loc <> tmp_var /\
  loc_var bls12_p_x_loc <> "gamma1_p2" /\
  loc_var bls12_p_x_loc <> "gamma2_p2" /\
  loc_var bls12_p_x_loc <> "w_frob_p2_c1".
Proof. repeat split; discriminate. Qed.
Lemma bls12_Hfresh_py :
  loc_var bls12_p_y_loc <> tmp_var /\
  loc_var bls12_p_y_loc <> "gamma1_p2" /\
  loc_var bls12_p_y_loc <> "gamma2_p2" /\
  loc_var bls12_p_y_loc <> "w_frob_p2_c1".
Proof. repeat split; discriminate. Qed.
Lemma bls12_Hfresh_qx :
  loc_var bls12_q_x_loc <> tmp_var /\
  loc_var bls12_q_x_loc <> "gamma1_p2" /\
  loc_var bls12_q_x_loc <> "gamma2_p2" /\
  loc_var bls12_q_x_loc <> "w_frob_p2_c1".
Proof. repeat split; discriminate. Qed.
Lemma bls12_Hfresh_qy :
  loc_var bls12_q_y_loc <> tmp_var /\
  loc_var bls12_q_y_loc <> "gamma1_p2" /\
  loc_var bls12_q_y_loc <> "gamma2_p2" /\
  loc_var bls12_q_y_loc <> "w_frob_p2_c1".
Proof. repeat split; discriminate. Qed.

Lemma bls12_Hout_shape : bls12_out_loc = loc_Fp12 (loc_var bls12_out_loc).
Proof. reflexivity. Qed.
Lemma bls12_Hpx_shape  : bls12_p_x_loc = loc_Fp   (loc_var bls12_p_x_loc).
Proof. reflexivity. Qed.
Lemma bls12_Hpy_shape  : bls12_p_y_loc = loc_Fp   (loc_var bls12_p_y_loc).
Proof. reflexivity. Qed.
Lemma bls12_Hqx_shape  : bls12_q_x_loc = loc_Fp2  (loc_var bls12_q_x_loc).
Proof. reflexivity. Qed.
Lemma bls12_Hqy_shape  : bls12_q_y_loc = loc_Fp2  (loc_var bls12_q_y_loc).
Proof. reflexivity. Qed.

Lemma bls12_Hdistinct_out_users :
  loc_var bls12_out_loc <> loc_var bls12_p_x_loc /\
  loc_var bls12_out_loc <> loc_var bls12_p_y_loc /\
  loc_var bls12_out_loc <> loc_var bls12_q_x_loc /\
  loc_var bls12_out_loc <> loc_var bls12_q_y_loc.
Proof. repeat split; discriminate. Qed.
...
Theorem bls12_pairing_rust_correct_concrete :
  ...
  rust_refines bls12_N bls12_u64_max bls12_pairing_fenv
               bls12_leaf_spec_concrete bls12_call_env
               (pairing_pre bls12_out_loc bls12_p_x_loc bls12_p_y_loc
                            bls12_q_x_loc bls12_q_y_loc)
               (pairing_body bls12_out_loc bls12_p_x_loc bls12_p_y_loc
                             bls12_q_x_loc bls12_q_y_loc)
               (pairing_post bls12_out_loc bls12_p_x_loc bls12_p_y_loc
                             bls12_q_x_loc bls12_q_y_loc
                             gamma1 gamma_y gamma1_p2).
Proof.
  intros gamma1 gamma_y gamma1_p2 Hg1 Hgy Hgw Hlen6.
  apply (bls12_pairing_rust_correct
           bls12_out_loc bls12_p_x_loc bls12_p_y_loc
           bls12_q_x_loc bls12_q_y_loc
           gamma1 gamma_y gamma1_p2);
    first [ exact bls12_Hout_Fp12
          | exact bls12_Hpx_Fp
          | exact bls12_Hpy_Fp
          | exact bls12_Hqx_Fp2
          | exact bls12_Hqy_Fp2
          | exact bls12_Hfresh_out
          | exact bls12_Hfresh_px
          | exact bls12_Hfresh_py
          | exact bls12_Hfresh_qx
          | exact bls12_Hfresh_qy
          | exact bls12_Hout_shape
          | exact bls12_Hpx_shape
          | exact bls12_Hpy_shape
          | exact bls12_Hqx_shape
          | exact bls12_Hqy_shape
          | exact bls12_Hdistinct_out_users
          | exact Hg1 | exact Hgy | exact Hgw | exact Hlen6 ].
Qed.
...
(** [Print Assumptions bls12_pairing_end_to_end_conditional] reports
    ...
    Phase 1 (composition, DONE):
      ...
      [pairing_calls_refines] and [bls12_pairing_body_refines]
    ...
    [bls12_miller_body] / [bls12_finalexp_body] above for the
    roadmap.  [bls12_optimal_ate_decomposes] (previously a
    Hypothesis) is now Qed by [reflexivity] on
    [loop_neg bls12_381_params = false]. *)
