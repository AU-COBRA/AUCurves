(** * BN256 Curve Operations: point_double, store_zero, point_negate,
    ...
    Provides:
    - [bn256_point_double] : dedicated doubling (dbl-2009-l, a=0)
...
Section BN256_CurveOps.
  ...
  (** ** Point doubling: dedicated dbl-2009-l formula (a=0).
      ...
      to "bn256_square", "bn256_mul", "bn256_add", "bn256_sub" via
      [bn256_field_parameters].
      ...
      from the cmd).  With [bn256_field_parameters] as an Existing Instance,
...
Section BN256_CurveOps_Specs.
  ...
  Lemma bn256_point_double_correct :
    forall functions,
      ...
      map.get functions "curve_double" = Some (snd bn256_point_double) ->
      bn256_point_double_spec functions.
  Proof.
    intros. unfold bn256_point_double_spec.
    eapply point_double_correct; try eassumption.
    { (* loose_bounds = tight_bounds for BN256 Montgomery *)
      reflexivity. }
...
Section BN256_AllCurveOps.
  ...
  Definition bn256_curve_op_funcs : list function_t :=
    [ bn256_G1_add;            (* ladderstep: "curve_add" *)
      bn256_point_double;      (* dedicated doubling: "curve_double" *)
