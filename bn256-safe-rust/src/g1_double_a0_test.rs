...
//! with `3b` recomputed here from `bn256_from_word`.  The two are
...
use crate::tower::{bn256_add, bn256_from_word, bn256_mul, bn256_sub, Fp};

const LIMBS: usize = 4;

fn three_b() -> Fp {
    let mut o = Fp([0u64; LIMBS]);
    bn256_from_word(&mut o, 9u64);
    o
}
...
fn alg9_reference(x: &Fp, y: &Fp, z: &Fp, b3: &Fp) -> [Fp; 3] {
    ...
    let mut t0 = Fp([0u64; LIMBS]); bn256_mul(&mut t0, y, y);     // 1
    let mut z3 = Fp([0u64; LIMBS]); bn256_add(&mut z3, &t0, &t0); // 2
    bn256_add(&mut u, &z3, &z3); z3 = u;                          // 3
    bn256_add(&mut u, &z3, &z3); z3 = u;                          // 4
    let mut t1 = Fp([0u64; LIMBS]); bn256_mul(&mut t1, y, z);     // 5
    let mut t2 = Fp([0u64; LIMBS]); bn256_mul(&mut t2, z, z);     // 6
    bn256_mul(&mut u, b3, &t2); t2 = u;                           // 7
    let mut x3 = Fp([0u64; LIMBS]); bn256_mul(&mut x3, &t2, &z3); // 8
    let mut y3 = Fp([0u64; LIMBS]); bn256_add(&mut y3, &t0, &t2); // 9
    bn256_mul(&mut u, &t1, &z3); z3 = u;                          // 10
    bn256_add(&mut u, &t2, &t2); t1 = u;                          // 11
    bn256_add(&mut u, &t1, &t2); t2 = u;                          // 12
    bn256_sub(&mut u, &t0, &t2); t0 = u;                          // 13
    bn256_mul(&mut u, &t0, &y3); y3 = u;                          // 14
    bn256_add(&mut u, &x3, &y3); y3 = u;                          // 15
    bn256_mul(&mut u, x, y); t1 = u;                              // 16
    bn256_mul(&mut u, &t0, &t1); x3 = u;                          // 17
    bn256_add(&mut u, &x3, &x3); x3 = u;                          // 18
    [x3, y3, z3]
}
...
impl Rng {
    fn next_fp(&mut self) -> Fp {
        ...
        bn256_from_word(&mut a, self.0 | 1);
        self.0 = self.0.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
        let mut b = Fp([0u64; LIMBS]);
        bn256_from_word(&mut b, self.0 | 1);
...
fn emitted_curve_constant_is_three_b() {
    ...
    let mut sq = Fp([0u64; LIMBS]); bn256_mul(&mut sq, &b3, &b3);   // 9b^2
    let mut t = Fp([0u64; LIMBS]); bn256_add(&mut t, &sq, &sq);      // 18b^2
    let mut s = Fp([0u64; LIMBS]); bn256_add(&mut s, &t, &sq);       // 27b^2
    let mut neg = Fp([0u64; LIMBS]); bn256_sub(&mut neg, &zero, &s); // -27b^2
