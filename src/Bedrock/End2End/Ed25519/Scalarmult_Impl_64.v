...
Section ScalarmultImpl64.
  ...
  (* ============================================================================
     ...
     (measured 2026-05-08 16:19, INNER_EXIT=124).

     2026-05-08 17:22 follow-up: ALSO collapsed the dealloc cascade
     into [DeallocCascadeHelper.dealloc_cascade_full] (single
     Qed-sealed Lemma absorbing 22 lines of manual existential
     threading at the post-parametric call).  R10 .vo dropped from
     77953 to 77433 (520 bytes); the eapply call site is one
     [eapply ...; [ exact ... | ... | exact Hsep_par_b ]].  Even with
     this collapse, [Time Qed] still INNER_EXIT=124 at 30 min.

     **Conclusion (definitive after 3 measurements)**: the kernel-check
     bottleneck is NOT in any of the extracted helper layers we landed.
     The remaining cost lives in bedrock2's [repeat straightline] glue
     between calls (lines 671/689/706) and/or the [Proper_call] +
     [eapply Hcall] proof terms generated by [vm_call_compat]'s
     spec-instantiation step.  Both are bedrock2-supplied tactics
     producing terms that don't factor into Qed-sealed Lemmas without
     deep modifications to bedrock2 itself.

     Only path forward (multi-week, out of session scope):
     - Reflective normalizer for [straightline] / [straightline_call]
       per the Rtac/Malecha approach (ESOP 2016).  Define a deep AST
       for cmd sequences, prove a soundness theorem, discharge each
       advance via [vm_compute]+[reflexivity].  Per
       reference_slow_proofs_fiat.md: ~88x speedup on similar problems.
       The existing infrastructure (SepDeep, SepReflectiveAC,
       SepCallReflect.vm_call) is partial groundwork; the reflective
       cmd-AST is the missing piece.
