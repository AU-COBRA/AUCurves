(** * BN254 (alt_bn128) Fp inversion via Bernstein-Yang divstep algorithm.

    Mirrors BLS12_FpInv.v for the BN254 (alt_bn128) base field.

    BN254 base prime:
      p = 36 u^4 + 36 u^3 + 24 u^2 + 6 u + 1,  u = 0x44E992B44A6909F1
        = 21888242871839275222246405745257275088696311157297823662689037894645226208583
      Z.log2 p + 1 = 254
      iterations = (49 * 254 + 57) / 17 = 12503 / 17 = 735
...
(* ================================================================== *)
(* BN254 prime and its parameters                                      *)
(* ================================================================== *)

(** The BN254 base field prime. *)
Definition bn254_p : Z :=
  Eval vm_compute in bn254_modulus.
...
Definition bn254_mbits : Z := Z.log2 bn254_p + 1.

Lemma bn254_mbits_val : bn254_mbits = 254.
Proof. vm_compute. reflexivity. Qed.

(** BN254 has >= 46 bits, so we use the large-prime formula. *)
Lemma bn254_mbits_ge_46 : 46 <= bn254_mbits.
...
(** The number of divstep iterations for BN254 inversion. *)
Definition bn254_divstep_iters : Z :=
  Eval vm_compute in ((49 * bn254_mbits + 57) / 17).

Lemma bn254_divstep_iters_val : bn254_divstep_iters = 735.
Proof. vm_compute. reflexivity. Qed.

(** This equals the formula from divstep_precomp_correct when mbits >= 46. *)
Lemma bn254_divstep_iters_eq_precomp_exp :
  bn254_divstep_iters =
  (if Decidable.dec (bn254_mbits < 46) then (49 * bn254_mbits + 80) / 17
   else (49 * bn254_mbits + 57) / 17).
Proof.
...
Section FpInv.
  ...
  (** Precomputation: ((p+1)/2)^N mod p with N = 735.
      Computed externally via:
        python3 -c "p=0x30644e72e131a029b85045b68181585d97816a916871ca8d3c208c16d87cfd47; print(hex(pow((p+1)//2, 735, p)))"
  *)
  Definition precomp_val : Z :=
    0x1cf9a092881bfbf3b94b2da656c8e8c2fe4b1372aedf89773a36fbc7b61841b0.

  Definition fp_inv_spec (x : Z) : Z :=
    let '(d, f, g, v, r) :=
      iter_divstep_spec p (Z.to_nat bn254_divstep_iters) 1 p x 0 1 in
  ...
  (** Bernstein-Yang convergence axiom (CHES 2019, Theorem 11.2),
      specialized to BN254. *)
  Axiom by_convergence_dfg_bn254 : forall x,
    0 < x < p ->
    Z.gcd x p = 1 ->
    let '(_, f_N, g_N) :=
      iter_divstep_dfg (Z.to_nat bn254_divstep_iters) 1 p x in
    g_N = 0 /\ (f_N = 1 \/ f_N = -1).
  ...
  Lemma precomp_times_pow2 :
    (precomp_val * 2 ^ bn254_divstep_iters) mod p = 1.
  Proof.
    subst p. unfold precomp_val, bn254_divstep_iters, bn254_p.
    vm_compute. reflexivity.
  Qed.

  (** Any 0 < x < p is coprime to p since p is prime. *)
  Lemma bn254_coprime : forall x, 0 < x < p -> Z.gcd x p = 1.
  Proof.
    intros x Hx.
    apply Zgcd_1_rel_prime.
    apply rel_prime_sym.
    apply prime_rel_prime.
    - subst p. unfold bn254_p. exact prime_bn254.
    - intros [k Hk]. subst x.
  ...
  Proof.
    intros x Hx Hgcd.
    unfold fp_inv_spec.
    destruct (iter_divstep_spec p (Z.to_nat bn254_divstep_iters) 1 p x 0 1)
      as [[[[d_N f_N] g_N] v_N] r_N] eqn:Hiter.
    pose proof (iter_invariant p (Z.to_nat bn254_divstep_iters) 1 p x 0 1 x
                  p_pos p_odd) as Hinv.
    ...
    pose proof (by_convergence_dfg_bn254 x Hx Hgcd) as Hconv.
    pose proof (iter_dfg_agree (Z.to_nat bn254_divstep_iters) p 1 p x 0 1) as Hagree.
    rewrite Hiter in Hagree.
    destruct (iter_divstep_dfg (Z.to_nat bn254_divstep_iters) 1 p x)
    ...
    - replace (1 <? 0) with false by reflexivity.
      ...
      assert (Hmod : (v_N * x) mod p = (2 ^ Z.of_nat (Z.to_nat bn254_divstep_iters)) mod p). {
        assert (Hp : p <> 0) by (pose proof p_pos; lia).
        apply Z.mod_divide in Hv_inv; [|exact Hp].
        destruct Hv_inv as [k Hk].
        assert (v_N * x = 2 ^ Z.of_nat (Z.to_nat bn254_divstep_iters) + k * p) by lia.
        rewrite H.
        rewrite Zplus_mod, Z_mod_mult, Z.add_0_r, Z.mod_mod by lia.
        reflexivity.
      }
      rewrite <- Zmult_mod_idemp_r, Hmod, Zmult_mod_idemp_r.
      rewrite Z2Nat.id by (unfold bn254_divstep_iters; vm_compute; discriminate).
      exact precomp_times_pow2.
    - replace (-1 <? 0) with true by reflexivity.
      replace (2 ^ Z.of_nat (Z.to_nat bn254_divstep_iters) * -1)
        with (- (2 ^ Z.of_nat (Z.to_nat bn254_divstep_iters))) in Hv_inv by ring.
      replace (v_N * x - - 2 ^ Z.of_nat (Z.to_nat bn254_divstep_iters))
        with (v_N * x + 2 ^ Z.of_nat (Z.to_nat bn254_divstep_iters)) in Hv_inv by ring.
      set (iters := Z.of_nat (Z.to_nat bn254_divstep_iters)) in *.
  ...
  Theorem fp_inv_correct : forall x,
    ...
    (fp_inv_spec x * x) mod p = 1.
  Proof.
    intros x Hx. apply fp_inv_correct_ax; auto. apply bn254_coprime; auto.
  Qed.

End FpInv.

(* ================================================================== *)
(* Concrete parameters for BN254 divstep                               *)
(* ================================================================== *)
...
Lemma bn254_p_pos : 1 < bn254_p.
Proof. vm_compute. reflexivity. Qed.

Lemma bn254_p_fits : bn254_p < (2^machine_wordsize) ^ Z.of_nat n.
Proof. vm_compute. reflexivity. Qed.

Lemma bn254_p_odd : Z.odd bn254_p = true.
Proof. vm_compute. reflexivity. Qed.
