(** * Fast processDivstep using Sint63 machine integers.

    D = (mantissa, exponent) where both are int (signed 63-bit).
    All arithmetic uses hardware operations via Rocq's Uint63/Sint63.
    Comparison uses Sint63.ltb (signed less-than).
    DDSet = sorted list with merge sort.
    Hull applied per-transform before union. *)
...
Definition D := (int * int)%type.  (* (mantissa, exponent) *)
...
(** Normalization: strip trailing zero bits *)
...
(** Signed comparison of int *)
Definition int_sleb (a b : int) : bool := Sint63.leb a b.
Definition int_sltb (a b : int) : bool := Sint63.ltb a b.

(** Alignment: shift smaller-exponent value to match larger *)
(** Alignment: shift mantissa to match exponents.
    The shift amount (exponent difference) is always non-negative and small (< 48).
    We compute it via Z to avoid Uint63 wrapping issues. *)
Definition Dalign (a b : D) : int * int :=
  let ea := D_exp a in let eb := D_exp b in
  let ma := D_mant a in let mb := D_mant b in
  if int_sltb ea eb then
    let shift := sub eb ea in  (* eb > ea, so this is positive *)
    (ma, lsl mb shift)
  else if int_sltb eb ea then
    let shift := sub ea eb in  (* ea > eb, so this is positive *)
    (lsl ma shift, mb)
  else (ma, mb).
...
Definition Dadd (a b : D) : D :=
  ...
  let ma := D_mant a in let mb := D_mant b in
  if int_sltb ea eb then
    Dred_aux 48 (add ma (lsl mb (sub eb ea))) ea
  else if int_sltb eb ea then
    Dred_aux 48 (add (lsl ma (sub ea eb)) mb) eb
...
Definition Dsub (a b : D) : D :=
  ...
  let ma := D_mant a in let mb := D_mant b in
  if int_sltb ea eb then
    Dred_aux 48 (sub ma (lsl mb (sub eb ea))) ea
  else if int_sltb eb ea then
    Dred_aux 48 (sub (lsl ma (sub ea eb)) mb) eb
...
(** *** Narrow — uses Z for M since M is 381 bits *)

(** narrow: check |M * g| < 1 for all g-coordinates.
    M * g = M * mantissa * 2^exponent.
    Since exponent is typically negative (e.g. -500), we check:
      |M * mantissa| < 2^(-exponent)
    i.e. -2^(-exp) < M * mantissa < 2^(-exp) *)
Definition narrow_check (M : Z) (m_val e_val : Z) : bool :=
  if Z.geb e_val 0 then
    let v := (M * m_val * Z.shiftl 1 e_val)%Z in
    andb (Z.ltb (-1) v) (Z.ltb v 1)
  else
    let bound := Z.shiftl 1 (Z.opp e_val) in
    andb (Z.ltb (Z.opp bound) (M * m_val)) (Z.ltb (M * m_val) bound).

Definition narrow (M : Z) (s : list DD) : bool :=
  match sl_min s, sl_max s with
  | Some p1, Some p2 =>
    andb (narrow_check M (Sint63.to_Z (D_mant (fst p1))) (Sint63.to_Z (D_exp (fst p1))))
         (narrow_check M (Sint63.to_Z (D_mant (fst p2))) (Sint63.to_Z (D_exp (fst p2))))
  | _, _ => true
  end.
...
(** *** processDivstep — hull before union (optimization 3) *)
