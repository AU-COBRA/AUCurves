(** Pallas G1 addition -- bedrock2 WP proof (new fiat-crypto API).

    Mirrors [BLS12Curve_G1.v] / [VestaCurve_G1_bedrock.v] but uses the
    new [Bedrock.Field.Synthesis.Examples.pallas_prime] synthesis
    (instead of the deleted Generic/Specialized API).

    The [Pallas_add] bedrock2 function body is UNCHANGED.  Only the
...
(*Parameters for the Pallas curve.*)
...
Existing Instances pallas_prime.pallas_field_parameters.
Existing Instances pallas_prime.pallas_field_parameters_ok.
Existing Instances pallas_prime.pallas_frep.
Existing Instances pallas_prime.pallas_frep_ok.
Existing Instances pallas_prime.pallas_ops.

(*  We instantiate specs of all imported bedrock2 functions using the
    Bignum-style bridge specs from PallasCurve_G1_WiredSpecs.  These
    expose [pallas_mul]/[pallas_add]/[pallas_sub] under the right names
    and give montmul/montadd/montsub postconditions after conversion. *)

Instance spec_of_reified_mul :
  spec_of (append prefix "mul") := spec_of_pallas_mul_bignum.

Instance spec_of_reified_add :
  spec_of (append prefix "add") := spec_of_pallas_add_bignum.

Instance spec_of_reified_sub :
  spec_of (append prefix "sub") := spec_of_pallas_sub_bignum.
...
Definition Pallas_add : Syntax.func := Eval cbv in (
    let outx := "outx" in
    let outy := "outy" in
    let outz := "outz" in
...
Instance spec_of_Pallas_add: spec_of "Pallas_add" :=
fun functions =>
    ...
    WeakestPrecondition.call functions ( "Pallas_add") t m0
    ...
    (exists (woutx wouty woutz : list Interface.word.rep) Rout,
        (
                (Pallas_add_Gallina_spec (toZ wX1) (toZ wY1) (toZ wZ1) (toZ wX2) (toZ wY2)
...
Add Ring Mp : (MontgomeryRingTheory.mont_enc_ring PallasCurve_G1_bedrock.m bw n r' m' r'_correct m'_correct bw_big n_nz m_small m_big).
...
Lemma montadd_to_Mp x y z (Hx : valid' m bw n x) (Hy : valid' m bw n y) (Hz : valid' m bw n z) :
  montadd z x y -> (enc_mont PallasCurve_G1_bedrock.m bw n z Hz)
    = mont_add (enc_mont PallasCurve_G1_bedrock.m bw n x Hx) (enc_mont PallasCurve_G1_bedrock.m bw n y Hy).
Proof.
...
montsub z x y -> (enc_mont PallasCurve_G1_bedrock.m bw n z Hz)
  = mont_sub (enc_mont PallasCurve_G1_bedrock.m bw n x Hx) (enc_mont PallasCurve_G1_bedrock.m bw n y Hy).
Proof.
...
montmul z x y -> (enc_mont PallasCurve_G1_bedrock.m bw n z Hz)
  = mont_mul (enc_mont PallasCurve_G1_bedrock.m bw n x Hx) (enc_mont PallasCurve_G1_bedrock.m bw n y Hy).
Proof.
...
assert (H : MontgomeryRingTheory.valid' PallasCurve_G1_bedrock.m bw n (toZ x)) by (apply H'; assumption).
...
(** Tactic to convert WiredSpecs postcondition format to
    ...
    WiredSpecs gives:
      eval (from_mont a) mod m = ((eval (from_mont b) mod m) OP (eval (from_mont c) mod m)) mod m
    We need:
      eval (from_mont a) mod m = (eval (from_mont b) OP eval (from_mont c)) mod m
    These are equal by [Z.mul_mod] / [Z.add_mod] / [Z.sub_mod]. *)
...
(*For function calls -- updated for WiredSpecs' 6-precondition format.
  WiredSpecs specs have:
    1) valid wsx   2) valid wsy   3) length old_out = n
    4) (Bignum n px wsx * Rx)%sep  5) (Bignum n py wsy * Ry)%sep
    6) (Bignum n pout old_out * Rout)%sep
...
Ltac next_call :=
  lazymatch goal with
    | [H' : (_ * _)%sep _ |- _] =>
      ...
      | ecancel_assumption (* FElem/Bignum px wsx * Rx *)
      | ecancel_assumption (* FElem/Bignum py wsy * Ry *)
      | ecancel_assumption (* FElem/Bignum pout old_out * Rout *)
      | ];
...
Theorem Pallas_add_func_ok : program_logic_goal_for_function! Pallas_add.
Proof.
  ...
  unfold Pallas_add_Gallina_spec.
  ...
  destruct (MontgomeryCurveG1Equiv.BLS12_add_specs_equiv' m bw n r' m' (PallasCurve_G1_bedrock.a) three_b a_small three_b_small r'_correct m'_correct bw_big n_nz m_small m_big _ _ _ _ _ _ _ _ _ Hvalid Hvalid0 Hvalid1 Hvalid2 Hvalid3 Hvalid4 Hvalid5 Hvalid6 Hvalid7) as [Heq _].
  apply Heq; clear Heq.
  ...
  remember (MontgomeryCurveSpecs.three_b_mont PallasCurve_G1_bedrock.m bw n r' m'
