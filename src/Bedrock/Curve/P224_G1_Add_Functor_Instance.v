(** * P224_G1_Add_Functor_Instance — P-224 instantiation of the
    ...
    the functor's, applied at the P-224 parameters (m = 2^224 - 2^96
    + 1, n = 4, 32 bytes, callee prefix "p224_coord_", function name
    "P224_G1_add_f", constants from [Bedrock.Curve.P224Curve_G1]).
    The WP proof replays the debugged P-256 pathfinder script; the
    store/fold machinery is in [Bedrock.Util.BignumStoreFold].
    ...
    [P224f_G1_add_func_ok] is [RcbGeneralAChain.rcb_general_a_chain_Z]
    (Qed) applied by [eapply] plus fixed-input [eassumption] rounds,
    as at P-256.  Build status (2026-08-29): this tail has not yet been
    executed at P-224; the file's final [Qed] is unobserved.  Depends
    on [P224Curve_G1.vo] (built 2026-08-28). *)
...
Section P224_G1_Add_Functor_Instance.
  ...
  (* m = 2^224 - 2^96 + 1 (concrete literal). *)
  Local Notation m :=
    26959946667150639794667015087019630673557916260026308143510066298881%Z.
  Local Notation n := 4%nat.
  Local Notation bw := 64.
  (* a_val = (-3) mod m *)
  Local Notation a_val :=
    26959946667150639794667015087019630673557916260026308143510066298878%Z.
  (* three_b_val = 3*b mod m *)
  Local Notation three_b_val :=
    2954965522398544411891975459442517899398210385985346940341571419930%Z.
  (* m' = modinv(-m, 2^64) = 2^64 - 1. *)
  ...
  (* Montgomery-encoded constant lists (concrete, from P224Curve_G1). *)
  Definition P224f_three_b_mont : list Z := P224Curve_G1.p224_three_b_mont.
  Definition P224f_a_mont : list Z := P224Curve_G1.p224_a_mont_list.

  (** The bedrock2 function: the functor body at the P-224 parameters. *)
  Definition P224f_G1_add : Syntax.func :=
    Eval vm_compute in
      g1_add_func 32 "p224_coord_"
        P224Curve_G1.p224_a_mont_list P224Curve_G1.p224_three_b_mont.

  (* ============================================================== *)
  (* Spec instances: the functor's, at the P-224 parameters          *)
  (* ============================================================== *)

  Local Instance spec_of_P224f_G1_add : spec_of "P224_G1_add_f" :=
    spec_of_g1_add m n "P224_G1_add_f" a_val three_b_val m'.

  Local Instance spec_of_p224f_coord_mul : spec_of "p224_coord_mul" :=
    spec_of_coord_mul m n "p224_coord_" m'.

  Local Instance spec_of_p224f_coord_add : spec_of "p224_coord_add" :=
    spec_of_coord_add m n "p224_coord_" m'.

  Local Instance spec_of_p224f_coord_sub : spec_of "p224_coord_sub" :=
    spec_of_coord_sub m n "p224_coord_" m'.
  ...
  Local Lemma p224f_three_b_mont_valid : valid P224f_three_b_mont.
  Proof. exact P224Curve_G1.p224_three_b_mont_valid. Qed.

  Local Lemma p224f_a_mont_valid : valid P224f_a_mont.
  Proof.
    cbv [P224f_a_mont P224Curve_G1.p224_a_mont_list].
  ...
  Local Lemma valid_toZ_wordofZ_three_b_mont :
    valid (toZ (List.map wordof_Z P224f_three_b_mont)).
  Proof.
    cbv [P224f_three_b_mont P224Curve_G1.p224_three_b_mont
         WordByWordMontgomery.valid toZ List.map].
    vm_compute. repeat split; auto; intros; discriminate.
  Qed.

  Local Lemma valid_toZ_wordofZ_a_mont :
    valid (toZ (List.map wordof_Z P224f_a_mont)).
  Proof.
    cbv [P224f_a_mont P224Curve_G1.p224_a_mont_list toZ].
    vm_compute. repeat split; auto; intros; discriminate.
  Qed.
  ...
  Lemma three_b_mont_rewrite
        (H : valid' (toZ (map wordof_Z P224f_three_b_mont))) :
    ((MontgomeryCurveG1Equiv.three_b_mont m bw n r' m' three_b_val
       three_b_small r'_correct m'_correct bw_big n_nz m_small m_big)
     = {| val := toZ (map wordof_Z P224f_three_b_mont); Hvalid := H |}).
  Proof.
    apply mont_enc_irr. rewrite !mont_enc_val.
    rewrite (toZ_ofZ_eq P224f_three_b_mont p224f_three_b_mont_valid).
    cbv [MontgomeryCurveG1Equiv.three_b_mont]. rewrite mont_enc_val.
    vm_compute. reflexivity.
  Qed.

  Lemma a_mont_rewrite (H : valid' (toZ (map wordof_Z P224f_a_mont))) :
    ((MontgomeryCurveG1Equiv.a_mont m bw n r' m' a_val a_small
       r'_correct m'_correct bw_big n_nz m_small m_big)
     = {| val := toZ (map wordof_Z P224f_a_mont); Hvalid := H |}).
  Proof.
    apply mont_enc_irr. rewrite !mont_enc_val.
    rewrite (toZ_ofZ_eq P224f_a_mont p224f_a_mont_valid).
  ...
  Theorem P224f_G1_add_func_ok :
    forall (functions : Semantics.env),
      map.get functions "P224_G1_add_f" = Some P224f_G1_add ->
      spec_of_p224f_coord_mul functions ->
      spec_of_p224f_coord_add functions ->
      spec_of_p224f_coord_sub functions ->
      spec_of_P224f_G1_add functions.
  Proof.
    intros functions EnvContains Hspec_mul Hspec_add Hspec_sub.
    cbv [spec_of_P224f_G1_add spec_of_g1_add]. intros.
    eapply WeakestPreconditionProperties.start_func;
      [ exact EnvContains | clear EnvContains ].
    cbv match beta delta [WeakestPrecondition.func P224f_G1_add].
    eexists. split.
    { reflexivity. }
    ...
    all: timeout 1800 (first [ reflexivity
                            | (cbv [P224f_a_mont P224f_three_b_mont
                                    P224Curve_G1.p224_a_mont_list
                                    P224Curve_G1.p224_three_b_mont List.map];
                               reflexivity) ]).
  Qed.
