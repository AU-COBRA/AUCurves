(** * BLS24-509 G1 group operation: ladderstep (combined doubling + addition).

    The function body is the generic [ladderstep_body] from CurveAdd.v
    instantiated for BLS24-509 (3b = 3). The WP correctness lemma states
...
Section bls24_509_G1.
    ...
    (* BLS24-509 curve: y^2 = x^3 + 1, so b = 1 and 3b = 3 *)
    Definition bls24_509_G1_add : function_t :=
      ("curve_add", ladderstep_body "bls24_509_three_b").

    Definition three_b_F : F := ModularArithmetic.F.of_Z M_pos 3.
    ...
    (* The G1 ladderstep correctness: this function satisfies
       ...
       here (rather than an [exact I : True] stub) so that the
    ...
    (* The actual spec parameterized over a bounded three_b witness.
       ...
       which is provided by bls24_509_three_b.three_b_mont_valid. *)
    Definition bls24_509_G1_spec_statement
      (three_b : Crypto.Bedrock.Specs.Field.felem)
      (functions : Semantics.env) : Prop :=
      @CurveAdd.spec_of_ladderstep _ _ _ _ _ _
        bls24_509_field_parameters bls24_509_frep three_b functions.
