(* secp256k1 curve integration into AUCurves Montgomery framework.
   Instantiates MontgomeryCurveSpecs for y^2 = x^3 + 7 over the
   secp256k1 prime field (m = 2^256 - 2^32 - 977). *)
...
(* Instantiate Montgomery curve specs for secp256k1 *)
Definition secp256k1_three_b_list := MontgomeryCurveSpecs.three_b_list bw n three_b.
Definition secp256k1_three_b_mont := Eval vm_compute in
  (MontgomeryCurveSpecs.three_b_mont_list m bw n m' three_b).
Definition secp256k1_a_list := MontgomeryCurveSpecs.a_list bw n a.
Definition secp256k1_a_mont_list := Eval vm_compute in
  (MontgomeryCurveSpecs.a_mont_list m bw n m' a).

(* Validity lemmas *)
Lemma secp256k1_three_b_list_valid : WordByWordMontgomery.valid bw n m secp256k1_three_b_list.
Proof.
...
(* Gallina specification of secp256k1 point addition (projective coordinates).
   This is the generic spec from MontgomeryCurveSpecs, instantiated for secp256k1. *)
Definition Secp256k1_add_Gallina_spec :=
...
Definition Secp256k1_add_Gallina_spec_a_0 X1 Y1 Z1 X2 Y2 Z2 outx outy outz :=
  let X1 := evfrom X1 in
  let Y1 := evfrom Y1 in
  let Z1 := evfrom Z1 in
  let X2 := evfrom X2 in
  ...
  let X3 := t1+'t2 in
  let t5 := t5-'X3 in
  let Z3 := eval secp256k1_three_b_list *'t2 in
  let X3 := t1-'Z3 in
  let Z3 := t1+'Z3 in
  let Y3 := X3*'Z3 in
  let t1 := t0+'t0 in
  let t1 := t1+'t0 in
  let t4 := eval secp256k1_three_b_list *'t4 in
  let t0 := t1*'t4 in
  let Y3 := Y3+'t0 in
  let t0 := t5*'t4 in
  let X3 := t3*'X3 in
...
(* The generic and a=0 specialized specs are equivalent for secp256k1. *)
Lemma add_specs_equiv : forall X1 X2 Y1 Y2 Z1 Z2 outx outy outz,
  Secp256k1_add_Gallina_spec X1 X2 Y1 Y2 Z1 Z2 outx outy outz
  <-> Secp256k1_add_Gallina_spec_a_0 X1 X2 Y1 Y2 Z1 Z2 outx outy outz.
Proof.
