//! Build script for the P-256 safe-Rust crate.
//!
//! Optionally links the two CryptOpt-superoptimized field leaves in
//! `generated/` and sets `cfg(p256_cryptopt_asm)`, which makes `fp_mul` and
...
//! `P256_NO_CRYPTOPT=1` in the environment to force the fallback.
...
fn main() {
    ...
    println!("cargo:rerun-if-changed=generated/p256_mul_cryptopt.asm");
    println!("cargo:rerun-if-changed=generated/p256_square_cryptopt.asm");
    println!("cargo:rerun-if-env-changed=P256_NO_CRYPTOPT");
    println!("cargo:rustc-check-cfg=cfg(p256_cryptopt_asm)");

    if env::var_os("P256_NO_CRYPTOPT").is_some() {
        println!("cargo:warning=P256_NO_CRYPTOPT set; using fiat-rust field leaves");
        return;
    }
    ...
    if !(std::is_x86_feature_detected!("bmi2") && std::is_x86_feature_detected!("adx")) {
        println!(
            "cargo:warning=host lacks BMI2/ADX; using fiat-rust field leaves for P-256"
        );
        return;
    }
    ...
    match status {
        Ok(s) if s.success() => {
            ...
            println!("cargo:rustc-link-lib=static=p256_cryptopt");
            println!("cargo:rustc-cfg=p256_cryptopt_asm");
        }
        _ => {
            println!(
                "cargo:warning=build.sh failed (nasm missing?); \
                 using fiat-rust field leaves for P-256"
            );
        }
    }
}
