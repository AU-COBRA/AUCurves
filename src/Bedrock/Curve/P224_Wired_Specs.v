(** Wired Bignum-style spec instances for P-224 field operations.

    P-224 analogue of [P256_Wired_Specs.v]. Uses the
    [p224_frep] field representation from
    [Bedrock.Field.Synthesis.Examples.p224_field] and exposes
    the synthesized [p224_coord_mul], [p224_coord_add], etc. as
...
Require Import Bedrock.Curve.P224_Bignum_Specs.
...
Local Notation p224_m' := (@Field.m' bw p224_field_parameters).
Notation eval := (@WordByWordMontgomery.WordByWordMontgomery.eval bw n).
Notation from_mont := (@WordByWordMontgomery.from_montgomerymod bw n m p224_m').
Local Notation toZ := (List.map Interface.word.unsigned).

(** Sanity checks: the Montgomery constants used downstream.
    m' = -m^-1 mod 2^64 = 2^64 - 1; r' = (2^64)^-1 mod m. *)
Lemma p224_m'_sanity : (m * 18446744073709551615) mod 2^64 = (-1) mod 2^64.
Proof. vm_compute. reflexivity. Qed.

Lemma p224_r'_sanity :
  (2^64 * 26959946667150639793205513449688727755354231427310025123858428723201) mod m = 1.
Proof. vm_compute. reflexivity. Qed.

(** ** Concrete spec_of instances *)

Instance spec_of_p224_mul_bignum : spec_of "p224_coord_mul" :=
  fun functions =>
...
Instance spec_of_p224_add_bignum : spec_of "p224_coord_add" :=
  fun functions =>
    forall (wsx wsy old_out : list word.rep)
           (px py pout : word.rep)
           (tr : trace) (mem0 : @map.rep _ _ BasicC64Semantics.mem)
...
Instance spec_of_p224_sub_bignum : spec_of "p224_coord_sub" :=
  fun functions =>
    forall (wsx wsy old_out : list word.rep)
           (px py pout : word.rep)
           (tr : trace) (mem0 : @map.rep _ _ BasicC64Semantics.mem)
...
Instance spec_of_p224_square_bignum : spec_of "p224_coord_square" :=
  fun functions =>
...
Instance spec_of_p224_opp_bignum : spec_of "p224_coord_opp" :=
  fun functions =>
    forall (wsx old_out : list word.rep)
           (px pout : word.rep)
           (tr : trace) (mem0 : @map.rep _ _ BasicC64Semantics.mem)
...
Lemma p224_mul_bignum_correct :
  forall functions,
    spec_of_BinOp bin_mul (field_representation:=p224_frep) functions ->
    spec_of_p224_mul_bignum functions.
Proof.
...
Lemma p224_add_bignum_correct :
  forall functions,
    spec_of_BinOp bin_add (field_representation:=p224_frep) functions ->
    spec_of_p224_add_bignum functions.
Proof.
  intros functions Hfelem.
  intros wsx wsy old_out px py pout tr m0 Rx Ry Rout.
  intros Hvalx Hvaly Hlenout Hsepx Hsepy Hsepout.
...
Lemma p224_sub_bignum_correct :
  forall functions,
    spec_of_BinOp bin_sub (field_representation:=p224_frep) functions ->
    spec_of_p224_sub_bignum functions.
Proof.
  intros functions Hfelem.
  intros wsx wsy old_out px py pout tr m0 Rx Ry Rout.
  intros Hvalx Hvaly Hlenout Hsepx Hsepy Hsepout.
...
Lemma p224_square_bignum_correct :
  forall functions,
    spec_of_UnOp un_square (field_representation:=p224_frep) functions ->
    spec_of_p224_square_bignum functions.
Proof.
  intros functions Hfelem.
...
Lemma p224_opp_bignum_correct :
  forall functions,
    spec_of_UnOp un_opp (field_representation:=p224_frep) functions ->
    spec_of_p224_opp_bignum functions.
Proof.
  intros functions Hfelem.
  intros wsx old_out px pout tr m0 Rx Rout.
  intros Hvalx Hlenout Hsepx Hsepout.
