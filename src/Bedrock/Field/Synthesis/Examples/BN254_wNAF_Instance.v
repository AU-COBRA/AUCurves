(** * BN254 single-scalar wNAF — instance discharge + composition.

    Discharges the algebraic hypotheses (Horner step, digit-point
    correctness, weighted-sum non-negativity) for BN254 and composes
    the full WP chain:

      single_load_and_process_ok  →  wnaf_single_loop_body_ok  →  wnaf_single_ok

    The result is a single theorem:
      Given BN254 curve_add/double specs and the digit/table data,
...
Section BN254_wNAF_Instance.
  ...
  (** Horner step: connects digit_point-based result to weighted_sum
      formula, up to [pt_eq]:
      ...
      Discharged by [wNAF_Single_HornerAlgebra.horner_step_single]. *)
  ...
  (** On-curve closure of one Horner step.  NEW, forced by G6:
      ...
      [Hhorner_step].  Discharged from
      [wNAF_Single_HornerAlgebra.digit_point_local_oncurve] together with
      [oncurve_curve_add]. *)
  ...
  (** ================================================================
      Full single-scalar wNAF theorem for BN254 (or any non-GLV curve).
  ...
  Proof.
    ...
    - (* HLoopBody *)
      ...
      (* Regroup sep associativity in the goal's postcondition so the
         ...
         NOTE (positional application): the leading arguments are the
         Section variables of [SingleLoopBodyProof] that survive Section
         discharge, in declaration order:
           1-12  width BW word mem locals ext_spec word_ok mem_ok
                 locals_ok ext_spec_ok field_parameters field_representation
           13-15 curve_add_name curve_double_name opp_name
           16    curve_add
           17-22 pt_eq pt_eq_equiv oncurve oncurve_id oncurve_curve_add
                 curve_add_Proper
           23-24 curve_add_id_l curve_add_assoc
           25-26 functions HCurveDouble
           27-31 dk Px Py Pz Honcurve_P
           32-34 num_iters Hws_nn table_entries
           35    HProcessDigit
         [Hbounds_eq], [field_parameters_ok], [field_representation_ok],
         [HCurveAddInplace], [HFelemCopy], [HOpp] are unused there and so
         are not generalized. *)
      eapply WeakestPreconditionProperties.Proper_cmd;
      ...
      { (* HProcessDigit — compose single_load_and_process_ok + Hhorner_step *)
        ...
        (* [Hhorner_oncurve] and [Hhorner_step] connect digit_point to scmul *)
