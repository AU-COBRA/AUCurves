(** * secp256k1 Fp inversion via Bernstein-Yang divstep algorithm.

    Mirrors BLS12_FpInv.v for the secp256k1 base field.

    secp256k1 base prime:
      p = 2^256 - 2^32 - 977
      Z.log2 p + 1 = 256
      iterations = (49 * 256 + 57) / 17 = 12601 / 17 = 741

    Word-by-word Montgomery layout: 4 limbs of 64 bits (saturated: 5 limbs).
    (Note: fiat-crypto also offers a Dettman-form unsaturated layout for
    secp256k1; this file targets the Montgomery layout consistent with
    [Bedrock.Curve.Secp256k1_*] in this repo.)
*)
...
(* ================================================================== *)
(* secp256k1 prime and its parameters                                  *)
(* ================================================================== *)

(** The secp256k1 base field prime. *)
Definition secp256k1_p : Z :=
  Eval vm_compute in secp256k1_modulus.

Definition machine_wordsize : Z := 64.

(** 4 x 64-bit limbs for Montgomery representation (256 bits). *)
Definition n : nat := 4.

(** Saturated representation uses n+1 = 5 limbs. *)
Definition sat_limbs : nat := 5.

Definition secp256k1_mbits : Z := Z.log2 secp256k1_p + 1.

Lemma secp256k1_mbits_val : secp256k1_mbits = 256.
Proof. vm_compute. reflexivity. Qed.

Lemma secp256k1_mbits_ge_46 : 46 <= secp256k1_mbits.
...
Definition secp256k1_divstep_iters : Z :=
  Eval vm_compute in ((49 * secp256k1_mbits + 57) / 17).

Lemma secp256k1_divstep_iters_val : secp256k1_divstep_iters = 741.
Proof. vm_compute. reflexivity. Qed.

Lemma secp256k1_divstep_iters_eq_precomp_exp :
  secp256k1_divstep_iters =
  (if Decidable.dec (secp256k1_mbits < 46) then (49 * secp256k1_mbits + 80) / 17
   else (49 * secp256k1_mbits + 57) / 17).
Proof.
...
Section FpInv.
  ...
  (** Precomputation: ((p+1)/2)^N mod p with N = 741.
      Computed externally via:
        python3 -c "p=2**256-2**32-977; print(hex(pow((p+1)//2, 741, p)))"
  *)
  Definition precomp_val : Z :=
    0xc1a22f882904238c7be48be51526440086a88aa7b01acfe6b7111e04880bdf7f.

  Definition fp_inv_spec (x : Z) : Z :=
    let '(d, f, g, v, r) :=
      iter_divstep_spec p (Z.to_nat secp256k1_divstep_iters) 1 p x 0 1 in
    let v_corrected := if (f <? 0) then (p - v) mod p else v mod p in
    (v_corrected * precomp_val) mod p.

  Axiom by_convergence_dfg_secp256k1 : forall x,
    0 < x < p ->
    Z.gcd x p = 1 ->
    let '(_, f_N, g_N) :=
      iter_divstep_dfg (Z.to_nat secp256k1_divstep_iters) 1 p x in
    g_N = 0 /\ (f_N = 1 \/ f_N = -1).
  ...
  Lemma precomp_times_pow2 :
    (precomp_val * 2 ^ secp256k1_divstep_iters) mod p = 1.
  Proof.
    subst p. unfold precomp_val, secp256k1_divstep_iters, secp256k1_p.
    vm_compute. reflexivity.
  Qed.

  Lemma secp256k1_coprime : forall x, 0 < x < p -> Z.gcd x p = 1.
  Proof.
    intros x Hx.
    apply Zgcd_1_rel_prime.
    apply rel_prime_sym.
    apply prime_rel_prime.
    - subst p. unfold secp256k1_p. exact prime_secp256k1.
    - intros [k Hk]. subst x.
  ...
  Proof.
    intros x Hx Hgcd.
    unfold fp_inv_spec.
    destruct (iter_divstep_spec p (Z.to_nat secp256k1_divstep_iters) 1 p x 0 1)
      as [[[[d_N f_N] g_N] v_N] r_N] eqn:Hiter.
    pose proof (iter_invariant p (Z.to_nat secp256k1_divstep_iters) 1 p x 0 1 x
                  p_pos p_odd) as Hinv.
    ...
    pose proof (by_convergence_dfg_secp256k1 x Hx Hgcd) as Hconv.
    pose proof (iter_dfg_agree (Z.to_nat secp256k1_divstep_iters) p 1 p x 0 1) as Hagree.
    rewrite Hiter in Hagree.
    destruct (iter_divstep_dfg (Z.to_nat secp256k1_divstep_iters) 1 p x)
    ...
    - replace (1 <? 0) with false by reflexivity.
      ...
      assert (Hmod : (v_N * x) mod p = (2 ^ Z.of_nat (Z.to_nat secp256k1_divstep_iters)) mod p). {
        assert (Hp : p <> 0) by (pose proof p_pos; lia).
        apply Z.mod_divide in Hv_inv; [|exact Hp].
        destruct Hv_inv as [k Hk].
        assert (v_N * x = 2 ^ Z.of_nat (Z.to_nat secp256k1_divstep_iters) + k * p) by lia.
        rewrite H.
        rewrite Zplus_mod, Z_mod_mult, Z.add_0_r, Z.mod_mod by lia.
        reflexivity.
      }
      rewrite <- Zmult_mod_idemp_r, Hmod, Zmult_mod_idemp_r.
      rewrite Z2Nat.id by (unfold secp256k1_divstep_iters; vm_compute; discriminate).
      exact precomp_times_pow2.
    - replace (-1 <? 0) with true by reflexivity.
      replace (2 ^ Z.of_nat (Z.to_nat secp256k1_divstep_iters) * -1)
        with (- (2 ^ Z.of_nat (Z.to_nat secp256k1_divstep_iters))) in Hv_inv by ring.
      replace (v_N * x - - 2 ^ Z.of_nat (Z.to_nat secp256k1_divstep_iters))
        with (v_N * x + 2 ^ Z.of_nat (Z.to_nat secp256k1_divstep_iters)) in Hv_inv by ring.
      set (iters := Z.of_nat (Z.to_nat secp256k1_divstep_iters)) in *.
  ...
  Theorem fp_inv_correct : forall x,
    ...
    (fp_inv_spec x * x) mod p = 1.
  Proof.
    intros x Hx. apply fp_inv_correct_ax; auto. apply secp256k1_coprime; auto.
  Qed.

End FpInv.

(* ================================================================== *)
(* Concrete parameters for secp256k1 divstep                           *)
(* ================================================================== *)
...
Definition secp256k1_m' : Z :=
  Eval vm_compute in
    (let r := 2^machine_wordsize in
     let p := secp256k1_p in
     ((-1 / p) mod r + r) mod r).

Lemma secp256k1_p_pos : 1 < secp256k1_p.
Proof. vm_compute. reflexivity. Qed.

Lemma secp256k1_p_fits : secp256k1_p < (2^machine_wordsize) ^ Z.of_nat n.
Proof. vm_compute. reflexivity. Qed.

Lemma secp256k1_p_odd : Z.odd secp256k1_p = true.
Proof. vm_compute. reflexivity. Qed.
