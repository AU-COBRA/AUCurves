...
use crate::group::G1;
use crate::scalar_mul_extracted::p384_wnaf_scalar_mul_extracted;
use crate::Fp;

/// Digit count: `p384_wNAF_Instance.p384_num_digits`.
pub const NUM_DIGITS: usize = 385;
...
/// `wnaf_digits 4 k 385` of `src/Bedrock/Field/Synthesis/Examples/wNAF.v`.
///
...
/// `G1 -> X || Y || Z`, 48 little-endian Montgomery bytes each — the
/// memory image of `BLS12_wNAF_ProcessDigits.TablePoint`.
pub fn point_to_bytes(p: &G1) -> [u8; 144] {
    let mut out = [0u8; 144];
...
pub fn point_from_bytes(b: &[u8; 144]) -> G1 {
    let rd = |off: usize| {
    ...
    G1 { x: rd(0), y: rd(48), z: rd(96) }
}

/// Variable-base scalar multiplication `k * P` through the Rocq-emitted
/// w = 4 wNAF driver.  `k` is `6` little-endian u64 limbs, as for
...
pub fn g1_scalar_mul_wnaf(k: &[u64; LIMBS], p: &G1) -> G1 {
    ...
    let mut table = [[0u8; 144]; 4];
    let mut base = point_to_bytes(p);
    let mut out = [0u8; 144];
    p384_wnaf_scalar_mul_extracted(&mut out, &mut base, &mut table, &mut digits);
