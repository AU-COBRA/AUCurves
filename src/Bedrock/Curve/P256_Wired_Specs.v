(** Wired Bignum-style spec instances for P-256 field operations.
    ...
    [Bedrock.Field.Synthesis.Examples.p256_prime] and exposes
...
Local Notation p256_m' := (@Field.m' bw p256_field_parameters).
Notation eval := (@WordByWordMontgomery.WordByWordMontgomery.eval bw n).
Notation from_mont := (@WordByWordMontgomery.from_montgomerymod bw n m p256_m').
Local Notation toZ := (List.map Interface.word.unsigned).

(** ** Concrete spec_of instances *)

Instance spec_of_p256_mul_bignum : spec_of "p256_coord_mul" :=
  fun functions =>
...
Instance spec_of_p256_add_bignum : spec_of "p256_coord_add" :=
  fun functions =>
    forall (wsx wsy old_out : list word.rep)
           (px py pout : word.rep)
           (tr : trace) (mem0 : @map.rep _ _ BasicC64Semantics.mem)
...
Instance spec_of_p256_sub_bignum : spec_of "p256_coord_sub" :=
  fun functions =>
    forall (wsx wsy old_out : list word.rep)
           (px py pout : word.rep)
           (tr : trace) (mem0 : @map.rep _ _ BasicC64Semantics.mem)
...
Instance spec_of_p256_square_bignum : spec_of "p256_coord_square" :=
  fun functions =>
...
Instance spec_of_p256_opp_bignum : spec_of "p256_coord_opp" :=
  fun functions =>
    forall (wsx old_out : list word.rep)
           (px pout : word.rep)
           (tr : trace) (mem0 : @map.rep _ _ BasicC64Semantics.mem)
