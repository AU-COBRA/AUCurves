//! Build script for the secp256k1 safe-Rust crate.
//!
//! Optionally links the two CryptOpt-superoptimized field leaves in
//! `generated/` and sets `cfg(secp256k1_cryptopt_asm)`, which makes `fp_mul`
//! and `fp_square` in `src/lib.rs` route to them instead of to fiat-rust.
//!
...
//! `SECP256K1_NO_CRYPTOPT=1` in the environment to force the fallback.
...
fn main() {
    ...
    println!("cargo:rerun-if-changed=generated/secp256k1_mul_cryptopt.asm");
    println!("cargo:rerun-if-changed=generated/secp256k1_square_cryptopt.asm");
    println!("cargo:rerun-if-env-changed=SECP256K1_NO_CRYPTOPT");
    println!("cargo:rustc-check-cfg=cfg(secp256k1_cryptopt_asm)");

    if env::var_os("SECP256K1_NO_CRYPTOPT").is_some() {
        println!("cargo:warning=SECP256K1_NO_CRYPTOPT set; using fiat-rust field leaves");
        return;
    }
    ...
    if !(std::is_x86_feature_detected!("bmi2") && std::is_x86_feature_detected!("adx")) {
        println!(
            "cargo:warning=host lacks BMI2/ADX; using fiat-rust field leaves for secp256k1"
        );
        return;
    }
    ...
    match status {
        Ok(s) if s.success() => {
            ...
            println!("cargo:rustc-link-lib=static=secp256k1_cryptopt");
            println!("cargo:rustc-cfg=secp256k1_cryptopt_asm");
        }
        _ => {
            println!(
                "cargo:warning=build.sh failed (nasm missing?); \
                 using fiat-rust field leaves for secp256k1"
            );
        }
    }
}
