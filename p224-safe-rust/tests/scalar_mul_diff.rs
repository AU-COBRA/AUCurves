...
//! under which `p224_wNAF_Instance.p224_wnaf_single_full` states its
//! conclusion.
//!
...
use p224::group::*;
use p224::wnaf::{g1_scalar_mul_wnaf, point_from_bytes, point_to_bytes, wnaf_digits_w4, LIMBS};
use p224::{fp_mul, Fp};
...
impl Rng {
    ...
    /// A scalar below 2^224 (the driver's digit budget).
    fn scalar(&mut self) -> [u64; LIMBS] {
        let mut s = [0u64; LIMBS];
        for limb in s.iter_mut() {
            *limb = self.next();
        }
        s[LIMBS - 1] &= 0x0000_0000_ffff_ffff;
        s
    }
}
...
fn wnaf_matches_ladder_on_kat_scalars() {
    ...
    assert!(g1_is_identity(&g1_scalar_mul_wnaf(&P224_N, &g)));
}
