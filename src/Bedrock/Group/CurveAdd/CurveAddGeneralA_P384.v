(** * P-384 instantiation of the Rupicola general-a RCB addition.
    ...
    transport, and [p384_curve_add_general_bignum_bridge_valid_out]
    (Qed) for the Bignum shape with valid output buffers on entry,
    which is what [spec_of_rcb_add_general] requires.

    Honesty ledger (this file): 0 Admitted.  The unconditional bridge
    [p384_curve_add_general_bignum_bridge] is not stated (not derivable
...
Section P384_GeneralA.
  ...
  (** The Montgomery decoding as it occurs in [P384_add_Gallina_spec]:
      the [P384Curve_G1] constants (Local Definitions there, qualified
      access) rather than the literals of [p384_valid]. *)
  ...
  Local Notation G_valid :=
    (@WordByWordMontgomery.valid P384Curve_G1.bw P384Curve_G1.n P384Curve_G1.m).

  (** The [MontgomeryRingTheory] lemmas at the P-384 parameters
  ...
  Local Notation G_evfrom_mod' :=
    (MontgomeryRingTheory.evfrom_mod'
       P384Curve_G1.m P384Curve_G1.bw P384Curve_G1.n P384Curve_G1.r' P384Curve_G1.m'
       P384Curve_G1.r'_correct P384Curve_G1.m'_correct P384Curve_G1.bw_big
       P384Curve_G1.n_nz P384Curve_G1.m_small P384Curve_G1.m_big).
  Local Notation G_valid_valid'_equiv :=
    (MontgomeryRingTheory.valid_valid'_equiv
       P384Curve_G1.m P384Curve_G1.bw P384Curve_G1.n
       P384Curve_G1.n_nz P384Curve_G1.m_big).
  ...
  (** The fiat-crypto Montgomery constant of [p384_field_parameters]
      is the [P384Curve_G1] one (both are modinv(-m, 2^64) = 2^32 + 1). *)
  Lemma p384_fiat_m'_eq : @Field.m' 64 p384_field_parameters = P384Curve_G1.m'.
  Proof. Timeout 600 vm_compute. reflexivity. Qed.

  Lemma p384_M_eq : Z.pos M_pos = P384Curve_G1.m.
  Proof. Timeout 600 vm_compute. reflexivity. Qed.

  (** [feval] is the Montgomery decoding, reduced mod m. *)
  Lemma p384_feval_evfrom (ws : list word) :
    F.to_Z (feval ws) = G_evfrom (toZ ws) mod P384Curve_G1.m.
  Proof.
    Timeout 600 change (feval ws)
      with (F.of_Z M_pos
              (@WordByWordMontgomery.eval 64 6
                 (@WordByWordMontgomery.from_montgomerymod 64 6 p384_m
                    (@Field.m' 64 p384_field_parameters) (toZ ws)))).
    rewrite p384_fiat_m'_eq, F.to_Z_of_Z, ?p384_M_eq.
    Timeout 600 reflexivity.
  Qed.

  Lemma p384_valid_evfrom_mod (l : list Z) :
    G_valid l -> G_evfrom l mod P384Curve_G1.m = G_evfrom l.
  Proof.
    intros Hv. symmetry. exact (G_evfrom_mod' l Hv).
  Qed.

  Lemma p384_feval_evfrom_valid (ws : list word) :
    p384_valid (toZ ws) -> G_evfrom (toZ ws) = F.to_Z (feval ws).
  Proof.
    intros Hv. rewrite p384_feval_evfrom. symmetry.
    apply p384_valid_evfrom_mod. exact Hv.
  Qed.
  ...
  Lemma p384_feval_inj (ws1 ws2 : list word) :
    p384_valid (toZ ws1) -> p384_valid (toZ ws2) ->
    feval ws1 = feval ws2 -> ws1 = ws2.
  Proof.
    intros Hv1 Hv2 Heq.
    apply (f_equal F.to_Z) in Heq.
    rewrite !p384_feval_evfrom in Heq.
    ...
    pose proof (G_eval_from_mont_mod_inj
                  (MontgomeryRingTheory.enc_mont
                     P384Curve_G1.m P384Curve_G1.bw P384Curve_G1.n (toZ ws1) Hv1')
                  (MontgomeryRingTheory.enc_mont
                     P384Curve_G1.m P384Curve_G1.bw P384Curve_G1.n (toZ ws2) Hv2')
                  Heq) as Hrec.
    apply map_unsigned_inj.
    exact (f_equal (MontgomeryRingTheory.val
                      P384Curve_G1.m P384Curve_G1.bw P384Curve_G1.n) Hrec).
  Qed.
  ...
  Lemma p384_a_toZ :
    @WordByWordMontgomery.eval P384Curve_G1.bw P384Curve_G1.n
      (MontgomeryCurveSpecs.a_list P384Curve_G1.bw P384Curve_G1.n P384Curve_G1.a)
    = F.to_Z (feval (proj1_sig p384_a_felem)).
  Proof.
    rewrite p384_a_feval, F.to_Z_of_Z. Timeout 600 vm_compute. reflexivity.
  Qed.

  Lemma p384_three_b_toZ :
    @WordByWordMontgomery.eval P384Curve_G1.bw P384Curve_G1.n
      (MontgomeryCurveSpecs.three_b_list
         P384Curve_G1.bw P384Curve_G1.n P384Curve_G1.three_b)
    = F.to_Z (feval (proj1_sig p384_three_b_felem)).
  Proof.
    rewrite p384_three_b_feval, F.to_Z_of_Z. Timeout 600 vm_compute. reflexivity.
  Qed.

  (** Memory-predicate transport, pointwise. *)
  Lemma p384_Bignum_to_FElem2 (p : word) (ws : list word) :
    p384_valid (toZ ws) ->
    Lift1Prop.impl1 (Bignum 6 p ws)
                    (Compilation2.FElem (Some tight_bounds) p (feval ws)).
  Proof.
    intros Hv mm HB.
    unfold Bignum in HB. apply sep_emp_l in HB. destruct HB as [Hlen Harr].
    change 6%nat with (felem_size_in_words (FieldRepresentation:=p384_frep)) in Hlen.
    unfold Compilation2.FElem, Lift1Prop.ex1.
    exists (exist _ ws Hlen).
    apply sep_emp_l. split; [split; [reflexivity | exact Hv] | exact Harr].
  Qed.

  Lemma p384_FElem2_to_Bignum (p : word) (v : F) (mm : BasicC64Semantics.mem) :
    Compilation2.FElem (Some tight_bounds) p v mm ->
    exists ws : list word,
      feval ws = v /\ p384_valid (toZ ws) /\ Bignum 6 p ws mm.
  Proof.
    intros HF.
  ...
  Lemma p384_pre_bridge
        (pX1 pY1 pZ1 pX2 pY2 pZ2 poutx pouty poutz : word)
        (wX1 wY1 wZ1 wX2 wY2 wZ2 wox woy woz : list word)
        (R : BasicC64Semantics.mem -> Prop) :
    p384_valid (toZ wX1) -> p384_valid (toZ wY1) -> p384_valid (toZ wZ1) ->
    p384_valid (toZ wX2) -> p384_valid (toZ wY2) -> p384_valid (toZ wZ2) ->
  ...
  (** The Bignum-level specification with the three output buffers
      ...
      i.e. [bounded_by tight_bounds] = [p384_valid] of their old
      contents.  [spec_of_p384_curve_add_general_bignum] above makes no
      ...
      one. *)
  Definition spec_of_p384_curve_add_general_bignum_valid_out
    : spec_of "curve_add_general" :=
    fun functions =>
      forall (wX1 wY1 wZ1 wX2 wY2 wZ2
              wold_outx wold_outy wold_outz : list word)
  ...
  (** Bridge from the FElem-level derived spec to the Bignum shape.
      1. pre-transport ([p384_pre_bridge]); 2. the FElem-level spec at
      [X1 := feval wX1] etc.; 3. post-transport by destructing the sep
      chain, [p384_FElem2_to_Bignum] on each clause, canonicity
      ([p384_feval_inj]) for the six preserved inputs, and
      ...
      ([p384_feval_evfrom_valid]) and the constant identifications. *)
  Theorem p384_curve_add_general_bignum_bridge_valid_out :
    forall functions,
      spec_of_rcb_add_general p384_three_b_felem p384_a_felem functions ->
      spec_of_p384_curve_add_general_bignum_valid_out functions.
  Proof.
    intros functions Hspec.
    unfold spec_of_p384_curve_add_general_bignum_valid_out.
    ...
    specialize (Hspec poutx pouty poutz pX1 pY1 pZ1 pX2 pY2 pZ2
                  ...
                  Rout tr m0).
    specialize (Hspec
                  (p384_pre_bridge pX1 pY1 pZ1 pX2 pY2 pZ2 poutx pouty poutz
    ...
    (* 4: algebra, by the generic F-level lemma; the constants
       ...
       [feval (proj1_sig p384_a_felem)] etc. by conversion. *)
    try unfold P384_add_Gallina_spec.
    Timeout 600 refine
      (rcb_general_a_gallina_to_Z (field_parameters := p384_field_parameters)
  ...
  (** The unconditional shape, NOT stated as a theorem.
      ...
      output buffers, i.e. canonical ([p384_valid]) old contents, and
      says nothing about a call on non-canonical output buffers, while
      [spec_of_p384_curve_add_general_bignum] assumes nothing about
      ...
      [p384_curve_add_general_bignum_bridge_valid_out] (Qed above); the
