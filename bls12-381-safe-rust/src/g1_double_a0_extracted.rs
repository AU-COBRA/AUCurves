//! Rocq-EMITTED complete projective doubling for BLS12-381 G1
...
//!
//!   Compute bls12_381_g1_double_a0_rs.       (* §4 of *)
...
//!   * `bls12_381_g1_double_a0_borrow_ok` holds by `vm_compute`.
//!   * The `cB3` constant below is
//!     `mont_bytes 6 bls12_381_m ((3*b) mod m)`, certified
//!     `bls12_381_threeb_bytes_len = 48`.
//!
...
use crate::extracted_leaves::{bls12_fp_add, bls12_fp_mul, bls12_fp_sub, FBYTES, LIMBS};
...
pub fn g1_proj_double_bytes(out: &mut [u8; PTBYTES], p: &[u8; PTBYTES]) {
    let mut inb = *p;
    bls12_381_g1_double_a0_extracted(out, &mut inb);
}
...
pub fn g1_proj_double_limbs(p: &[[u64; LIMBS]; 3]) -> [[u64; LIMBS]; 3] {
    ...
    bls12_381_g1_double_a0_extracted(&mut outb, &mut inb);
...
pub fn bls12_381_g1_double_a0_extracted(out: &mut [u8; 144], arg0: &mut [u8; 144]) {
    ...
    cB3.copy_from_slice(&[46u8, 85u8, 39u8, 0u8, 0u8, 0u8, 118u8, 68u8, 32u8, 0u8, 72u8, 67u8, 154u8, 0u8, 184u8, 220u8, 89u8, 139u8, 110u8, 74u8, 206u8, 233u8, 126u8, 111u8, 198u8, 91u8, 169u8, 192u8, 183u8, 48u8, 3u8, 177u8, 183u8, 84u8, 30u8, 251u8, 252u8, 177u8, 64u8, 97u8, 225u8, 180u8, 11u8, 127u8, 9u8, 190u8, 129u8, 3u8]);
    unsafe { bls12_fp_mul(t0.as_mut_ptr(), Y1.as_ptr(), Y1.as_ptr()) };
    unsafe { bls12_fp_add(za.as_mut_ptr(), t0.as_ptr(), t0.as_ptr()) };
    unsafe { bls12_fp_add(zb.as_mut_ptr(), za.as_ptr(), za.as_ptr()) };
    unsafe { bls12_fp_add(zc.as_mut_ptr(), zb.as_ptr(), zb.as_ptr()) };
    unsafe { bls12_fp_mul(t1.as_mut_ptr(), Y1.as_ptr(), Z1.as_ptr()) };
    unsafe { bls12_fp_mul(t2.as_mut_ptr(), Z1.as_ptr(), Z1.as_ptr()) };
    unsafe { bls12_fp_mul(t2b.as_mut_ptr(), cB3.as_ptr(), t2.as_ptr()) };
    unsafe { bls12_fp_mul(xa.as_mut_ptr(), t2b.as_ptr(), zc.as_ptr()) };
    unsafe { bls12_fp_add(ya.as_mut_ptr(), t0.as_ptr(), t2b.as_ptr()) };
    unsafe { bls12_fp_mul(Z3.as_mut_ptr(), t1.as_ptr(), zc.as_ptr()) };
    unsafe { bls12_fp_add(t1b.as_mut_ptr(), t2b.as_ptr(), t2b.as_ptr()) };
    unsafe { bls12_fp_add(t2c.as_mut_ptr(), t1b.as_ptr(), t2b.as_ptr()) };
    unsafe { bls12_fp_sub(t0b.as_mut_ptr(), t0.as_ptr(), t2c.as_ptr()) };
    unsafe { bls12_fp_mul(yb.as_mut_ptr(), t0b.as_ptr(), ya.as_ptr()) };
    unsafe { bls12_fp_add(Y3.as_mut_ptr(), xa.as_ptr(), yb.as_ptr()) };
    unsafe { bls12_fp_mul(t1c.as_mut_ptr(), X1.as_ptr(), Y1.as_ptr()) };
    unsafe { bls12_fp_mul(xb.as_mut_ptr(), t0b.as_ptr(), t1c.as_ptr()) };
    unsafe { bls12_fp_add(X3.as_mut_ptr(), xb.as_ptr(), xb.as_ptr()) };
    for i in 0u64..48u64 {
