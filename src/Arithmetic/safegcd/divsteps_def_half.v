(** * Bernstein–Yang divstep algorithm — δ₀ = 1/2 variant.

    Mirror of [divsteps_def.v] using Wuille's [zeta]-encoding for δ₀ = 1/2:
    integer field [delta := zeta = -(δ + 1/2)] with [delta_0 = -1]
    representing the half-integer [δ_0 = 1/2].

    Algorithmic translation:
    - test "δ > 0"      ↔ "delta < 0"
    - "δ ↦ -δ + 1" (swap) ↔ "delta ↦ -delta - 2"
    - "δ ↦  δ + 1" (no swap) ↔ "delta ↦ delta - 1"

    Modular bookkeeping on (f, g, d, e) is **identical** to the original
    file; only the [delta] update and test direction change.  The proofs
    of [gcd_step], [modulo_step], etc., go through verbatim.
*)

Require Import ZArith.

Module divsteps_half.
...
if Z.even (g st)
  then {| delta := delta st - 1                  (* δ ↦ δ + 1 *)
        ...
        ; modulus := modulus st
        |}
  else if (delta st <? 0)%Z                       (* δ > 0 *)
         then {| delta := - delta st - 2          (* δ ↦ -δ + 1 *)
               ; f := g st
               ; g := (g st - f st) / 2
               ; d := e st
...
(** All the basic structural lemmas carry over because they don't use
    [delta] arithmetic — only the case-split on [Z.even (g st)] /
    [delta <? 0]. *)
...
case (Z.even g) eqn:Hg;[|case (delta <? 0)%Z];auto.
Qed.
...
intros ->.
cut (divsteps_half.g (Nat.iter n step st) = 0%Z /\ divsteps_half.d (Nat.iter n step st) = d).
 tauto.
induction n; auto.
simpl.
...
(** Same proof as [divsteps_def.gcd_step] — the [delta] cases differ only
    in the test ([delta <? 0]), but the [gcd] computation in the three
...
unfold step, gcd.
cbn -[Z.ltb Z.add Z.sub] in *.
case (Z.even g) eqn:Hg;[|case (delta <? 0)%Z];cbn -[Z.add Z.sub].
...
(** The remaining lemmas ([eqm], [div2M], [inv2M] interactions) are
    quoted verbatim from [divsteps_def.v] — they don't reference
    [delta] at all. *)
...
unfold step.
cbn -[Z.ltb Z.add Z.sub] in *.
case (Z.even g) eqn:Hg;[|case (delta <? 0)%Z];simpl;split;auto.
