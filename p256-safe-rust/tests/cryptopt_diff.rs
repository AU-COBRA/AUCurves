...
//! leaves they replace.
//!
//! `p256::fp_mul` / `p256::fp_square` are the CryptOpt assembly when
//! `build.rs` linked it and the fiat functions otherwise;
//! `p256::fp_mul_fiat` / `p256::fp_square_fiat` are always the fiat
...
use p256::{fp_mul, fp_mul_fiat, fp_square, fp_square_fiat, fp_to_montgomery, Fp, FpRaw};
...
fn edges() -> Vec<Fp> {
    ...
    vec![
        ...
        Fp([!0, !0, !0, 0]),
        // Only the high limb set, just under p's high limb.
        Fp([0, 0, 0, 0xffff_ffff_0000_0000]),
        // Straddles the zero limb of p.
        Fp([!0, 0x0000_0000_ffff_ffff, 0, 0xffff_ffff_0000_0000]),
        Fp([0, 0, !0, 0xffff_ffff_0000_0000]),
...
impl Rng {
    ...
    /// A uniform-ish element of `[0, p)`.  Biased towards the top of the
    /// range on purpose -- that is where the final conditional subtraction
    /// of the Montgomery reduction is exercised.
    fn fe(&mut self) -> Fp {
        loop {
            let mut w = [self.word(), self.word(), self.word(), self.word()];
            // Half the draws get a near-maximal high limb.
            if w[0] & 1 == 0 {
                w[3] |= 0xffff_ffff_0000_0000;
                w[3] &= 0xffff_ffff_0000_0001;
            }
            if lt_p(&w) {
                return Fp(w);
            }
