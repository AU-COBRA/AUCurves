(** * BN446 Fp inversion via Bernstein-Yang divstep algorithm.

    Mirrors [BN254_FpInv.v] for the BN446 base field.

    BN446 base prime:
      p = 36 u^4 + 36 u^3 + 24 u^2 + 6 u + 1,  u = 2^110 + 2^36 + 1
      Z.log2 p + 1 = 446
      iterations = (49 * 446 + 57) / 17 = 21911 / 17 = 1288

    Word-by-word Montgomery layout: 7 limbs of 64 bits (saturated: 8 limbs).
...
(* ================================================================== *)
(* BN446 prime and its parameters                                      *)
(* ================================================================== *)

(** The BN446 base field prime. *)
Definition bn446_p : Z :=
  Eval vm_compute in bn446_modulus.

(** Machine parameters for 64-bit limbs. *)
Definition machine_wordsize : Z := 64.

(** Number of 64-bit limbs for Montgomery representation (7 limbs = 448 bits). *)
Definition n : nat := 7.

(** Number of limbs for the saturated representation (n+1 = 8). *)
Definition sat_limbs : nat := 8.

(** The number of bits in p. *)
Definition bn446_mbits : Z := Z.log2 bn446_p + 1.

Lemma bn446_mbits_val : bn446_mbits = 446.
Proof. vm_compute. reflexivity. Qed.

(** BN446 has >= 46 bits, so we use the large-prime formula. *)
Lemma bn446_mbits_ge_46 : 46 <= bn446_mbits.
...
(** The number of divstep iterations for BN446 inversion. *)
Definition bn446_divstep_iters : Z :=
  Eval vm_compute in ((49 * bn446_mbits + 57) / 17).

Lemma bn446_divstep_iters_val : bn446_divstep_iters = 1288.
Proof. vm_compute. reflexivity. Qed.

(** This equals the formula from divstep_precomp_correct when mbits >= 46. *)
Lemma bn446_divstep_iters_eq_precomp_exp :
  bn446_divstep_iters =
  (if Decidable.dec (bn446_mbits < 46) then (49 * bn446_mbits + 80) / 17
   else (49 * bn446_mbits + 57) / 17).
Proof.
...
Section FpInv.
  ...
  (** Precomputation: ((p+1)/2)^N mod p with N = 1288.
      Computed externally via:
        python3 -c "p=...; print(hex(pow((p+1)//2, 1288, p)))"
  *)
  Definition precomp_val : Z :=
    0xf13326149e06fb0ad6b6ed5b5cc1dda1710bdb1b86fe94d5314861540772fd4a62a06f8d5e91461e79bb770ee0fea57fe0ba9af9a462b8b.

  Definition fp_inv_spec (x : Z) : Z :=
    let '(d, f, g, v, r) :=
      iter_divstep_spec p (Z.to_nat bn446_divstep_iters) 1 p x 0 1 in
    let v_corrected := if (f <? 0) then (p - v) mod p else v mod p in
    (v_corrected * precomp_val) mod p.

  Axiom by_convergence_dfg_bn446 : forall x,
    0 < x < p ->
    Z.gcd x p = 1 ->
    let '(_, f_N, g_N) :=
      iter_divstep_dfg (Z.to_nat bn446_divstep_iters) 1 p x in
    g_N = 0 /\ (f_N = 1 \/ f_N = -1).
  ...
  Lemma precomp_times_pow2 :
    (precomp_val * 2 ^ bn446_divstep_iters) mod p = 1.
  Proof.
    subst p. unfold precomp_val, bn446_divstep_iters, bn446_p.
    vm_compute. reflexivity.
  Qed.

  Lemma bn446_coprime : forall x, 0 < x < p -> Z.gcd x p = 1.
  Proof.
    intros x Hx.
    apply Zgcd_1_rel_prime.
    apply rel_prime_sym.
    apply prime_rel_prime.
    - subst p. unfold bn446_p. exact prime_bn446.
    - intros [k Hk]. subst x.
  ...
  Proof.
    intros x Hx Hgcd.
    unfold fp_inv_spec.
    destruct (iter_divstep_spec p (Z.to_nat bn446_divstep_iters) 1 p x 0 1)
      as [[[[d_N f_N] g_N] v_N] r_N] eqn:Hiter.
    pose proof (iter_invariant p (Z.to_nat bn446_divstep_iters) 1 p x 0 1 x
                  p_pos p_odd) as Hinv.
    ...
    pose proof (by_convergence_dfg_bn446 x Hx Hgcd) as Hconv.
    pose proof (iter_dfg_agree (Z.to_nat bn446_divstep_iters) p 1 p x 0 1) as Hagree.
    rewrite Hiter in Hagree.
    destruct (iter_divstep_dfg (Z.to_nat bn446_divstep_iters) 1 p x)
    ...
    - replace (1 <? 0) with false by reflexivity.
      ...
      assert (Hmod : (v_N * x) mod p = (2 ^ Z.of_nat (Z.to_nat bn446_divstep_iters)) mod p). {
        assert (Hp : p <> 0) by (pose proof p_pos; lia).
        apply Z.mod_divide in Hv_inv; [|exact Hp].
        destruct Hv_inv as [k Hk].
        assert (v_N * x = 2 ^ Z.of_nat (Z.to_nat bn446_divstep_iters) + k * p) by lia.
        rewrite H.
        rewrite Zplus_mod, Z_mod_mult, Z.add_0_r, Z.mod_mod by lia.
        reflexivity.
      }
      rewrite <- Zmult_mod_idemp_r, Hmod, Zmult_mod_idemp_r.
      rewrite Z2Nat.id by (unfold bn446_divstep_iters; vm_compute; discriminate).
      exact precomp_times_pow2.
    - replace (-1 <? 0) with true by reflexivity.
      replace (2 ^ Z.of_nat (Z.to_nat bn446_divstep_iters) * -1)
        with (- (2 ^ Z.of_nat (Z.to_nat bn446_divstep_iters))) in Hv_inv by ring.
      replace (v_N * x - - 2 ^ Z.of_nat (Z.to_nat bn446_divstep_iters))
        with (v_N * x + 2 ^ Z.of_nat (Z.to_nat bn446_divstep_iters)) in Hv_inv by ring.
      set (iters := Z.of_nat (Z.to_nat bn446_divstep_iters)) in *.
  ...
  Theorem fp_inv_correct : forall x,
    ...
    (fp_inv_spec x * x) mod p = 1.
  Proof.
    intros x Hx. apply fp_inv_correct_ax; auto. apply bn446_coprime; auto.
  Qed.

End FpInv.

(* ================================================================== *)
(* Concrete parameters for BN446 divstep                               *)
(* ================================================================== *)
...
Lemma bn446_p_pos : 1 < bn446_p.
Proof. vm_compute. reflexivity. Qed.

Lemma bn446_p_fits : bn446_p < (2^machine_wordsize) ^ Z.of_nat n.
Proof. vm_compute. reflexivity. Qed.

Lemma bn446_p_odd : Z.odd bn446_p = true.
Proof. vm_compute. reflexivity. Qed.
