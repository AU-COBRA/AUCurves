(** * BN256_PairingRustConcrete.v — Step 1: concrete predicates and fenv.
 *
 * Discharges the [Parameter]s in [BN256_PairingRust.v] by giving
 * concrete [Definition]s for:
 *
 *   - The BN256 [CallEnv] ([bn256_call_env]).
 *   - The BN256 function environment ([bn256_pairing_fenv]).
 *   - The leaf_spec (imported from [SafeRustBN256Concrete]).
 ...
 *
 * The file is kept self-contained (does not import [BN256_PairingRust.v])
 * to avoid the Parameter-vs-Definition name clash on
 * [bn256_leaf_spec_concrete].
 *)
...
Require Import Bedrock.SafeRustBN256Concrete.
...
(** NB: we do NOT [Require Import MillerLoopWP] because that
    ...
    small [bn256_optimal_ate_spec] definition locally below. *)

(* ================================================================ *)
(* §0. Global parameters                                             *)
(* ================================================================ *)

Definition bn256_N : nat := 5.
Definition bn256_u64_max : nat := Nat.pow 2 64.

(** Local mirror of [BN256_PairingCorrect.bn256_miller_loop_with_corrections].
    Copied to keep this file independent of [BN256_PairingCorrect.v]
    (which pulls in a heavy bedrock2 instance stack).  Step 2's
    bridge proofs connect the two via reflexivity / unfolding. *)
Local Definition bn256_p_val : Z := prime_p bn256_params.
Local Definition bn256_xi_val : Fp2_Z := (3%Z, 1%Z).  (** BN256: xi = 3 + u *)

Definition bn256_miller_loop_with_corrections
           (gamma1 gamma_y gamma1_p2 : Fp2_Z)
           (Px Py : Z) (Qx Qy : Fp2_Z) : Fp12_Z :=
  let '(f, Tx, Ty) :=
    affine_miller_aux bn256_zmod_ops
      (loop_abs bn256_params)
      (Z.to_nat (Z.log2 (loop_abs bn256_params)))
      Px Py Qx Qy
      (fp12_one bn256_zmod_ops) Qx Qy in
  PairingSpec.apply_corrections
    bn256_zmod_ops
    (zfp2_conj bn256_p_val)
    (zfp2_mul_const bn256_p_val)
    (optimal_ate_extras bn256_params)
    f Tx Ty Px Py Qx Qy
    gamma1 gamma_y gamma1_p2.

(** Local copy of [MillerLoopWP.bn256_optimal_ate_spec] (see note
...
Definition bn256_optimal_ate_spec
  ...
  PairingSpec.optimal_ate
    bn256_zmod_ops
    (zfp12_conj bn256_p_val)
    (zfp12_inv bn256_p_val bn256_xi_val)
    (zfp12_frob_p2 bn256_p_val bn256_xi_val)
    (zfp12_pow bn256_p_val bn256_xi_val)
    (zfp2_conj bn256_p_val)
    (zfp2_mul_const bn256_p_val)
    bn256_params
    gamma1 gamma_y gamma1_p2
...
Definition bn256_fp2_eval (v : rust_val TFp2) : Fp2_Z :=
  match v with
  | VFp2 a b => (bn256_fp_eval a, bn256_fp_eval b)
  end.

Definition bn256_fp6_eval (v : rust_val TFp6) : Fp6_Z :=
  match v with
  | VFp6 c0 c1 c2 =>
      (bn256_fp2_eval c0, bn256_fp2_eval c1, bn256_fp2_eval c2)
  end.

Definition bn256_fp12_eval (v : rust_val TFp12) : Fp12_Z :=
  match v with
  | VFp12 c0 c1 => (bn256_fp6_eval c0, bn256_fp6_eval c1)
  end.

(* ================================================================ *)
(* §2. BN256 CallEnv (concrete Definition, not Parameter)            *)
(* ================================================================ *)

Fixpoint bn256_bind_params_aux
  ...
  | p :: ps, a :: as_ =>
      ...
      | Some (existT t v) =>
          bn256_bind_params_aux ps as_ rs_caller
...
Definition bn256_bind_params
           (params : list var) (args : list located) (rs : rust_state)
  : option rust_state :=
  bn256_bind_params_aux params args rs rs_empty.

Definition bn256_extract_output
...
Definition bn256_writeback_output
...
Definition bn256_call_env : CallEnv :=
  {| ce_bind_params     := bn256_bind_params;
     ce_extract_output  := bn256_extract_output;
     ce_writeback_output := bn256_writeback_output |}.

(* ================================================================ *)
(* §3. Concrete function environment                                 *)
(*                                                                   *)
(*   The bodies [bn256_*_body] are still [Parameter]s at this step   *)
(*   — they will be filled in by applying [btranslate] to the        *)
(*   bedrock2 source in Steps 3-4.  What Step 1 buys us is:          *)
(*                                                                   *)
(*     ∙ [bn256_pairing_fenv] is a concrete list literal, so         *)
(*       [fenv_lookup bn256_pairing_fenv "bn256_..."] reduces by     *)
(*       [cbv] to the expected [Some (params, body)].                *)
(*                                                                   *)
(*     ∙ The 5 [fenv_has_*] axioms in BN256_PairingRust.v become     *)
...
(** Concrete loader body template: 8 [RLimbStore]s laying down the
    ...
    body shape instantiates to any BN256 Frobenius constant. *)
(** Concrete loader body template: 10 [RLimbStore]s laying down the
    [N = 5] limbs of the two [Fp] components of an [Fp2] result. *)
Definition bn256_loader_body
           (l00 l01 l02 l03 l04 l10 l11 l12 l13 l14 : nat) : rust_cmd :=
        ...
        (RSeq (RLimbStore loaders_out_c1 1 (SLit l11))
         (RSeq (RLimbStore loaders_out_c1 2 (SLit l12))
          (RSeq (RLimbStore loaders_out_c1 3 (SLit l13))
                (RLimbStore loaders_out_c1 4 (SLit l14)))))))))).

(** Loader parameters: 10 limbs per Frobenius constant (5 per Fp component). *)
Parameter bn256_gamma1_l00 bn256_gamma1_l01 bn256_gamma1_l02 bn256_gamma1_l03
          bn256_gamma1_l04
          bn256_gamma1_l10 bn256_gamma1_l11 bn256_gamma1_l12 bn256_gamma1_l13
          bn256_gamma1_l14
        : nat.
Parameter bn256_gamma_y_l00 bn256_gamma_y_l01 bn256_gamma_y_l02 bn256_gamma_y_l03
          bn256_gamma_y_l04
          bn256_gamma_y_l10 bn256_gamma_y_l11 bn256_gamma_y_l12 bn256_gamma_y_l13
          bn256_gamma_y_l14
        : nat.
Parameter bn256_gamma1_p2_l00 bn256_gamma1_p2_l01 bn256_gamma1_p2_l02 bn256_gamma1_p2_l03
          bn256_gamma1_p2_l04
          bn256_gamma1_p2_l10 bn256_gamma1_p2_l11 bn256_gamma1_p2_l12 bn256_gamma1_p2_l13
          bn256_gamma1_p2_l14
        : nat.

Definition bn256_load_g1_body : rust_cmd :=
  bn256_loader_body
    bn256_gamma1_l00 bn256_gamma1_l01 bn256_gamma1_l02 bn256_gamma1_l03 bn256_gamma1_l04
    bn256_gamma1_l10 bn256_gamma1_l11 bn256_gamma1_l12 bn256_gamma1_l13 bn256_gamma1_l14.
Definition bn256_load_g2_body : rust_cmd :=
  bn256_loader_body
    bn256_gamma_y_l00 bn256_gamma_y_l01 bn256_gamma_y_l02 bn256_gamma_y_l03 bn256_gamma_y_l04
    bn256_gamma_y_l10 bn256_gamma_y_l11 bn256_gamma_y_l12 bn256_gamma_y_l13 bn256_gamma_y_l14.
Definition bn256_load_w_body : rust_cmd :=
  bn256_loader_body
    bn256_gamma1_p2_l00 bn256_gamma1_p2_l01 bn256_gamma1_p2_l02 bn256_gamma1_p2_l03 bn256_gamma1_p2_l04
    bn256_gamma1_p2_l10 bn256_gamma1_p2_l11 bn256_gamma1_p2_l12 bn256_gamma1_p2_l13 bn256_gamma1_p2_l14.
...
(** Concrete bodies for the miller loop and final exponentiation.
    ...
    2. Build [bn256_finalexp_body] as a chain of [RCall]s into
       [bn256_Fp12_*] tower ops, each itself a [RCall] chain into
       [bn256_Fp6_*] / [bn256_Fp2_*] / [bn256_Fp_*] until reaching
       leaf operations.
    3. Build [bn256_miller_body] as a [RWhileNz] over a loop counter,
...
Definition bn256_miller_body : rust_cmd :=
  RLimbStore miller_dummy_loc 0 (SLit 0).
Definition bn256_finalexp_body : rust_cmd :=
  RLimbStore miller_dummy_loc 0 (SLit 0).

(** Parameter lists as used in the bedrock2 sources. *)
Definition bn256_load_g1_params : list var := ["out"].
Definition bn256_load_g2_params : list var := ["out"].
Definition bn256_load_w_params  : list var := ["out"].
Definition bn256_miller_params  : list var :=
  ["tmp"; "px"; "py"; "qx"; "qy"].
Definition bn256_finalexp_params : list var :=
  ["out"; "f"; "g1"; "g2"; "w"].

Definition bn256_pairing_fenv : rust_fenv :=
  [ ("bn256_load_gamma1_p2",      (bn256_load_g1_params,
                                   bn256_load_g1_body));
    ("bn256_load_gamma2_p2",      (bn256_load_g2_params,
                                   bn256_load_g2_body));
    ("bn256_load_w_frob_p2_c1",   (bn256_load_w_params,
                                   bn256_load_w_body));
    ("bn256_miller_loop_optimal", (bn256_miller_params,
                                   bn256_miller_body));
    ("bn256_final_exp_dsd",       (bn256_finalexp_params,
                                   bn256_finalexp_body)) ].
  ...
  Definition mid_ml : spec_t :=
    fun rs =>
      ...
      exists ml_val px py qx qy,
        ...
        bn256_fp12_eval ml_val =
          bn256_miller_loop_with_corrections
            gamma1 gamma_y gamma1_p2
            (bn256_fp_eval px) (bn256_fp_eval py)
            (bn256_fp2_eval qx) (bn256_fp2_eval qy).

  (** [pairing_post]: after final_exp, [out_loc] holds the full
      optimal-ate pairing value.  Matches the G2 strong spec
      postcondition (see BN256_PairingCorrect.v:387-390). *)
  Definition pairing_post : spec_t :=
    fun rs =>
      exists out px py qx qy,
        ...
        extract_at q_y_loc TFp2 rs = Some qy /\
        bn256_fp12_eval out =
          bn256_optimal_ate_spec gamma1 gamma_y gamma1_p2
            (bn256_fp_eval px) (bn256_fp_eval py)
            (bn256_fp2_eval qx) (bn256_fp2_eval qy).
...
Lemma fenv_has_pairing_load_g1 :
  fenv_lookup bn256_pairing_fenv "bn256_load_gamma1_p2" =
    Some (bn256_load_g1_params, bn256_load_g1_body).
Proof. reflexivity. Qed.

Lemma fenv_has_pairing_load_g2 :
  fenv_lookup bn256_pairing_fenv "bn256_load_gamma2_p2" =
    Some (bn256_load_g2_params, bn256_load_g2_body).
Proof. reflexivity. Qed.

Lemma fenv_has_pairing_load_w :
  fenv_lookup bn256_pairing_fenv "bn256_load_w_frob_p2_c1" =
    Some (bn256_load_w_params, bn256_load_w_body).
Proof. reflexivity. Qed.

Lemma fenv_has_pairing_miller :
  fenv_lookup bn256_pairing_fenv "bn256_miller_loop_optimal" =
    Some (bn256_miller_params, bn256_miller_body).
Proof. reflexivity. Qed.

Lemma fenv_has_pairing_finalexp :
  fenv_lookup bn256_pairing_fenv "bn256_final_exp_dsd" =
    Some (bn256_finalexp_params, bn256_finalexp_body).
Proof. reflexivity. Qed.
  ...
  Definition finalexp_callee_post (f_v : Fp12_Z) : spec_t :=
    fun rs =>
      exists out,
        ...
        bn256_fp12_eval out =
          PairingSpec.final_exp bn256_zmod_ops
            (zfp12_conj bn256_p_val)
            (zfp12_inv bn256_p_val bn256_xi_val)
            (zfp12_frob_p2 bn256_p_val bn256_xi_val)
            (zfp12_pow bn256_p_val bn256_xi_val)
            (prime_p bn256_params) (scalar_r bn256_params)
            f_v.
...
(*     [bn256_call_env] from §2.  These proofs use only [destruct],  *)
(*     [exists], and the concrete definitions of [extract_at] /      *)
(*     [bn256_bind_params] / [bn256_extract_output] /                *)
(*     [bn256_writeback_output].                                     *)
(* ================================================================ *)

(* ================================================================ *)
(* §7.5 Loader body execution lemmas                                 *)
(*                                                                   *)
(*   Structural facts about [bn256_loader_body]: executing the 8    *)
...
Lemma limb_store_c0_effect :
  forall k v rs rs' vc0 vc1,
    ...
    rust_exec_fenv bn256_N bn256_u64_max bn256_pairing_fenv
                   bn256_leaf_spec_concrete bn256_call_env
...
(** Full loader body effect: 10 [RLimbStore]s transform an [Fp2] value
    whose two components are 5-limb lists into one where each limb
    matches the corresponding parameter.  The length-5 assumption
    captures the BN256 limb count [bn256_N = 5]. *)
Lemma bn256_loader_body_effect :
  forall l00 l01 l02 l03 l04 l10 l11 l12 l13 l14 rs rs' lc0_init lc1_init,
    length lc0_init = 5%nat -> length lc1_init = 5%nat ->
    extract_at (loc_Fp2 "out") TFp2 rs = Some (VFp2 (VFp lc0_init) (VFp lc1_init)) ->
    rust_exec_fenv bn256_N bn256_u64_max bn256_pairing_fenv
                   bn256_leaf_spec_concrete bn256_call_env
                   (bn256_loader_body l00 l01 l02 l03 l04
                                      l10 l11 l12 l13 l14) rs rs' ->
    extract_at (loc_Fp2 "out") TFp2 rs' =
      Some (VFp2 (VFp [l00; l01; l02; l03; l04])
                 (VFp [l10; l11; l12; l13; l14])).
Proof.
  intros l00 l01 l02 l03 l04 l10 l11 l12 l13 l14 rs rs'
         lc0_init lc1_init Hlen0 Hlen1 Hext Hexec.
  unfold bn256_loader_body in Hexec.
  ...
  inversion R8   as [| ? ? ? ? r9  H9  H10 | | | | | | | | | | ]; clear R8; subst.
  ...
  destruct lc0_init as [| a0 [| a1 [| a2 [| a3 [| a4 [| ??]]]]]];
    try (cbn in Hlen0; lia).
  cbn in E5.
  pose proof (limb_store_c1_effect _ _ _ _ _ _ E5   H6)  as E6.  cbn in E6.
  ...
  destruct lc1_init as [| b0 [| b1 [| b2 [| b3 [| b4 [| ??]]]]]];
...
Section PairingBodyRefines.
  ...
  (** Fresh-variable hypotheses: the 4 stackalloc names must not
      collide with the user-location variable names.  In BN256 the
  ...
  (** [load_*_body_refines]: discharged in Phase 3 by [inversion] on
      ...
      [bn256_loader_body_skeleton] is built from [RSeq] + [RLimbStore]
  ...
  (** Arithmetic hypotheses connecting the specific [bn256_gamma*_l**]
  ...
  Hypothesis Hgamma1_fp2_eval :
    bn256_fp2_eval
      (VFp2 (VFp [bn256_gamma1_l00; bn256_gamma1_l01; bn256_gamma1_l02;
                  bn256_gamma1_l03; bn256_gamma1_l04])
            (VFp [bn256_gamma1_l10; bn256_gamma1_l11; bn256_gamma1_l12;
                  bn256_gamma1_l13; bn256_gamma1_l14]))
    = gamma1.
  Hypothesis Hgamma_y_fp2_eval :
    bn256_fp2_eval
      (VFp2 (VFp [bn256_gamma_y_l00; bn256_gamma_y_l01; bn256_gamma_y_l02;
                  bn256_gamma_y_l03; bn256_gamma_y_l04])
            (VFp [bn256_gamma_y_l10; bn256_gamma_y_l11; bn256_gamma_y_l12;
                  bn256_gamma_y_l13; bn256_gamma_y_l14]))
    = gamma_y.
  Hypothesis Hgamma1_p2_fp2_eval :
    bn256_fp2_eval
      (VFp2 (VFp [bn256_gamma1_p2_l00; bn256_gamma1_p2_l01; bn256_gamma1_p2_l02;
                  bn256_gamma1_p2_l03; bn256_gamma1_p2_l04])
            (VFp [bn256_gamma1_p2_l10; bn256_gamma1_p2_l11; bn256_gamma1_p2_l12;
                  bn256_gamma1_p2_l13; bn256_gamma1_p2_l14]))
    = gamma1_p2.

  Hypothesis Hloader_input_len5 :
    forall rs,
      (exists v, extract_at (loc_Fp2 out_var) TFp2 rs = Some v) ->
      exists lc0 lc1,
        length lc0 = 5%nat /\ length lc1 = 5%nat /\
        extract_at (loc_Fp2 out_var) TFp2 rs =
          Some (VFp2 (VFp lc0) (VFp lc1)).

  Theorem load_g1_body_refines :
    rust_refines bn256_N bn256_u64_max bn256_pairing_fenv
                 bn256_leaf_spec_concrete bn256_call_env
                 load_g1_callee_pre bn256_load_g1_body
                 (load_g1_callee_post gamma1).
  Proof.
    unfold rust_refines, load_g1_callee_pre.
    intros rs1 rs2 Hpre Hexec.
    pose proof (Hloader_input_len5 _ Hpre) as [lc0 [lc1 [Hlen0 [Hlen1 Hext]]]].
    unfold load_g1_callee_post.
    eexists. split;
      [|exact Hgamma1_fp2_eval].
    unfold bn256_load_g1_body in Hexec.
    exact (bn256_loader_body_effect _ _ _ _ _ _ _ _ _ _ _ _ _ _
  ...
  Theorem load_g2_body_refines :
    rust_refines bn256_N bn256_u64_max bn256_pairing_fenv
                 bn256_leaf_spec_concrete bn256_call_env
                 load_g2_callee_pre bn256_load_g2_body
  ...
  Proof.
    ...
    unfold bn256_load_g2_body in Hexec.
    exact (bn256_loader_body_effect _ _ _ _ _ _ _ _ _ _ _ _ _ _
             Hlen0 Hlen1 Hext Hexec).
  Qed.
  ...
  Theorem load_w_body_refines :
    rust_refines bn256_N bn256_u64_max bn256_pairing_fenv
                 bn256_leaf_spec_concrete bn256_call_env
                 load_w_callee_pre bn256_load_w_body
  ...
  Proof.
    ...
    unfold bn256_load_w_body in Hexec.
    exact (bn256_loader_body_effect _ _ _ _ _ _ _ _ _ _ _ _ _ _
             Hlen0 Hlen1 Hext Hexec).
  Qed.
    ...
    forall px_v py_v qx_v qy_v,
      rust_refines bn256_N bn256_u64_max bn256_pairing_fenv
                   ...
                   (miller_callee_post gamma1 gamma_y gamma1_p2
                                       px_v py_v qx_v qy_v).
  Proof.
    intros; unfold rust_refines, bn256_miller_body.
  ...
  Proof.
    intros; unfold rust_refines, bn256_finalexp_body.
    intros rs1 rs2 _ Hexec. exfalso.
    inversion Hexec; discriminate.
  Qed.
  ...
  Definition pairing_calls : rust_cmd :=
    RSeq (RCall "bn256_load_gamma1_p2"      g1_loc [g1_loc])
      (RSeq (RCall "bn256_load_gamma2_p2"   g2_loc [g2_loc])
        (RSeq (RCall "bn256_load_w_frob_p2_c1" w_loc [w_loc])
          (RSeq (RCall "bn256_miller_loop_optimal" tmp_loc
                       [tmp_loc; p_x_loc; p_y_loc; q_x_loc; q_y_loc])
                (RCall "bn256_final_exp_dsd" out_loc
            ...
            (exists rs1,
               (exists rs0,
                  ...
                  rs1 = rs_set_tower rs0 tmp_var
                          (exist_tval TFp12 (tt_zero bn256_N TFp12)))
               /\ rs2 = rs_set_tower rs1 "gamma1_p2"
                          (exist_tval TFp2 (tt_zero bn256_N TFp2)))
  ...
  (*   [bn256_bind_params] uses (and vice versa for [bn256_extract_    *)
  ...
  (** Link from the high-level [extract_at] to the low-level
      [lookup_t] view used by [bn256_bind_params].  The [eq_rect]
  ...
  (*   [bn256_extract_output] + [bn256_writeback_output] to land in   *)
  (*   [mid1_sa]).                                                    *)
  (* ================================================================ *)

  Lemma compose_load_g1 :
    call_composes_from bn256_N bn256_u64_max bn256_pairing_fenv
                       bn256_leaf_spec_concrete bn256_call_env
                       "bn256_load_gamma1_p2"
                       bn256_load_g1_params bn256_load_g1_body
  ...
  Proof.
    ...
    - (* pre-bridge: pre_sa rs1 => in_vals + rs_init + callee_pre *)
      ...
      + (* bn256_bind_params *)
        unfold bn256_bind_params; cbn.
        unfold bn256_bind_params_aux; cbn.
    ...
    - (* post-bridge: caller_pre + callee_post + extract + writeback => mid1_sa *)
      ...
      (* bn256_extract_output ["out"] rs_mid = Some (existT _ TFp2 v_out) *)
      unfold ce_extract_output, bn256_call_env, bn256_extract_output,
             bn256_load_g1_params in Hext; cbn in Hext.
      rewrite Hout_ext in Hext.
      injection Hext as Hrs_out. subst rs_out_val.
      (* Hwb: bn256_writeback_output g1_loc (existT _ TFp2 v_out) rs1 = Some rs2.
         By the definition of bn256_writeback_output and the fact that
         loc_dst g1_loc = TFp2, this reduces to located_update rs1 g1_loc v_out. *)
      assert (Hwb' : located_update rs1 g1_loc v_out = Some rs2).
      { unfold ce_writeback_output, bn256_call_env,
               bn256_writeback_output in Hwb.
        cbn in Hwb.
  ...
  Lemma compose_load_g2 :
    call_composes_from bn256_N bn256_u64_max bn256_pairing_fenv
                       ...
                       bn256_load_g2_params bn256_load_g2_body
  ...
  Proof.
    ...
    - (* post-bridge *)
      ...
      unfold out_var in Hout_ext.
      unfold ce_extract_output, bn256_call_env, bn256_extract_output,
             bn256_load_g2_params in Hext; cbn in Hext.
      rewrite Hout_ext in Hext.
      injection Hext as Hrs_out. subst rs_out_val.
  ...
  Lemma compose_load_w :
    call_composes_from bn256_N bn256_u64_max bn256_pairing_fenv
                       ...
                       bn256_load_w_params bn256_load_w_body
  ...
  Proof.
    ...
    - (* post-bridge *)
      ...
      unfold out_var in Hout_ext.
      unfold ce_extract_output, bn256_call_env, bn256_extract_output,
             bn256_load_w_params in Hext; cbn in Hext.
      rewrite Hout_ext in Hext.
      injection Hext as Hrs_out. subst rs_out_val.
  ...
  Lemma compose_miller :
    forall px_v py_v qx_v qy_v,
      call_composes_from bn256_N bn256_u64_max bn256_pairing_fenv
                         bn256_leaf_spec_concrete bn256_call_env
                         "bn256_miller_loop_optimal"
                         bn256_miller_params bn256_miller_body
                         (miller_callee_pre px_v py_v qx_v qy_v)
                         (miller_callee_post gamma1 gamma_y gamma1_p2
                                             px_v py_v qx_v qy_v)
                         tmp_loc
  ...
  Proof.
    ...
    - (* pre-bridge *)
      ...
      + (* bn256_bind_params *)
        unfold bn256_bind_params, bn256_bind_params_aux,
               bn256_miller_params; cbn.
    ...
    - (* post-bridge *)
      ...
      unfold ce_extract_output, bn256_call_env, bn256_extract_output,
             bn256_miller_params in Hext; cbn in Hext.
      ...
      + (* mid_ml rs2 *)
        ...
        * (* ml witness *)
          (* Need: extract_at tmp_loc TFp12 rs2 = Some ml_val /\
                   ...
                   bn256_fp12_eval ml_val = miller_with_corrections ... *)
          ...
          -- erewrite extract_at_writeback_other;
               [exact Hqy_rs1
                | rewrite loc_var_tmp_loc; exact Hqy_t
                | exact Hwb'].
          -- (* bn256_fp12_eval v_tmp_out =
                miller_with_corrections(bn256_fp_eval v_px_rs1, ...).
                Chain: Htmp_eval gives this for (px_v, py_v, qx_v, qy_v).
                The caller-provided equations Hpx_eq : bn256_fp_eval v_px_in = px_v
                plus determinism of extract_at (Hpx_ext_in vs Hpx_rs1 both at p_x_loc) give
                bn256_fp_eval v_px_rs1 = px_v. *)
  ...
  Lemma miller_call_refines :
    rust_refines bn256_N bn256_u64_max bn256_pairing_fenv
                 bn256_leaf_spec_concrete bn256_call_env
                 mid3_sa
                 (RCall "bn256_miller_loop_optimal" tmp_loc
                        [tmp_loc; p_x_loc; p_y_loc; q_x_loc; q_y_loc])
                 mid4_sa.
  Proof.
    unfold rust_refines.
    ...
    destruct Hpre_plain as [_ [[_ [v_px Hpx_ext]]
                                [[_ [v_py Hpy_ext]]
                                 [[_ [v_qx Hqx_ext]]
                                  [_ [v_qy Hqy_ext]]]]]].
    set (px_v := bn256_fp_eval v_px).
    set (py_v := bn256_fp_eval v_py).
    set (qx_v := bn256_fp2_eval v_qx).
    set (qy_v := bn256_fp2_eval v_qy).
  ...
  (*                                                                   *)
  (*   For BN256 (loop_neg = false), the [bn256_optimal_ate_spec]     *)
  ...
  Lemma bn256_optimal_ate_decomposes :
    forall px py qx qy,
      ...
      PairingSpec.final_exp bn256_zmod_ops
        ...
        (zfp12_pow bn256_p_val bn256_xi_val)
        (prime_p bn256_params) (scalar_r bn256_params)
        (bn256_miller_loop_with_corrections gamma1 gamma_y gamma1_p2
                                            px py qx qy).
  Proof.
    intros.
    unfold bn256_optimal_ate_spec, bn256_miller_loop_with_corrections,
           PairingSpec.optimal_ate.
    (* [loop_neg bn256_params = false] by [vm_compute]. *)
    assert (Hneg : loop_neg bn256_params = false) by reflexivity.
    rewrite Hneg.
    destruct (affine_miller_aux bn256_zmod_ops (loop_abs bn256_params)
               (Z.to_nat (Z.log2 (loop_abs bn256_params)))
               px py qx qy (fp12_one bn256_zmod_ops) qx qy)
             as [[f Tx] Ty].
    reflexivity.
  Qed.
  ...
  Lemma compose_finalexp :
    forall f_v,
      call_composes_from bn256_N bn256_u64_max bn256_pairing_fenv
                         bn256_leaf_spec_concrete bn256_call_env
                         "bn256_final_exp_dsd"
                         bn256_finalexp_params bn256_finalexp_body
                         (finalexp_callee_pre f_v)
                         (finalexp_callee_post f_v)
                         out_loc
                         [out_loc; tmp_loc; g1_loc; g2_loc; w_loc]
  ...
  Proof.
    ...
    - (* post-bridge *)
      ...
      unfold out_var in Hout_ext.
      unfold ce_extract_output, bn256_call_env, bn256_extract_output,
             bn256_finalexp_params in Hext; cbn in Hext.
      rewrite Hout_ext in Hext.
      injection Hext as Hrs_out. subst rs_out_val.
      ...
      (* Math chain: tmp_loc in rs1 holds v_f (from Hf_ext_rs1) AND v_ml
         (from Hml_tmp). So v_f = v_ml. Then f_v = bn256_fp12_eval v_ml =
      ...
      + exact Hqy_rs2.
      + (* Math equation *)
        rewrite bn256_optimal_ate_decomposes.
        rewrite Hout_eval.
        f_equal.
  ...
  Theorem bn256_pairing_body_refines :
    rust_refines bn256_N bn256_u64_max bn256_pairing_fenv
                 bn256_leaf_spec_concrete bn256_call_env
  ...
  Theorem bn256_pairing_rust_correct :
    rust_refines bn256_N bn256_u64_max bn256_pairing_fenv
                 bn256_leaf_spec_concrete bn256_call_env
                 pre pairing_body post.
  Proof. exact bn256_pairing_body_refines. Qed.
  ...
  (** End-to-end corollary: for any post-pairing state [rs2] reached
      ...
      This is the Step 5 wiring: it packages [bn256_pairing_rust_correct]
      in the style of [BN256_EndToEnd.bn256_pairing_end_to_end], yielding
      a direct math equality at the exit. *)
  Theorem bn256_pairing_end_to_end_conditional :
    forall rs1 rs2,
      pre rs1 ->
      rust_exec_fenv bn256_N bn256_u64_max bn256_pairing_fenv
                     bn256_leaf_spec_concrete bn256_call_env
  ...
  Proof.
    intros rs1 rs2 Hpre Hexec.
    exact (bn256_pairing_rust_correct rs1 rs2 Hpre Hexec).
  Qed.

End PairingBodyRefines.

(* ================================================================ *)
(* §8bis.  Concrete BN256 pairing entry point                        *)
(*                                                                   *)
...
(*   1× [Hloader_input_len5] (structural, discharged when the caller *)
...
Definition bn256_out_loc  : located := loc_Fp12 "pout".
Definition bn256_p_x_loc  : located := loc_Fp   "px".
Definition bn256_p_y_loc  : located := loc_Fp   "py".
Definition bn256_q_x_loc  : located := loc_Fp2  "qx".
Definition bn256_q_y_loc  : located := loc_Fp2  "qy".

(** The 16 architectural hypotheses, each discharged by
    [reflexivity] or [repeat split; discriminate]. *)
Lemma bn256_Hout_Fp12 : loc_dst bn256_out_loc = TFp12. Proof. reflexivity. Qed.
Lemma bn256_Hpx_Fp    : loc_dst bn256_p_x_loc = TFp.   Proof. reflexivity. Qed.
Lemma bn256_Hpy_Fp    : loc_dst bn256_p_y_loc = TFp.   Proof. reflexivity. Qed.
Lemma bn256_Hqx_Fp2   : loc_dst bn256_q_x_loc = TFp2.  Proof. reflexivity. Qed.
Lemma bn256_Hqy_Fp2   : loc_dst bn256_q_y_loc = TFp2.  Proof. reflexivity. Qed.

Lemma bn256_Hfresh_out :
  loc_var bn256_out_loc <> tmp_var /\
  loc_var bn256_out_loc <> "gamma1_p2" /\
  loc_var bn256_out_loc <> "gamma2_p2" /\
  loc_var bn256_out_loc <> "w_frob_p2_c1".
Proof. repeat split; discriminate. Qed.
Lemma bn256_Hfresh_px :
  loc_var bn256_p_x_loc <> tmp_var /\
  loc_var bn256_p_x_loc <> "gamma1_p2" /\
  loc_var bn256_p_x_loc <> "gamma2_p2" /\
  loc_var bn256_p_x_loc <> "w_frob_p2_c1".
Proof. repeat split; discriminate. Qed.
Lemma bn256_Hfresh_py :
  loc_var bn256_p_y_loc <> tmp_var /\
  loc_var bn256_p_y_loc <> "gamma1_p2" /\
  loc_var bn256_p_y_loc <> "gamma2_p2" /\
  loc_var bn256_p_y_loc <> "w_frob_p2_c1".
Proof. repeat split; discriminate. Qed.
Lemma bn256_Hfresh_qx :
  loc_var bn256_q_x_loc <> tmp_var /\
  loc_var bn256_q_x_loc <> "gamma1_p2" /\
  loc_var bn256_q_x_loc <> "gamma2_p2" /\
  loc_var bn256_q_x_loc <> "w_frob_p2_c1".
Proof. repeat split; discriminate. Qed.
Lemma bn256_Hfresh_qy :
  loc_var bn256_q_y_loc <> tmp_var /\
  loc_var bn256_q_y_loc <> "gamma1_p2" /\
  loc_var bn256_q_y_loc <> "gamma2_p2" /\
  loc_var bn256_q_y_loc <> "w_frob_p2_c1".
Proof. repeat split; discriminate. Qed.

Lemma bn256_Hout_shape : bn256_out_loc = loc_Fp12 (loc_var bn256_out_loc).
Proof. reflexivity. Qed.
Lemma bn256_Hpx_shape  : bn256_p_x_loc = loc_Fp   (loc_var bn256_p_x_loc).
Proof. reflexivity. Qed.
Lemma bn256_Hpy_shape  : bn256_p_y_loc = loc_Fp   (loc_var bn256_p_y_loc).
Proof. reflexivity. Qed.
Lemma bn256_Hqx_shape  : bn256_q_x_loc = loc_Fp2  (loc_var bn256_q_x_loc).
Proof. reflexivity. Qed.
Lemma bn256_Hqy_shape  : bn256_q_y_loc = loc_Fp2  (loc_var bn256_q_y_loc).
Proof. reflexivity. Qed.

Lemma bn256_Hdistinct_out_users :
  loc_var bn256_out_loc <> loc_var bn256_p_x_loc /\
  loc_var bn256_out_loc <> loc_var bn256_p_y_loc /\
  loc_var bn256_out_loc <> loc_var bn256_q_x_loc /\
  loc_var bn256_out_loc <> loc_var bn256_q_y_loc.
Proof. repeat split; discriminate. Qed.
...
Theorem bn256_pairing_rust_correct_concrete :
  forall (gamma1 gamma_y gamma1_p2 : Fp2_Z),
  bn256_fp2_eval
    (VFp2 (VFp [bn256_gamma1_l00; bn256_gamma1_l01; bn256_gamma1_l02;
                bn256_gamma1_l03; bn256_gamma1_l04])
          (VFp [bn256_gamma1_l10; bn256_gamma1_l11; bn256_gamma1_l12;
                bn256_gamma1_l13; bn256_gamma1_l14])) = gamma1 ->
  bn256_fp2_eval
    (VFp2 (VFp [bn256_gamma_y_l00; bn256_gamma_y_l01; bn256_gamma_y_l02;
                bn256_gamma_y_l03; bn256_gamma_y_l04])
          (VFp [bn256_gamma_y_l10; bn256_gamma_y_l11; bn256_gamma_y_l12;
                bn256_gamma_y_l13; bn256_gamma_y_l14])) = gamma_y ->
  bn256_fp2_eval
    (VFp2 (VFp [bn256_gamma1_p2_l00; bn256_gamma1_p2_l01; bn256_gamma1_p2_l02;
                bn256_gamma1_p2_l03; bn256_gamma1_p2_l04])
          (VFp [bn256_gamma1_p2_l10; bn256_gamma1_p2_l11; bn256_gamma1_p2_l12;
                bn256_gamma1_p2_l13; bn256_gamma1_p2_l14])) = gamma1_p2 ->
  (forall rs,
     (exists v, extract_at (loc_Fp2 out_var) TFp2 rs = Some v) ->
     exists lc0 lc1,
       length lc0 = 5%nat /\ length lc1 = 5%nat /\
  ...
  rust_refines bn256_N bn256_u64_max bn256_pairing_fenv
               bn256_leaf_spec_concrete bn256_call_env
               (pairing_pre bn256_out_loc bn256_p_x_loc bn256_p_y_loc
                            bn256_q_x_loc bn256_q_y_loc)
               (pairing_body bn256_out_loc bn256_p_x_loc bn256_p_y_loc
                             bn256_q_x_loc bn256_q_y_loc)
               (pairing_post bn256_out_loc bn256_p_x_loc bn256_p_y_loc
                             bn256_q_x_loc bn256_q_y_loc
                             gamma1 gamma_y gamma1_p2).
Proof.
  intros gamma1 gamma_y gamma1_p2 Hg1 Hgy Hgw Hlen5.
  apply (bn256_pairing_rust_correct
           bn256_out_loc bn256_p_x_loc bn256_p_y_loc
           bn256_q_x_loc bn256_q_y_loc
           gamma1 gamma_y gamma1_p2);
    first [ exact bn256_Hout_Fp12
          | exact bn256_Hpx_Fp
          | exact bn256_Hpy_Fp
          | exact bn256_Hqx_Fp2
          | exact bn256_Hqy_Fp2
          | exact bn256_Hfresh_out
          | exact bn256_Hfresh_px
          | exact bn256_Hfresh_py
          | exact bn256_Hfresh_qx
          | exact bn256_Hfresh_qy
          | exact bn256_Hout_shape
          | exact bn256_Hpx_shape
          | exact bn256_Hpy_shape
          | exact bn256_Hqx_shape
          | exact bn256_Hqy_shape
          | exact bn256_Hdistinct_out_users
          | exact Hg1 | exact Hgy | exact Hgw | exact Hlen5 ].
Qed.
...
(** [Print Assumptions bn256_pairing_end_to_end_conditional] reports
    ...
    Phase 1 (composition, DONE):
      ...
      [pairing_calls_refines] and [bn256_pairing_body_refines]
    ...
    [bn256_miller_body] / [bn256_finalexp_body] above for the
    roadmap.  [bn256_optimal_ate_decomposes] (previously a
    Hypothesis) is now Qed by [reflexivity] on
    [loop_neg bn256_params = false]. *)
