(* BW6-761 curve integration into AUCurves Montgomery framework.
   Instantiates MontgomeryCurveSpecs for y^2 = x^3 - 1 over the
   BW6-761 prime field (761-bit prime, 12 x 64-bit limbs). *)
...
(* BW6-761 prime (761 bits) *)
Local Definition m := 6891450384315732539396789682275657542479668912536150109513790160209623422243491736087683183289411687640864567753786613451161759120554247759349511699125301598951605099378508850372543631423596795951899700429969112842764913119068299.
Local Definition bw := 64.
Local Definition n := 12%nat.

(* Curve equation: y^2 = x^3 - 1  (a = 0, b = m - 1) *)
Local Definition a := 0%Z.
Local Definition b_coeff := Eval vm_compute in (m - 1)%Z.
Local Definition three_b := Eval vm_compute in (m - 3)%Z.
...
(* Instantiate Montgomery curve specs for BW6-761 *)
Definition bw6_761_three_b_list := MontgomeryCurveSpecs.three_b_list bw n three_b.
Definition bw6_761_three_b_mont := Eval vm_compute in
  (MontgomeryCurveSpecs.three_b_mont_list m bw n m' three_b).
Definition bw6_761_a_list := MontgomeryCurveSpecs.a_list bw n a.
Definition bw6_761_a_mont_list := Eval vm_compute in
  (MontgomeryCurveSpecs.a_mont_list m bw n m' a).

(* Validity lemmas *)
Lemma bw6_761_three_b_list_valid : WordByWordMontgomery.valid bw n m bw6_761_three_b_list.
Proof.
...
(* Gallina specification of BW6-761 point addition (projective coordinates).
   This is the generic spec from MontgomeryCurveSpecs, instantiated for BW6-761. *)
...
Definition BW6_761_add_Gallina_spec_a_0 X1 Y1 Z1 X2 Y2 Z2 outx outy outz :=
  ...
  let X3 := t1+'t2 in
  let t5 := t5-'X3 in
  let Z3 := eval bw6_761_three_b_list *'t2 in
  let X3 := t1-'Z3 in
  let Z3 := t1+'Z3 in
  let Y3 := X3*'Z3 in
  let t1 := t0+'t0 in
  let t1 := t1+'t0 in
  let t4 := eval bw6_761_three_b_list *'t4 in
  let t0 := t1*'t4 in
  let Y3 := Y3+'t0 in
  let t0 := t5*'t4 in
  let X3 := t3*'X3 in
...
(* The generic and a=0 specialized specs are equivalent for BW6-761. *)
...
Proof.
  ...
  change (three_b_list bw n three_b) with bw6_761_three_b_list.
  remember (eval bw6_761_three_b_list) as tb.
