(** * P-256 (secp256r1) Fp inversion via Bernstein-Yang divstep algorithm.

    Mirrors BLS12_FpInv.v for the NIST P-256 base field.
...
(* p256_prime.v defines p256_modulus inside `Section Field`; since the section
   has no Variable, the definition is accessible at top level. *)
...
(* ================================================================== *)
(* P-256 prime and its parameters                                      *)
(* ================================================================== *)

(** The P-256 base field prime. *)
Definition p256_p : Z :=
  Eval vm_compute in p256_modulus.
...
Definition p256_mbits : Z := Z.log2 p256_p + 1.

Lemma p256_mbits_val : p256_mbits = 256.
Proof. vm_compute. reflexivity. Qed.

Lemma p256_mbits_ge_46 : 46 <= p256_mbits.
...
Definition p256_divstep_iters : Z :=
  Eval vm_compute in ((49 * p256_mbits + 57) / 17).

Lemma p256_divstep_iters_val : p256_divstep_iters = 741.
Proof. vm_compute. reflexivity. Qed.

Lemma p256_divstep_iters_eq_precomp_exp :
  p256_divstep_iters =
  (if Decidable.dec (p256_mbits < 46) then (49 * p256_mbits + 80) / 17
   else (49 * p256_mbits + 57) / 17).
Proof.
...
Section FpInv.
  ...
  (** Precomputation: ((p+1)/2)^N mod p with N = 741.
      Computed externally via:
        python3 -c "p=2**256-2**224+2**192+2**96-1; print(hex(pow((p+1)//2, 741, p)))"
  *)
  Definition precomp_val : Z :=
    0xc7fffffe68000001a800000007fffffea00000026ffffffed7ffffff98000001.

  Definition fp_inv_spec (x : Z) : Z :=
    let '(d, f, g, v, r) :=
      iter_divstep_spec p (Z.to_nat p256_divstep_iters) 1 p x 0 1 in
    let v_corrected := if (f <? 0) then (p - v) mod p else v mod p in
    (v_corrected * precomp_val) mod p.

  Axiom by_convergence_dfg_p256 : forall x,
    0 < x < p ->
    Z.gcd x p = 1 ->
    let '(_, f_N, g_N) :=
      iter_divstep_dfg (Z.to_nat p256_divstep_iters) 1 p x in
    g_N = 0 /\ (f_N = 1 \/ f_N = -1).
  ...
  Lemma precomp_times_pow2 :
    (precomp_val * 2 ^ p256_divstep_iters) mod p = 1.
  Proof.
    subst p. unfold precomp_val, p256_divstep_iters, p256_p.
    vm_compute. reflexivity.
  Qed.

  Lemma p256_coprime : forall x, 0 < x < p -> Z.gcd x p = 1.
  Proof.
    intros x Hx.
    apply Zgcd_1_rel_prime.
    apply rel_prime_sym.
    apply prime_rel_prime.
    - subst p. unfold p256_p. exact prime_p256.
    - intros [k Hk]. subst x.
  ...
  Proof.
    intros x Hx Hgcd.
    unfold fp_inv_spec.
    destruct (iter_divstep_spec p (Z.to_nat p256_divstep_iters) 1 p x 0 1)
      as [[[[d_N f_N] g_N] v_N] r_N] eqn:Hiter.
    pose proof (iter_invariant p (Z.to_nat p256_divstep_iters) 1 p x 0 1 x
                  p_pos p_odd) as Hinv.
    ...
    pose proof (by_convergence_dfg_p256 x Hx Hgcd) as Hconv.
    pose proof (iter_dfg_agree (Z.to_nat p256_divstep_iters) p 1 p x 0 1) as Hagree.
    rewrite Hiter in Hagree.
    destruct (iter_divstep_dfg (Z.to_nat p256_divstep_iters) 1 p x)
    ...
    - replace (1 <? 0) with false by reflexivity.
      ...
      assert (Hmod : (v_N * x) mod p = (2 ^ Z.of_nat (Z.to_nat p256_divstep_iters)) mod p). {
        assert (Hp : p <> 0) by (pose proof p_pos; lia).
        apply Z.mod_divide in Hv_inv; [|exact Hp].
        destruct Hv_inv as [k Hk].
        assert (v_N * x = 2 ^ Z.of_nat (Z.to_nat p256_divstep_iters) + k * p) by lia.
        rewrite H.
        rewrite Zplus_mod, Z_mod_mult, Z.add_0_r, Z.mod_mod by lia.
        reflexivity.
      }
      rewrite <- Zmult_mod_idemp_r, Hmod, Zmult_mod_idemp_r.
      rewrite Z2Nat.id by (unfold p256_divstep_iters; vm_compute; discriminate).
      exact precomp_times_pow2.
    - replace (-1 <? 0) with true by reflexivity.
      replace (2 ^ Z.of_nat (Z.to_nat p256_divstep_iters) * -1)
        with (- (2 ^ Z.of_nat (Z.to_nat p256_divstep_iters))) in Hv_inv by ring.
      replace (v_N * x - - 2 ^ Z.of_nat (Z.to_nat p256_divstep_iters))
        with (v_N * x + 2 ^ Z.of_nat (Z.to_nat p256_divstep_iters)) in Hv_inv by ring.
      set (iters := Z.of_nat (Z.to_nat p256_divstep_iters)) in *.
  ...
  Theorem fp_inv_correct : forall x,
    ...
    (fp_inv_spec x * x) mod p = 1.
  Proof.
    intros x Hx. apply fp_inv_correct_ax; auto. apply p256_coprime; auto.
  Qed.

End FpInv.

(* ================================================================== *)
(* Concrete parameters for P-256 divstep                               *)
(* ================================================================== *)
...
Lemma p256_p_pos : 1 < p256_p.
Proof. vm_compute. reflexivity. Qed.

Lemma p256_p_fits : p256_p < (2^machine_wordsize) ^ Z.of_nat n.
Proof. vm_compute. reflexivity. Qed.

Lemma p256_p_odd : Z.odd p256_p = true.
Proof. vm_compute. reflexivity. Qed.
