(** * BLS12-377 Fp inversion via Bernstein-Yang divstep algorithm.

    Mirrors [BLS12_FpInv.v] / [BN254_FpInv.v] for the BLS12-377 base field.

    BLS12-377 base prime:
      p = ((u-1)^2 * (u^4 - u^2 + 1)) / 3 + u,  u = 0x8508c00000000001
      Z.log2 p + 1 = 377
      iterations = (49 * 377 + 57) / 17 = 18530 / 17 = 1090

    Word-by-word Montgomery layout: 6 limbs of 64 bits (saturated: 7 limbs).

    The algorithm and proof structure are identical to BLS12_FpInv.v;
...
(* ================================================================== *)
(* BLS12-377 prime and its parameters                                  *)
(* ================================================================== *)

(** The BLS12-377 base field prime. *)
Definition bls12_377_p : Z :=
  Eval vm_compute in bls12_377_modulus.
...
Definition bls12_377_mbits : Z := Z.log2 bls12_377_p + 1.

Lemma bls12_377_mbits_val : bls12_377_mbits = 377.
Proof. vm_compute. reflexivity. Qed.

Lemma bls12_377_mbits_ge_46 : 46 <= bls12_377_mbits.
...
Definition bls12_377_divstep_iters : Z :=
  Eval vm_compute in ((49 * bls12_377_mbits + 57) / 17).

Lemma bls12_377_divstep_iters_val : bls12_377_divstep_iters = 1090.
Proof. vm_compute. reflexivity. Qed.

Lemma bls12_377_divstep_iters_eq_precomp_exp :
  bls12_377_divstep_iters =
  (if Decidable.dec (bls12_377_mbits < 46) then (49 * bls12_377_mbits + 80) / 17
   else (49 * bls12_377_mbits + 57) / 17).
Proof.
...
Section FpInv.
  ...
  (** Precomputation: ((p+1)/2)^N mod p with N = 1090. *)
  Definition precomp_val : Z :=
    0x15f674509e87091f8fb26a5bb2a603362316863dce0ccc0113004a7beb602f95ba30b861c1c68353c135a7738f8b3a9.

  Definition fp_inv_spec (x : Z) : Z :=
    let '(d, f, g, v, r) :=
      iter_divstep_spec p (Z.to_nat bls12_377_divstep_iters) 1 p x 0 1 in
    let v_corrected := if (f <? 0) then (p - v) mod p else v mod p in
    (v_corrected * precomp_val) mod p.

  Axiom by_convergence_dfg_bls12_377 : forall x,
    0 < x < p ->
    Z.gcd x p = 1 ->
    let '(_, f_N, g_N) :=
      iter_divstep_dfg (Z.to_nat bls12_377_divstep_iters) 1 p x in
    g_N = 0 /\ (f_N = 1 \/ f_N = -1).
  ...
  Lemma precomp_times_pow2 :
    (precomp_val * 2 ^ bls12_377_divstep_iters) mod p = 1.
  Proof.
    subst p. unfold precomp_val, bls12_377_divstep_iters, bls12_377_p.
    vm_compute. reflexivity.
  Qed.

  Lemma bls12_377_coprime : forall x, 0 < x < p -> Z.gcd x p = 1.
  Proof.
    intros x Hx.
    apply Zgcd_1_rel_prime.
    apply rel_prime_sym.
    apply prime_rel_prime.
    - subst p. unfold bls12_377_p. exact prime_bls12_377.
    - intros [k Hk]. subst x.
  ...
  Proof.
    intros x Hx Hgcd.
    unfold fp_inv_spec.
    destruct (iter_divstep_spec p (Z.to_nat bls12_377_divstep_iters) 1 p x 0 1)
      as [[[[d_N f_N] g_N] v_N] r_N] eqn:Hiter.
    pose proof (iter_invariant p (Z.to_nat bls12_377_divstep_iters) 1 p x 0 1 x
                  p_pos p_odd) as Hinv.
    ...
    pose proof (by_convergence_dfg_bls12_377 x Hx Hgcd) as Hconv.
    pose proof (iter_dfg_agree (Z.to_nat bls12_377_divstep_iters) p 1 p x 0 1) as Hagree.
    rewrite Hiter in Hagree.
    destruct (iter_divstep_dfg (Z.to_nat bls12_377_divstep_iters) 1 p x)
    ...
    - replace (1 <? 0) with false by reflexivity.
      ...
      assert (Hmod : (v_N * x) mod p = (2 ^ Z.of_nat (Z.to_nat bls12_377_divstep_iters)) mod p). {
        assert (Hp : p <> 0) by (pose proof p_pos; lia).
        apply Z.mod_divide in Hv_inv; [|exact Hp].
        destruct Hv_inv as [k Hk].
        assert (v_N * x = 2 ^ Z.of_nat (Z.to_nat bls12_377_divstep_iters) + k * p) by lia.
        rewrite H.
        rewrite Zplus_mod, Z_mod_mult, Z.add_0_r, Z.mod_mod by lia.
        reflexivity.
      }
      rewrite <- Zmult_mod_idemp_r, Hmod, Zmult_mod_idemp_r.
      rewrite Z2Nat.id by (unfold bls12_377_divstep_iters; vm_compute; discriminate).
      exact precomp_times_pow2.
    - replace (-1 <? 0) with true by reflexivity.
      replace (2 ^ Z.of_nat (Z.to_nat bls12_377_divstep_iters) * -1)
        with (- (2 ^ Z.of_nat (Z.to_nat bls12_377_divstep_iters))) in Hv_inv by ring.
      replace (v_N * x - - 2 ^ Z.of_nat (Z.to_nat bls12_377_divstep_iters))
        with (v_N * x + 2 ^ Z.of_nat (Z.to_nat bls12_377_divstep_iters)) in Hv_inv by ring.
      set (iters := Z.of_nat (Z.to_nat bls12_377_divstep_iters)) in *.
  ...
  Theorem fp_inv_correct : forall x,
    ...
    (fp_inv_spec x * x) mod p = 1.
  Proof.
    intros x Hx. apply fp_inv_correct_ax; auto. apply bls12_377_coprime; auto.
  Qed.

End FpInv.

(* ================================================================== *)
(* Concrete parameters for BLS12-377 divstep                           *)
(* ================================================================== *)

Definition bls12_377_r' : Z :=
  Eval vm_compute in
    (let r := 2^machine_wordsize in
     let p := bls12_377_p in
     (fst (Z.ggcd r p))).
...
Lemma bls12_377_p_pos : 1 < bls12_377_p.
Proof. vm_compute. reflexivity. Qed.

Lemma bls12_377_p_fits : bls12_377_p < (2^machine_wordsize) ^ Z.of_nat n.
Proof. vm_compute. reflexivity. Qed.

Lemma bls12_377_p_odd : Z.odd bls12_377_p = true.
Proof. vm_compute. reflexivity. Qed.
