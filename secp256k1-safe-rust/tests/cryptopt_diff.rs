...
//! leaves they replace.
//!
//! `secp256k1::fp_mul` / `secp256k1::fp_square` are the CryptOpt assembly
//! when `build.rs` linked it and the fiat functions otherwise;
//! `secp256k1::fp_mul_fiat` / `secp256k1::fp_square_fiat` are always the
//! fiat functions.  Every case below asserts bit-for-bit equality of the
//! two, so on a build with the assembly this is a real differential test and
//! on a build without it the assertions are trivially true.
//!
...
//! which target the same word-by-word Montgomery 4x64 representation this
//! crate uses.  CryptOpt's `fiat_secp256k1_dettman_*` seeds implement a
//! different (unsaturated) representation and must not be substituted here.

use secp256k1::{fp_mul, fp_mul_fiat, fp_square, fp_square_fiat, fp_to_montgomery, Fp, FpRaw};
...
fn sub_small(w: &[u64; 4], d: u64) -> [u64; 4] {
    let mut o = *w;
...
fn edges() -> Vec<Fp> {
    ...
    let mut one = Fp([0; 4]);
    fp_to_montgomery(&mut one, &FpRaw([1, 0, 0, 0]));
    vec![
        Fp([0, 0, 0, 0]),
        Fp([1, 0, 0, 0]),
        Fp(sub_small(&P, 1)),
        ...
        Fp([0, 0, 0, !0]),
        // Sits just below the 2^32 + 977 gap that makes p not a power of two.
        Fp([0xffff_fffe_ffff_fc2e, 0, 0, 0]),
        Fp([0, !0, 0, !0]),
        Fp([!0, 0, !0, 0]),
        Fp([0x0123_4567_89ab_cdef, 0xfedc_ba98_7654_3210, !0, 1]),
    ]
}

/// Below p? Only used to keep the random generator inside the input domain.
fn lt_p(w: &[u64; 4]) -> bool {
    for i in (0..4).rev() {
        if w[i] != P[i] {
            return w[i] < P[i];
        }
...
impl Rng {
    ...
    fn fe(&mut self) -> Fp {
        loop {
            let mut w = [self.word(), self.word(), self.word(), self.word()];
            if w[0] & 1 == 0 {
...
fn which_backend() {
    ...
    println!(
        "secp256k1 field leaves: {}",
        if secp256k1::CRYPTOPT_ASM {
