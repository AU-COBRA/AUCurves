//! Basic known-answer tests for BN256.
//!
//! Verifies the Rust stub leaf ops + safe-Rust tower call chain.
//! Limited to round-trip and identity properties — full pairing
//! KATs require IETF or library-cross-checked test vectors.

use bn256::*;

fn fp_zero() -> Fp { Fp([0; 4]) }
...
fn sub_self_is_zero() {
    let one = fp_one();
    let mut zero = fp_one();  // start non-zero to detect bugs
    fp_sub(&mut zero, &one, &one);
    assert_eq!(zero.0, [0u64; 4]);
}
